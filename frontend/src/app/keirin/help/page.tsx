...
// 現行は S1/7SS/7S の3ペーパーランク。旧S1(6車三連単)/A(一致波乱二連単)は正規プロトコル
// 再検証で不合格のため 2026-07-17 全廃。S2(旧U)/S3(旧M)は対象レース数・的中率・期待値の
// 観点で継続困難と判断し 2026-07-21 全廃（内部rankコード7PLUS_U/7PLUS_Mへの新規書き込み終了）。
// 7SS/7S は元々1つだったS7ランク（単勝×複勝指数トップ3重なり軸×波乱度選出）を、軸2車が
// WINTICKET公式◎◯と重なる数に応じて表示ランクを分割したもの（内部rankコードは7SS/7Sいずれも
// SEVEN_S7のまま・gate_labelカラムで区別）。正規プロトコルの検証/テスト分割は未実施のため
...
const RANKS = [
  {
    ...
    bg: "#ca8a04",
    label: "7SS",
    title: "7SSランク（単勝×複勝指数トップ3重なり軸×波乱度選出／軸2車がWT◎◯と完全不一致・2026-07-21導入）",
    ...
    condition:
      "軸2車 = 単勝指数上位3∩複勝指数上位3の重なり車 ｜ 7SS: 軸2車がWINTICKET公式◎◯と全く重ならない場合に無条件で採用 ｜ 三連複 軸2車+残り5車のいずれか1車の5点（オッズ下限なし）",
    detail:
      "単勝モデルと複勝モデルの両方が上位に評価する2車を軸にする設計のうち、軸2車がWINTICKET公式の◎◯（人気予想）と一切重ならないレースだけを抽出した最上位区分。市場のコンセンサスから外れた軸ほど回収率が高いという発見（2026-07-21、重なり0=ROI408.1%／重なり1=148.7%／重なり2=完全一致=75.7%赤字と判明）を受け、旧S7ランクをWT◎◯重なり数で7SS/7Sの2表示ランクへ分割。母数は少ないが的中率39.4%・ROI232.8%と全ランク中最高。2026-07-23〜07-27の間、軸2車の級班に各グレード最上位(S1級/A1級)を含まないサブセットを「7SS+」として観察用に分岐表示していたが、サンプル数不足（365R）のためユーザー判断で廃止・SSへ統合した。",
  ...
  },
  {
    ...
    bg: "#16a34a",
    label: "7S",
    title: "7Sランク（単勝×複勝指数トップ3重なり軸×波乱度選出／軸2車の片方だけがWT◎◯と重なる・2026-07-21導入）",
    subtitle: "7車 ｜ 三連複 2軸総流し(5点)",
    test: "120.6%",
    testSub: "honest全期間実績のみ（正規プロトコルの検証/テスト分割は本ランクでは未実施）",
    full: "120.6%",
    ...
    condition:
      "軸2車 = 単勝指数上位3∩複勝指数上位3の重なり車 ｜ 7S: 軸2車の片方だけがWT◎◯と重なる場合にaxis_sum(軸2車の複勝指数合計)昇順で日次10件採用 ｜ 三連複 軸2車+残り5車のいずれか1車の5点（オッズ下限なし）",
    detail:
      "7SSと同じ軸選定ロジックのうち、軸2車の片方だけがWINTICKET公式の◎◯と重なるレースを対象に、波乱度指数(axis_sum)が小さい順に日次10件を機械的に採用する主力区分。母数は7SSの約9.5倍（8,984R・約9.6R/日）と多く、的中率も7SS(39.4%)に迫る36.0%を確保しつつROI120.6%で黒字を維持する。",
  ...
  },
  {
    ...
    detail:
      "「1着になるか」だけを学習した専用モデルで軸を固定し、3着内モデルで相手2車を選ぶ新設計。top3_gap閾値は当初0.15→同日中に0.22へ引き上げていたが、2026-07-21に高配当（万車券含む）の取りこぼしを防ぐ方針へ再設計し0.15へ戻したうえ、軸の単勝勝率が高い（本命決着になりやすい）レースを除外する条件を追加。2026-07-22には軸がS1級・A1級（各グレード内の格上認定選手）の場合を除外する条件も追加。的中率は変えずに母数を絞り込みROI・配当の質を上げる狙い（5万円以上の高配当payoutは85.7%が残存）。7SS/7Sとの重複はわずかとほぼ独立。",
  ...
  },
  {
    ...
    detail:
      "7SS/7Sはボリュームが小さい（合計約1.2R/日）ため、ROIはやや落ちても的中率のあるゾーンで推奨数を増やしたいというユーザー要望を受けて新設。7SS/7Sの判定条件のうち「ちょうど1つだけ僅かに外れた惜しいレース」を対象にすると、直近7四半期連続でROI100%超（101.7〜172.9%）という頑健な結果を確認できた。wt_overlap==2（WT◎◯完全一致）側も含めて厳選できないか検証したが、こちらは全10四半期でROI70〜96%と一度も安定して100%を超えず不採用とした。",
  ...
  },
  {
    ...
    subtitle: "9車 ｜ 三連複 2軸総流し(7点) ｜ S9より低配当帯を狙いボリュームを増やす枠",
    test: "108.4%",
    testSub: "honest全期間実績のみ（正規プロトコルの検証/テスト分割は本ランクでは未実施）",
    full: "108.4%",
    fullSub: "全期間実績（2024-01-01〜2026-07-26・四半期walk-forward）: 1,032R・的中35.1%・ROI 108.4%（直近四半期は約0.5〜1.7R/日）",
    condition:
      "軸2車 = S9と同じ選定（単勝指数上位3∩複勝指数上位3の重なり車、車数非依存の汎用実装を再利用） ｜ S9の2条件（entropy≤1.9938・軸2車のWT◎◯△一致数≤1）のうち、ちょうど1つだけ不合格の候補を採用（0個=S9・2個とも不合格は対象外） ｜ 三連複 軸2車+残り7車のいずれか1車の7点（オッズ下限なし）",
    detail:
      "7Aの9車版。S9自体が低ボリューム（約0.25R/日）のため、9A追加後も9車合計は1R/日未満に留まるが、7Aと同じ「境界ランク」設計で的中率のあるゾーンを追加した。9車立ては元々出走レースが少ない（全体の約8%）ため、7Aほどの絶対的なボリューム増加は見込めない。",
...
const TERMS = [
  ...
  {
    term: "axis_sum（波乱度指数）",
    def: "7SS/7Sの軸2車（単勝指数上位3∩複勝指数上位3の重なり車）の複勝指数合計。7Sランクはこの値が小さい順に日次10件を採用する。",
  },
  {
    term: "7SS+（2026-07-23導入・2026-07-27廃止）",
    def: "7SSランクのうち、軸2車の級班に各グレード内の最上位クラス（S1級・A1級）を含まないレースの表示上の内訳として観察用に追加していたが、サンプル数（365R）が少なすぎるとのユーザー判断により2026-07-27にSSへ統合・廃止した。",
  },
  {
    term: "正規プロトコル（テスト回収率）",
    def: "学習=2025-03-31以前・検証=2025-04-01〜2026-03-31の1年（条件選択はここだけ）・テスト=2026-04-01〜07-15（選択条件のみ1回評価）の時系列3分割。テスト期間を条件探しに使わないため「直近だけ良い」レジーム依存の条件を検出・排除できる。現行ランクではS1のみこのプロトコルの合格ランク（7SS/7Sは旧S7からの分割ランクのためプロトコル未実施・honest全期間実績で運用判定）。",
  },
  {
    ...
    <div className="w-full sm:max-w-3xl sm:mx-auto px-3 sm:px-4 py-4 space-y-5">
      ...
      <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-2">
        ...
        <p className="text-xs text-gray-400">
          全期間実績（7SS/7S=2026-07-23・7A=2026-07-27・S1=2026-07-22・実精算）: 7SS 232.8%(943R) ／ 7S 120.6%(8,984R) ／ 7A 107.4%(6,029R) ／ S1 182.5%(6,426R)。
          7SS/7S/7Aは正規プロトコルのテスト分割を経ていない全期間honest実績（詳細は各ランク説明参照）。
          観察用サブランク「7SS+」（軸2車の級班に各グレード最上位を含まないSS内訳）はサンプル数不足のため2026-07-27にSSへ統合・廃止した。
        </p>
        <p className="text-xs text-gray-400">
      ...
      <section className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-4 py-2.5 border-b border-gray-100 bg-gray-50">
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            ...
            7SS/7Sは旧S7からの分割ランクのためこのプロトコルは未実施（全期間honest実績のみ）。
          </p>
        </div>
      ...
      <section className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-1.5">
        ...
        <ul className="text-xs text-amber-700 space-y-1 list-disc list-inside">
          ...
          <li>S1/7SS/7S の過去実績はOOS評価モデルによる遡及再判定（買い目は発走前オッズ盤面基準）。早期年の成績が低く見えるのは学習データが少ない時期のモデルで判定しているため（リークなし遡及の仕様）。</li>
          ...
          <li><b>同日、表示ランクを7SS/7Sの2ランクへ再編</b>。内部rankコード（SEVEN_S7）は変更せず、軸2車がWT◎◯と全く重ならない選出を<b>7SS</b>（943R・的中39.4%・ROI232.8%）、片方だけ重なる選出を<b>7S</b>（8,984R・的中36.0%・ROI120.6%）として表示・集計・通知を分離した（内訳の区別は既存のgate_labelカラムを流用）。</li>
        </ul>
      </section>
