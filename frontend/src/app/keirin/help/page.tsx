...
// 独立戦略が導入された。
//
// その RANK_7SS（波乱軸選出）は 2026-08-02 に全廃した（ROI73.5%・n=16,298 と
// 控除率75%を下回り続けたため）。
//
// 【2026-08-05】さらに同じ "7SS" ラベルを **3度目** に再利用する形で、
// RANK_7SS（entropy不合格 × 軸2車が同一ライン・keirin PR#10 `cb419d4`）を新設した。
// 過去2つの「7SS」とは無関係の別戦略である点に注意（picks_history の旧7SS行は
// 2026-08-02 に全削除済み＝0件なので、live成績が混ざることはない）。
//
// 現行は RANK_7SS / RANK_7S / RANK_7A / RANK_7B / RANK_9S / RANK_9A の6ペーパーランク。
// 数値は keirin側 src/strategy_wt.py の CURRENT_PAPER_RANKS に基づく。
// ---------------------------------------------------------------------------
...
const RANKS = [
  {
    ...
    full: "—",
    fullSub: "2026-08-05 新設のためlive実績はこれから蓄積（picks_historyの旧同名ランク行は0件）",
    condition:
      "軸2車 = 7S/7Aと同じ3ヘッド選定（軸1=1着率最上位・軸2=z(3着内率)−0.3×z(大敗率)最上位） ｜ 軸2車のWT公式印◎◯重なり数∈{0,1} ∧ axis_sum≤1.40 ∧ フィールド指数エントロピー>1.8329（＝7Sのentropyゲートだけ不合格＝荒れ模様） ∧ 軸1と軸2が同一ライン（wt_entries.line_group一致） ｜ 三連複 軸2車+残り5車のいずれか1車の5点（オッズ下限なし）",
    detail:
      "⚠️ 2026-08-02に全廃した旧7SS（波乱軸選出・穴レース検知・ROI73.5%）とは無関係の別戦略で、名前だけを引き継いでいる。7S/7Aには「軸2車の3着内率が独立仮定より2.7〜2.9pt低い」という負の相関があり、7車で3枠を奪い合うためと考えられていた。その正体がライン関係であることを2026-08-05に特定した——別ラインの2車は互いに competitor だが、同一ラインなら連動して残る。line_groupはDBに存在しながら軸選定にもゲートにも使われていなかった。確認窓4窓（掃引に未使用・閾値固定）で、entropy不合格群のうち同一ラインに限ると 1.90件/日・的中41.2%・ROI85.9%（別ラインは26.7%・75.0%）。⚠️ 単体では4窓中1窓が74.3%と「全窓75%以上」を0.7pt割るがユーザー承知の上で採用（7SS+7S+7Aの構成全体では79.2/84.2/83.8/80.8%と全窓合格）。なお同一ライン条件はaxis_sum不合格群（=7A）には効かず（確認窓80.4% vs 別ライン78.7%・基準80.8%）、7Bにも効かない（75.8% vs 75.3%）。ROIは 7SS 85.9% > 7S 84.4% > 7A 80.8% と単調になった。",
    investment: "名目 100円/点・5点/レース・約1.9件/日",
  },
  {
    key: "7S",
    bg: "#16a34a",
  ...
  },
  {
    ...
    bg: "#b45309",
    label: "7B",
    title: "7Bランク（◎◯一致だが順序・相手で市場と不一致／内部名RANK_7B）",
    subtitle: "7車 ｜ 三連複 2軸・相手絞り(3点) ｜ 7S/7Aの枯渇を補う増枠",
    test: "78.7%",
    testSub: "honest実績（月次凍結vintage・2025-01〜2026-08 / 3,230R・5.58件/日）",
    full: "__FULL__",
    fullSub: "__FULLSUB__",
    condition:
      "軸2車 = 7S/7Aと同じ選定（単勝指数上位3∩複勝指数上位3の重なり車） ｜ 軸2車がWT公式印◎◯と完全一致（重なり2）＝7S/7Aとは母集団が排他 ｜ ゲート: 順序不一致（単勝モデルの最上位がWT◎ではない）のみ。axis_sum・entropyのゲートは掛けない ｜ 買い目は三連複 軸2車+相手（残り5車からWT△を除外し複勝指数上位3車）の3点（オッズ下限なし）",
    detail:
      "7S/7Aは「軸2車がWT公式印◎◯と完全一致するレース」を配当が縮むという理由で全除外している。しかしモデルが公式印へ収束する方向に進化した結果、この完全一致が母集団の8割近くを占めるようになり、7S/7Aの対象が2024-01の39件/日から12件/日まで枯渇した（2026-08-02・08-03は◎◯不一致レースを全件採用しても6件・5件）。そこで「軸は一致していても、順序（モデル1位≠◎）と相手（△を買い目から外す）で市場と不一致なら価値がある」というユーザー方針を検証し、2026-08-03に新設した。honest検証（579日・36,791候補）では、◎◯一致レース全体は的中率56.3%と高い一方でROI72.4%・的中中央値4.0倍・ガミ率60.2%（当たっても賭け金を割る率）と配当が消えていた。entropy・相手集中度・点数の120通りを掃引してもROIは全て72〜76%で平坦だったが、相手からWT△を外すと的中中央値が3.4倍→6.1倍・ガミ率42.4%→10.8%・20倍超の的中が175本→435本へ回復。さらに順序不一致を重ねた本ランクはROI78.7%（7S+7Aの78.3%と同水準）・ガミ率8.1%・的中中央値6.8倍となり、年次2025:78.6%/2026:78.8%・四半期7期すべて68〜89%で崩れなし・月次ROI60%割れ0回を確認した。的中率は25.2%と7S/7A（36.9%）より低いが、ROIを落とさずに件数・見せ場・ガミ率を改善する枠という位置づけ（ROIそのものは控除率75%の壁を越えない）。",
    investment: "名目 100円/点・3点/レース・約5.6件/日",
  },
  {
    key: "9S",
    bg: "#0891b2",
