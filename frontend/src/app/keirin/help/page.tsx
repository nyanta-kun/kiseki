...
const TERMS = [
  {
    ...
    def: "出走表に表示するAIモデルの予測確率。単勝指数=1着専用モデル(1着モデル)の予測確率、複勝指数=3着内モデルの予測確率が元。各選手独立モデルの生確率のためレース内合計が揃わないので、表示時にレース内で単勝=合計100%・複勝=合計300%(出走3名未満のレースは出走数×100%)になるよう按分している。既存の「指数」列（競走得点）とは別軸の情報として単→複→指数の順に並べている。2024年以降の過去レースにも同じ四半期ウォークフォワードモデルで遡及反映済み。",
  },
  {
    term: "top3_gap",
