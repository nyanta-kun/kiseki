...
const RANKS = [
  {
    ...
    bg: "#a16207",
    label: "7SS+",
    title: "7SS+ランク（7SS内、軸2車の級班に各グレード最上位(S1級/A1級)を含まない観察用サブランク・2026-07-23導入）",
    subtitle: "7車 ｜ 三連複 2軸総流し(5点) ｜ 7SSの内訳（買い目・投資額は7SSと同一）",
    test: "362.1%",
    testSub: "honest全期間実績のみ（正規プロトコルの検証/テスト分割は本ランクでは未実施）",
    full: "362.1%",
    fullSub: "全期間実績（2024-01-01〜2026-07-23）: 365R・的中40.8%・ROI 362.1%",
    condition:
      "軸2車 = 単勝率上位3∩複勝率上位3の重なり車 ｜ 7SS+: 軸2車がWINTICKET公式◎◯と全く重ならず、かつ軸2車の級班に各グレード最上位(S1級・A1級)を含まない場合に採用 ｜ 三連複 軸2車+残り5車のいずれか1車の5点（オッズ下限なし・買い目は7SSと同一）",
    detail:
      "7SSのうち、軸2車の級班が各グレード内の最上位クラス（S1級・A1級）を含まないレースだけを抽出した観察用の内訳区分。「格上」認定選手が軸に含まれると回収率がやや下がる傾向があるかを検証する目的の表示分岐で、買い目・投資額は7SSと同一（集計上の二重計上はしない）。",
    investment: "名目 100円/点・5点/レース・7SSの内訳のため合算には含めない",
  },
  {
    key: "7SS",
    bg: "#ca8a04",
    ...
    detail:
      "単勝モデルと複勝モデルの両方が上位に評価する2車を軸にする設計のうち、軸2車がWINTICKET公式の◎◯（人気予想）と一切重ならないレースだけを抽出した最上位区分。市場のコンセンサスから外れた軸ほど回収率が高いという発見（2026-07-21、重なり0=ROI408.1%／重なり1=148.7%／重なり2=完全一致=75.7%赤字と判明）を受け、旧S7ランクをWT◎◯重なり数で7SS/7Sの2表示ランクへ分割。母数は少ないが的中率39.4%・ROI232.8%と全ランク中最高。",
...
const TERMS = [
  ...
  {
    term: "7SS+（2026-07-23追加・観察用サブランク）",
    def: "7SSランクのうち、軸2車の級班に各グレード内の最上位クラス（S1級・A1級）を含まないレースの表示上の内訳。買い目・投資額は7SSと同一で、集計上も7SSに含まれる（トップラインの合算には二重計上しない）。軸が「格上」でない7SSがより回収率が高いか観察する目的の表示分岐。",
  },
  {
    term: "正規プロトコル（テスト回収率）",
    ...
    <div className="w-full sm:max-w-3xl sm:mx-auto px-3 sm:px-4 py-4 space-y-5">
      ...
      <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-2">
        ...
        <p className="text-xs text-gray-400">
          全期間実績（7SS+/7SS/7S=2026-07-23・S1=2026-07-22・実精算）: 7SS+ 362.1%(365R) ／ 7SS 232.8%(943R) ／ 7S 120.6%(8,984R) ／ S1 182.5%(6,426R)。
          7SS+/7SS/7Sは正規プロトコルのテスト分割を経ていない全期間honest実績（詳細は各ランク説明参照）。
        </p>
        <p className="text-xs text-gray-400">
