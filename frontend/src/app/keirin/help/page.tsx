...
// 独立戦略が導入された。
//
// 現行は RANK_7S / RANK_7A / RANK_9S / RANK_9A / RANK_7SS の5ペーパーランク
// （表示ラベル 7S/7A/9S/9A/7SS）。数値は keirin側 src/strategy_wt.py の
...
const RANKS = [
  ...
  },
  {
    ...
    bg: "#ca8a04",
    label: "7SS",
    title: "7SSランク（波乱軸選出・穴レース検知／内部名RANK_7SS・2026-07-31新設）",
    subtitle: "7車 ｜ 三連複 2軸総流し(5点) ｜ モデル非依存の独立戦略（ROI改善ではなく高配当検知が目的）",
    test: "70〜72%",
    testSub: "TEST(2024-01-01〜2026-07-30)honest実績。控除率75%の壁を超えられずROI100%には届かない",
    full: "73.5%",
    fullSub: "全期間実績（2022-12-01〜2026-07-30）: 16,273R",
    condition:
      "軸1 = race_point（競走得点）単独レース内top1 ｜ 軸2 = WT公式印(prediction_mark∈{◯,△,✕})のうち軸1以外でthird_rate（3着内率）最大の1頭 ｜ 「穴指数」= 拮抗度を表す13特徴（競走得点/1着率/3着内率の最大値・標準偏差・1-2位差、ライン数・最大ライン人数・単騎数・ラインentropy）の標準化合算スコアがTRAIN上位20%点以上 ｜ 三連複 軸2車+残り5車流しの5点（100円/点・オッズ下限なし）",
    detail:
      "S7/S9（単勝・複勝の予測モデル出力）や旧S1（1着専用モデル）とは全く異なり、単勝×複勝の予測確率を一切使わない設計。TRAIN(2022-01-01〜2023-12-31・7車立てn=22,953)のみで13特徴の平均・標準偏差・符号を凍結し、TEST(2024-01-01〜2026-07-30)では再学習・再計算を一切行わない（honest固定閾値）。軸1はrace_point単独top1が「1着率+3着内率」複合より安定して優れ、軸2はWT公式印(◯△✕)のうち軸1以外でthird_rate最大の1頭が最も目減りが少ないと判明。ROIはTESTで70〜72%と控除率75%の壁は超えられないが、的中時配当は最高354.2倍（TEST全体）・最高250.5倍（穴指数上位20%）に達し、購入者へのアピール（見せ場）を目的としてユーザー判断により採用された。他の4ランク（7S/7A/9S/9A）が的中率・ROI重視の設計なのに対し、7SSは高配当の的中頻度そのものを目的とする点で性質が異なる。",
    investment: "名目 100円/点・5点/レース",
  },
];
...
const TERMS = [
  {
    term: "単勝率・複勝率",
    def: "出走表に表示するAIモデルの予測確率。単勝率=1着専用モデルの予測確率、複勝率=3着内モデルの予測確率が元。各選手独立モデルの生確率のためレース内合計が揃わないので、表示時にロジット(対数オッズ)空間で一律シフトして単勝=合計100%・複勝=合計300%(出走3名未満のレースは出走数×100%)になるよう補正している。7S/7A/9S/9Aの軸選定に使う（7SSは使わない・下記参照）。",
  },
  {
    term: "指数エントロピー",
  ...
  {
    term: "穴指数（7SS・RANK_7SS_SCOREとも表記）",
    def: "7SS専用のスコア。競走得点・1着率・3着内率の最大値/標準偏差/1-2位差、ライン数・最大ライン人数・単騎数・ラインentropyの13特徴を、TRAIN(2022-01-01〜2023-12-31)だけで凍結した平均・標準偏差・符号で標準化し合算したもの。値が高いほど「際立った実力上位馬が不在＝拮抗している」＝三連複が高配当になりやすいレースと解釈する。7S/7A/9S/9Aの指数エントロピーとは別物（単勝・複勝モデルを使わない）。",
  },
  {
  ...
  {
    term: "三連複",
    def: "1〜3着を順不同で当てる券種。現行の全ランク（7S/7A/9S/9A/7SS）はいずれも三連複の2軸総流し（軸2車+残りのいずれか1車。7車=5点・9車=7点）。",
  },
];
    ...
    <div className="w-full sm:max-w-3xl sm:mx-auto px-3 sm:px-4 py-4 space-y-5">
      ...
      <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-2">
        ...
        <p className="text-xs sm:text-sm text-gray-600 leading-relaxed">
          LightGBMモデルによる出走選手の3着内確率予測をもとに、期待回収率が高いレースだけを自動推奨します（7SSのみモデル予測を使わない例外・下記参照）。
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 pt-1">
          ...
          <div className="bg-gray-50 rounded-lg p-2.5 text-center">
            ...
            <p className="text-lg font-bold" style={{ color: "#ca8a04" }}>73.5%</p>
            <p className="text-xs text-gray-400">16,273R</p>
          </div>
        </div>
        <p className="text-xs text-gray-400">
          全期間実績（2026-07-31時点・実精算）: 7S 79.1%(6,572R) ／ 7A 77.4%(11,419R) ／
          9S 79.2%(109R) ／ 9A 71.9%(967R) ／ 7SS 73.5%(16,273R)。
          いずれも正規プロトコルのテスト分割を経ていない全期間honest実績（詳細は各ランク説明参照）。
          7SSのみROI改善ではなく高配当の的中頻度（見せ場）を目的とした設計であり、
          ROIが控除率75%の壁を超えないことは織り込み済みの仕様。
        </p>
      </section>
      ...
      <section className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-1.5">
        ...
        <ul className="text-xs text-amber-700 space-y-1 list-disc list-inside">
          ...
          <li>
            <b>同日、新たに「7SS」（RANK_7SS・波乱軸選出/穴レース検知）を独立の新ランクとして
            導入</b>（keirin側commit dc89f14）。上記で廃止した旧7SS（gate_label由来）とは
            軸選定ロジック・意味とも無関係の全く新しい戦略で、単勝×複勝の予測モデルを
            使わず race_point・WINTICKET公式印・ライン構成のみで判定する。ROI改善では
            なく高配当の的中頻度（見せ場）が目的で、TEST honest実績はROI70〜72%
            （控除率75%の壁を超えない）。的中時最高配当354.2倍。
          </li>
          <li>
          ...
          <li>7S/7A/9S/9A/7SS の過去実績はOOS評価モデルによる遡及再判定（買い目は発走前オッズ盤面基準）。早期年の成績が低く見えるのは学習データが少ない時期のモデルで判定しているため（リークなし遡及の仕様）。7SSはモデル非依存のためこの限りではない。</li>
