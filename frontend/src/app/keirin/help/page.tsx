...
const RANKS = [
  {
    ...
    bg: "#ca8a04",
    label: "SS",
    title: "SSランク（単勝×複勝指数トップ3重なり軸×波乱度選出／軸2車がWT◎◯と完全不一致／ペーパー検証中・2026-07-21導入）",
    subtitle: "7車 ｜ 三連複 2軸総流し(5点) ｜ 検証記録のみ・賭けなし",
    test: "232.8%",
    ...
    investment: "ペーパートレード（名目 100円/点・5点/レース・約1.0R/日）",
  },
  {
    key: "S",
    bg: "#16a34a",
    label: "S",
    title: "Sランク（単勝×複勝指数トップ3重なり軸×波乱度選出／軸2車の片方だけがWT◎◯と重なる／ペーパー検証中・2026-07-21導入）",
    subtitle: "7車 ｜ 三連複 2軸総流し(5点) ｜ 検証記録のみ・賭けなし",
    test: "120.6%",
    testSub: "honest全期間実績のみ（正規プロトコルの検証/テスト分割は本ランクでは未実施）",
    full: "120.6%",
    ...
    investment: "ペーパートレード（名目 100円/点・5点/レース・約9.6R/日）",
  },
  {
    key: "S1",
    bg: "#d97706",
    ...
    full: "143.3%",
    fullSub: "全期間実績（2024-01-01〜2026-07-22）: 13,489R（約14.4R/日）・的中10.6%・ROI 143.3%",
    condition:
      "軸 = 1着専用モデルのレース内1位（固定） × 相手 = 3着内モデルで軸を除いた上位2頭(p1,p2) × top3_gap(p1-p2の3着内確率差) ≥ 0.15 × 軸の単勝勝率 ≤ 50%（本命決着＝低配当レースを除外） ｜ 三連単 軸→p1→p2, 軸→p2→p1 の2点流し（目オッズ下限なし）",
    detail:
      "「1着になるか」だけを学習した専用モデルで軸を固定し、3着内モデルで相手2車を選ぶ新設計。top3_gap閾値は当初0.15→同日中に0.22へ引き上げていたが、2026-07-21に高配当（万車券含む）の取りこぼしを防ぐ方針へ再設計し0.15へ戻したうえ、軸の単勝勝率が高い（本命決着になりやすい）レースを除外する条件を追加。的中率は下がるがROIと高配当の再現率を優先する狙い。SS/Sとの重複はわずかとほぼ独立。",
    ...
    <div className="w-full sm:max-w-3xl sm:mx-auto px-3 sm:px-4 py-4 space-y-5">
      ...
      <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-2">
        ...
        <p className="text-xs text-gray-400">
          全期間実績（S1=2026-07-22・SS/S=2026-07-20・実精算）: S1 143.3%(13,489R) ／ SS 232.8%(943R) ／ S 120.6%(8,984R)。
