...
const RANKS = [
  ...
  },
  {
    ...
    bg: "#7c3aed",
    label: "S3",
    title: "S3ランク（◎不一致×軸信頼／ペーパー検証中・2026-07-19 win_rank OR拡張）",
    subtitle: "7車 ｜ 三連複 2車軸流し ｜ 検証記録のみ・賭けなし",
    test: "149.5%",
    testSub: "正規プロトコル: 検証1年 158.2%(531R) → テスト 149.5%(152R・目≥15限定)（実精算・ペーパー）",
    condition:
      "WINTICKET◎ ≠ システム◎（モデル1位）かつ [gap12 ≥ 0.10（軸信頼ゲート） または 1着モデル内順位 ≥ 3位（勝ちきれない評価ゲート）] ｜ システム◎×同ライン「逃」相方の三連複流し・目オッズ ≥ 15倍のみ",
    detail:
      "外部予想（WINTICKET◎）と当システムの◎が割れたレースを2つの独立シグナルで拾う。①gap12≥0.10（不一致システム◎の3着内率が68%→73%に上がる帯）②1着専用モデル（3着内モデルとは別に学習）内でシステム◎が3位以下に評価される帯（＝「勝ちきれない」と評価されるほど市場オッズが甘くなりROIが上がる逆説的シグナル）。①②はほぼ独立（重複率5%）で、OR統合すると母数が旧定義の約2倍に拡大しつつROIも維持。S2と同一買い目になる場合はS2優先で記録しない。live 100R 以上で採否判定。",
    investment: "ペーパートレード（名目 100円/点・約1.2R/日）",
  },
];
...
const PROTOCOL_RESULTS = [
  { month: "検証 2025-04〜2026-03", s2: "127.8%（320R）", s3: "158.2%（531R）" },
  { month: "テスト 2026-04〜07-15", s2: "117.1%（87R・的中14.9%）", s3: "149.5%（152R・目≥15限定）" },
];
...
const TERMS = [
  ...
  {
    term: "1着モデル",
    def: "3着内モデルとは別に「1着になるか」だけを学習したAIモデル（2026-07-19導入）。3着内モデルより判別力が高い（AUC0.83 vs 0.78）。S3ではシステム◎がこのモデルで低評価（3位以下）なレースを、市場が甘くなりやすい帯として狙う。",
  },
  {
    term: "指数エントロピー",
    ...
    <div className="w-full sm:max-w-3xl sm:mx-auto px-3 sm:px-4 py-4 space-y-5">
      ...
      <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2 pt-1">
          ...
          <div className="bg-gray-50 rounded-lg p-2.5 text-center">
            ...
            <p className="text-lg font-bold text-violet-600">149.5%</p>
            <p className="text-xs text-gray-400">152R ・ 目≥15限定（検証1年 158.2%）</p>
          </div>
        </div>
      ...
      <section className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-1.5">
        ...
        <ul className="text-xs text-amber-700 space-y-1 list-disc list-inside">
          ...
          <li>2026-07-16 に指数へ競走得点トレンド4特徴を追加（選手の成長・好不調を反映／モデル44特徴化）。2026-07-18 にレース単位のS/B取得・上がりタイム由来4特徴を追加（48特徴化）。2026-07-19 に「1着になるか」だけを学習する専用モデルを新規導入し、S3の軸信頼ゲートに統合（win_rankゲート）。本ページの検証数値は都度新指数で再計算済み。</li>
