      ...
      <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-xs sm:grid-cols-4">
        ...
        <div>
          ...
          {/* 🔴 `min_payout` は入稿時点の板由来で楽観的（実測 中央 確定/表示 0.860・
              ...
              🔴 2026-08-29: この額は**買う点数を織り込んだ**下側25%分位になった
              （旧実装は1点あたりの分位の最小で、5点の商品では実測 74.5% が割っていた）。
              計算の正本は backend/src/services/keirin_payout_floor.py。 */}
          {p.min_payout_low !== null && (
            <span
              className="ml-1 text-[10px] text-amber-600 dark:text-amber-400"
              title="確定までにオッズが下振れした場合の、買った点のうち一番低い払戻（下側25%分位）。買う点数が多いほど深く見積もる。承認判断はこちらを見てください。"
            >
