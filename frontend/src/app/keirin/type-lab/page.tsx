...
const PLAN_NOTE: Record<string, string> = {
  // 段分け商品（2026-09-15〜・7車の主力）。型ではなく軸信頼の段で決まる。
  T_firm: "固め：三連単 確率上位を3〜5点・払戻をそろえる（合成2.2倍）",
  T_mid: "広め：三連単 3〜8点で押さえる・払戻をそろえる（合成2.2倍）",
  T_axis: "手広く：荒れの中でも1着率が抜けた車を1着に固定し、2・3着を確率順に組む",
  T_upset: "荒れ：三連単 人気1位のラインを1-2着から外し、払戻10万円前後",
  A_hit: "三連単 1着=軸1・2着=軸2 固定 → 3着へ3車（表示的中）",
  A_trio: "三連複 軸2車＋相手2点（順序を捨てて当たる回数を取る）",
  A_ana: "三連単 軸1を外した6車から確率上位5点（軸が飛ぶ側）",
  A_pay: "三連単 1着=軸1固定・2着2車 → 3着流し（払戻）",
  A_sign: "三連単 看板枠（当たれば15万円を狙うダッチ）",
  A_big: "三連単 特大狙い（軸1を外し、当たれば40万円を狙うダッチ）",
  B_hit: "三連単 確率上位を想定平均払戻3万円の床まで",
  B_sign: "三連単 看板枠（当たれば15万円を狙うダッチ）",
  B_big: "三連単 特大狙い（軸1を外し、当たれば40万円を狙うダッチ）",
  C_hit: "三連単 予測20倍以上から確率上位12点",
  C_sign: "三連単 看板枠（当たれば15万円を狙うダッチ）",
  C_big: "三連単 特大狙い（軸1を外し、当たれば40万円を狙うダッチ）",
  D_hit: "三連複 軸2車＋相手4点（最人気の相手を外す）",
  D_sign: "三連単 看板枠（当たれば15万円を狙うダッチ）",
  D_big: "三連単 特大狙い（軸1を外し、当たれば40万円を狙うダッチ）",
  E_hit: "三連単 予測30倍以上から確率上位14点",
  E_sign: "三連単 看板枠（当たれば15万円を狙うダッチ）",
  E_big: "三連単 特大狙い（軸1を外し、当たれば40万円を狙うダッチ）",
  ...
  // 逃げ先頭ライン（2026-09-24〜・検証中・型ラボが売らないレースへ・準決勝系と型Eを除く・上限なし）。詳細は /keirin/type-lab/line-lead
  L_lead: "逃げ先頭：得点1位でないラインの逃げ先頭→番手→3着・1レース1万円均等（型ラボが売らないレースへ・準決勝系と型Eを除く・上限なし・穴狙い）",
};
    ...
    <main className="w-full px-3 py-3 sm:mx-auto sm:max-w-6xl sm:px-4 sm:py-4 space-y-3 pb-16">
      ...
      <details className="rounded border border-gray-200 bg-white text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 text-xs">
        ...
        <p className="px-3 pb-3 leading-relaxed">
          ...
          ROI 83.0/89.1%）。決勝だけ残すのは<b>9車の決勝の 83.8% が型F</b>（残りの型C・型E は
          元々そのまま売る側）で、その型F は表示的中 14〜21%・払戻中央 3.5〜5.2万円と
          型F 全体より良い側だから。
