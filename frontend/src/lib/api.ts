...
export interface KeirinProposal {
   ...
   *  発走18時前・合成2.5倍以上のレースのうち**的中確率 Σp が最大**の1件に付く
   *  （2026-09-19〜。それ以前は期待値 EV 最大だった）。 */
  is_confident: boolean;
  /** 「自信あり」の選定に使った値。**2026-09-19 から Σp（買い目の的中確率・0〜1）**。
   *  🔴 名前は `_ev` だが**期待値ではない**（列名は互換のため据え置き）。
   *  `expected_value`（回収率・1.00で収支トントン）とは単位が違うので、
   *  同じバッジに載せないこと。中身は世代で変わる（旧EV → Σp → EV → Σp）。 */
...
// ---------------------------------------------------------------------------
// 型ラボ（2026-08-27 新設・2026-08-29 から売り物の唯一の出どころ）
// 🔴 既存の keirin 一覧・統計とは**別テーブル**（keirin.type_lab_picks）を見る。
//    当初はペーパー検証・実地検証の確認窓口として作った（旧ランクは #603 で全廃）。
