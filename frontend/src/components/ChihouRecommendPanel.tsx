...
function buildVenueStats(
  ...
  const result = new Map<
    string,
    Record<ChihouRecommendCategory, VenueCellStat>
  >();
  ...
  const emptyRow = (): Record<ChihouRecommendCategory, VenueCellStat> => ({
    sweet_spot: empty(),
    place_bet: empty(),
    low_odds_trusted: empty(),
    low_odds_untrusted: empty(),
  });
...
function VenueCell({ stat }: { stat: VenueCellStat }) {
  ...
  const hitText =
    stat.n_settled > 0
      ? `${stat.n_hits}/${stat.n_settled}`
      : `0/${stat.n_total}*`;
  const roi =
    stat.payout_count > 0
      ? stat.payout_sum / (stat.payout_count * 100)
      : null;
  return (
    <div className="text-[11px] leading-tight">
      ...
      {roi != null ? (
        <div
          className={cn(
            "text-[10px]",
            roi >= 1 ? "text-emerald-600 font-bold" : "text-gray-400",
          )}
        >
          {stat.bet_type === "place" ? "複" : "単"}
          {roi.toFixed(2)}
        </div>
      ) : (
      ...
      <table className="min-w-full text-xs border-collapse">
        <thead>
          <tr className="border-b border-gray-200">
            <th className="text-left py-1.5 px-2 font-medium text-gray-500 sticky left-0 bg-white">
              競馬場
            </th>
            {cats.map((c) => (
              <th
                key={c}
                className={cn(
                  "text-center py-1.5 px-2 font-medium border-l border-gray-100 min-w-[70px]",
                  CATEGORY_ACCENT[c].split(" ")[0],
                )}
              >
                {CATEGORY_LABEL[c]}
              </th>
          ...
          {venues.map((venue) => {
            ...
            return (
              <tr key={venue} className="border-b border-gray-50">
                <td className="py-1.5 px-2 font-semibold text-gray-700 sticky left-0 bg-white">
                  {venue}
                </td>
                {cats.map((c) => (
                  <td
                    key={c}
                    className="text-center py-1.5 px-2 border-l border-gray-100"
                  >
        ...
        <>
          ...
          {summary.n_settled < summary.n_total && (
            <span className="ml-1.5 text-gray-400">
              / 残{summary.n_total - summary.n_settled}件
            </span>
          )}
        </>
      )}
    ...
    <div className="space-y-0.5 text-right">
      {rec.target_horses.map((h) => (
        <div
          ...
          className={cn(
            "text-xs font-bold",
            (h.ev ?? 0) >= 1.5 ? "text-red-600" : "text-gray-600",
          )}
        >
...
function CategoryTable({
  ...
  const sorted = [...items].sort((a, b) => {
    const ta = a.race.post_time ?? "";
    const tb = b.race.post_time ?? "";
  ...
  return (
    <div>
      ...
      <div
        className={cn(
          "overflow-x-auto -mx-2 px-2 border-t",
          CATEGORY_ACCENT[category].split(" ")[2],
        )}
      >
        <table className="min-w-full text-xs border-collapse">
          <thead>
            <tr className="border-b border-gray-100 text-gray-500">
              <th className="text-left py-1 px-1.5 font-normal w-[6.5rem]">
                発走
              </th>
              ...
              {isPlace && (
                <th className="text-right py-1 px-1.5 font-normal w-12">複</th>
              )}
                ...
                >
                  ...
                  <td className="py-1.5 px-1.5 align-top">
                    <Link
                      ...
                      className="text-blue-700 hover:underline whitespace-nowrap font-medium"
                    >
                      {rec.race.course_name}
                      {rec.race.race_number}R
                    </Link>
                    <div className="text-[10px] text-gray-400 leading-tight">
                      {rec.race.distance}m{surface}
                    </div>
                  </td>
                  <td className="py-1.5 px-1.5 align-top">
                    <HorseInline rec={rec} />
                  ...
                  <td className="py-1.5 px-1.5 align-top">
                    <OddsInline rec={rec} field="win_odds" />
                  </td>
                  {isPlace && (
                    <td className="py-1.5 px-1.5 align-top">
                      <OddsInline rec={rec} field="place_odds" />
                    </td>
                  )}
                  {(category === "sweet_spot" || category === "place_bet") && (
                    <td className="py-1.5 px-1.5 align-top">
  ...
  return (
    <tr className="border-b border-gray-50 hover:bg-gray-50/50">
      ...
      <td className="py-1.5 px-1.5 align-top text-[11px]">
        <span
          className={cn(
            "text-[10px] font-bold px-1 py-0.5 rounded mr-1",
            rec.bet_type === "win"
      ...
      <td className="py-1.5 px-1.5 align-top text-[10px] whitespace-nowrap">
        {rec.odds_decision === "buy" && (
          <span className="text-emerald-600 font-bold">◎買</span>
        )}
        {rec.odds_decision === "pass" && (
          <span className="text-gray-400">✕見送</span>
        )}
      </td>
      <td className="py-1.5 px-1.5 align-top whitespace-nowrap">
        <ResultCell
            ...
            <table className="min-w-full text-xs border-collapse">
              <thead>
                <tr className="border-b border-gray-100 text-gray-500">
                  ...
                  <th className="text-left py-1 px-1.5 font-normal w-16">
                    判断
                  </th>
                  <th className="text-left py-1 px-1.5 font-normal w-20">
                    結果
                  </th>
                </tr>
              </thead>
              <tbody>
                {[...recList]
                  .sort((a, b) =>
                    (a.race.post_time ?? "").localeCompare(
                      b.race.post_time ?? "",
                    ),
                  )
                  .map((rec) => (
                    <LegacyRecRow key={rec.id} rec={rec} />
                  ))}
              </tbody>
            </table>
          </div>
