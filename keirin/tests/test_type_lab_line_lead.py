...
ユーザー決定（2026-09-24）: 「型ラボに新ランクを追加。実入稿は様子を見てから」。
ここで固定するのは次の4つ:

- **入稿されない**こと（`sell_plans_for` がどの型・車数・種別・引数でも返さず、
  入稿スクリプトの `SELLABLE_PLAN_KEYS` にも無い）
...
🔴 売ると決めたら `sell_plans_for` に分岐を足し、`test_入稿しない` を書き換える
   （ユーザー決定なしに売り始めないための固定）。
"""
...
def test_入稿しない(label, n, race_type):
