...
def test_compression_direction_is_never_inverted():
    """決勝・上位グレードでは a<=1（分散を縮める）＝過大評価を潰す向き。

    🔴 **守るのは向き（a<=1）であって大きさではない。**
       2026-08-20 まで `< 0.95` という**大きさ**を要求していたが、これは
       旧モデル（60特徴・改名前）の挙動を焼き込んだ閾値だった。

       ライン特徴の追加(60→66)と定義是正の後、**決勝の過大評価は実測で
       +2.84pt → +0.22pt** まで縮んだ（2025年・12,199行・バックフィル済み指数）。
       較正が解こうとしていた問題がモデル側で解消されたため、決勝の a は
       0.8740 → 0.9620 となり `< 0.95` を満たさなくなった。

       大きさで縛ると「モデルが良くなるとテストが落ちる」ことになるので向きだけ守る。
       ⚠️ 逆に **a>1（分散を広げる）へ反転したら異常**。それは過大評価を
          さらに増幅する向きで、較正の目的と逆。ここは落とすこと。
    """
    assert coefficients("決勝", 1)[0] <= 1.0, "決勝の補正が過大評価を増幅する向き"
    assert coefficients("予選", 5)[0] <= 1.0, "上位グレードの補正が増幅する向き"
    # 上位グレードは依然として強く縮める必要がある（実測でここだけ乖離が残る）
    assert coefficients("予選", 5)[0] < 0.90, "上位グレードの圧縮が失われている"
    # 平場は触らない
