...
def _write_cand(picks, legs_tf, race_key="20260807_13_02"):
    (picks / f"wave_picks_wt_{DATE}_s7h1_candidates.json").write_text(
        json.dumps([{
            "race_key": race_key, "legs": legs_tf, "legs_tf": legs_tf,
            "bet_amount": 9600, "stake_tf": 1200,
...
def test_7h1_の暫定行が朝に書かれる(env):
    ...
    assert n_combos == len(TF)
    # pred_combo は notify_prerace_wt._insert_rank_7h1_pick と同一形式でなければ
    # 採点・Web 表示が食い違う（2026-08-15 の三連単一本化で "三複:… / " が消えた）
    assert pred == "三単:" + ",".join(TF)
