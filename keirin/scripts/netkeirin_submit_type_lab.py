...
def _passes_axis_gate(row: dict) -> bool:
    ...
    🔴🔴 **2026-08-28 の「看板は素通し」を取り消した。** 素通しされていた層が
       はっきり弱く、そこを外すと的中体験が両窓で改善するため:

           看板でゲートを通る分   10.7件/日  表示的中 23.41%  ROI 88.8%
           看板でゲートに落ちる分  3.8件/日  表示的中 16.92%  ROI 68.2%   ← 外す

       全体 43.2 → 39.4件/日・表示的中 25.96 → 26.83%・ROI 82.4 → 83.8%（確認2026）。
       探索2025 も同じ向き（55.5→50.6件/日・25.11→25.88%・76.5→76.7%）。
       **同数を無作為に落とす対照20本に、表示的中で両窓とも 20/20 で勝つ**
       （ROI は確認 19/20 ↔ 探索 14/20 なので、確かなのは表示的中の方）。

    🔴 **代償は「看板レースに商品が出ない日がある」こと**（3.8件/日）。
       2026-08-09 の「看板レースには必ず推奨を出す」方針を、
       2026-08-31 にユーザーが明示的に取り消した上での変更。
       戻すなら `_is_marquee(row)` の素通しをここへ復活させるだけでよい。

    ⚠️ **ゲートの検証では必ずこの関数の挙動を再現すること。** 看板は母集団の
...
def run(day: str, session: str, dry_run: bool, only_key: str | None,
    ...
    def _reject(r: dict):
        ...
        #
        # 🔴🔴 **この行の `rank_key`（＝型）を型として読んではいけない**（2026-09-02 追記）。
        #    欠測時の型判定は退化する。`arare` の加算5項のうち4項（ライン構成・
        #    ラインの維持・先頭の脚質・番手の得点）が並び依存で、並びが無いと
        #    全員同ラインとして同値になり、`arare` は実質「開催日目 − 2」だけの
        #    関数になる。印が全車ゼロだと `axis_sum` も下がって必ず混戦側へ落ちる。
        #    結果、**同じ回の欠測レースが全部同じ型になる**:
        #        2026-08-29 morning（日目3）18件すべて F
        #        2026-08-31 morning（日目2）19件中18件が E
        #        2026-09-01 morning（日目3）16件すべて F
        #    後の波で並びが取れると型はばらける（9/1 は 16件中 12件が別の型）。
        #    `submission_skips` に残るのは**欠測入力から出た偽の型**なので、
        #    見送りを型別に集計すると「特定の型が取りこぼされている」と読み違える
        #    （2026-09-02 に実際に読み違えた）。集計は `reason_code` だけで行う。
