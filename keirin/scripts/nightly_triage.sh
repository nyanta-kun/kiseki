...
6. **累積はレポート冒頭の「前向き確認の起点」（2026-08-29・型ラボ全面移行日）から数え、
   「累積から除外した日」（2026-09-15・段の商品 T_firm/T_mid/T_axis/T_upset を売った日。
   9/16 に販売停止し 9/14 までの規則へ戻した）は除いている。** 除外日の成績を
   累積や他の日と並べて論じない。対象日そのものが除外日のレポートでも、
   §1 の異常は通常どおり扱うが、§2〜§6 からは何も提案しない。
7. **「参照分布なし」の商品（段の商品 T_* など）について、参照との比較を書かない。**
   旧プランの分布を当てはめて高い・低いと言わない。
8. 起点より前の成績や、他の起点の台帳（`type_lab_nightly_ledger_20260915.csv` 等）の
   件数を持ち出して比べない。
