...
6. **累積はレポート冒頭の「前向き確認の起点」から数え直している**（2026-09-15 に
   7車の売り物を段の商品 T_firm/T_mid/T_axis/T_upset へ差し替えてリセット）。
   起点より前の成績・旧台帳の件数を持ち出して比べない。起点直後は件数が
   少ないのが正常で、「件数が足りない」こと自体を課題にしない。
7. **「参照分布なし」の商品（段の商品 T_* など）について、参照との比較を書かない。**
   旧プランの分布を当てはめて高い・低いと言わない。
8. §4（軸信頼ゲート）は起点以降、売り物を動かしていない参考値。ここから施策を出さない。
