#!/usr/bin/env python3
"""7T3（三連単・決勝の中配当枠・RANK_7T3）の過去分バックフィル。

**候補生成そのものは `build_7t3_candidates.build()` を呼ぶ**。本スクリプトは
その候補に対して、live と同じ盤面フィルタ・採点・記録だけを行う。

## 🔴 2026-01-01 以降しか honest に作れない（7T1 と同じ制約）

7T3 は三連単オッズ予測モデル（`data/models/odds_tf_n7.txt`）で帯（30倍以上）を
切る。このモデルの学習終端は **2025-12-31**（`odds_tf_meta.json` の `train_end`）で、
**月次 vintage が無い**。したがって 2025 年以前へ遡って適用すると
model-vintage look-ahead になる。`ODDS_TF_TRAIN_END` で機械的に弾く。

## 採点式

  - 買い目は順序つき三連単の **5点固定**（帯の中から位置別合成 PL の確率上位）
  - 賭け金は**均等**（2,000円/点）。的中した点の賭け金で按分する
...
## 欠車の扱い
  ...
  - 残りが0点になったら skip
  - 残った点数で `rank_7t3_stakes()` を**引き直す**（1レース1万円の枠は動かさない）
    ⚠️ 均等配分なので、点が落ちると1点あたりの賭け金は**増える**。
...
使い方:
    PYTHONPATH=. .venv/bin/python scripts/backfill_7t3_rank_wt.py \\
...
from scripts.build_7t3_candidates import build as build_candidates  # noqa: E402
from src.database import get_connection  # noqa: E402
from src.evaluation.backtest_wt import _load_payouts_wt  # noqa: E402
from src.strategy_wt import (  # noqa: E402
    RANK_7T3_LEGS, RANK_7T3_MIN_ODDS, RANK_7T3_NE, rank_7t3_stakes,
)
...
def assert_odds_model_is_honest(date_from: str) -> None:
    ...
    🔴 **月次 vintage が無いので「古いモデルを使う」回避策が取れない。**
       静かに通すと 7T3 だけ in-sample な数字になり、他ランクと並べたときに
       強く見える（このリポジトリが繰り返し踏んでいる型）。
    """
    if date_from <= ODDS_TF_TRAIN_END:
        raise SystemExit(
            f"[backfill_7t3] {date_from} は三連単オッズモデルの学習終端"
            f"（{ODDS_TF_TRAIN_END}）以前です。7T3 の honest 再構築は "
...
def build_rows(date_from: str, date_to: str, *, eval_model: str,
    ...
    """バックフィル対象の 7T3（#7T3）行（採点済み）を構築する。"""
    ...
    for c in cands:
        ...
        if not legs:
            ...
            # 🔴 **1点でも成立する**。欠車で帯の目が消えても、残った点は
            #    そのまま買う（点数の下限は置かない）。

        # 🔴 賭け金は**残った点数で均等に引き直す**。候補JSONの stakes をそのまま
        #    使うと落ちた目のぶん予算を下回る。
        #    ⚠️ **均等が仕様**なので確率で重み付けしてはいけない
        #       （`rank_7t3_stakes` が正本）。30倍という足切りは
        #       「全点が等額」を前提に決めてある。
...
def insert_rows(rows: list[dict], dry_run: bool) -> None:
    ...
    with get_connection() as conn:
        conn.executemany(
            ...
            rows_ins)
        conn.commit()
    print(f"[backfill-7t3] get_connection先 {len(rows)}件 書き込み完了")

    db_url = os.environ.get("KEIRIN_DB_URL")
    if not db_url:
        print("[backfill-7t3] KEIRIN_DB_URL 未設定 → VPS PG ミラーはスキップ")
        return
    import psycopg2
    ...
    with psycopg2.connect(db_url) as pg:
        with pg.cursor() as cur:
            ...
            """, rows, page_size=200)
    print(f"[backfill-7t3] VPS PG {len(rows)}件 書き込み完了")
...
def main() -> None:
    ...
    # 🔴 **7T1 のような「目標額」を持たない枠**なので、そこは数えない。
    #    見るのは設計書 §11 の3つ（件数・的中率・払戻中央）。
    #    ⚠️ ROI は出すが**採否の基準にしない**（的中率が低く払戻の分散が大きい）。
    pays = sorted(r["payout"] for r in rows if r["hit"])
    med = pays[len(pays) // 2] if pays else 0
    print(f"[backfill-7t3] {args.start}〜{args.end}: {len(rows)}R "
          f"({RANK_7T3_NE}車・{RANK_7T3_MIN_ODDS:.0f}倍以上×{RANK_7T3_LEGS}点) "
          f"的中{n_hit} ({n_hit / len(rows) * 100 if rows else 0:.1f}%) "
          f"投資{bet:,} → 回収{pay:,} "
          f"ROI {pay / bet * 100 if bet else 0:.1f}% / 払戻中央 {med:,}円")
    if args.wipe:
