...
from src.type_lab import (                          # noqa: E402
    BUDGET, UNIT, rule_version, sell_plans_for)
...
#: 🔴 **現行 rule_version の行だけを読む**（2026-09-02）。8/31 の測定は帯20倍の
#:    paper の上で行われており、#395 で帯を15倍へ下げた後は母集団が違う。
#:    同じテーブルに新旧が混ざるので version で必ず絞る。
RULE_VERSION = rule_version(7)
...
def main() -> int:
    with get_connection() as c:
        rows = [dict(r) for r in c.execute(
            ...
            "WHERE t.mode = ? AND t.rule_version = ? "
