...
# 予測オッズ（三連複・三連単）モデル。**存在の有無だけ**を見る（2026-08-26 追加）。
#
...
#
# 🔴 **三連単も監視する**（2026-08-29 追加）。型ラボへ移行してから、売っている
#    6プランのうち **5つが三連単**（A_hit/B_hit/C_hit/E_hit/F_pay。三連複は
#    D_hit だけ）。三連単の予測オッズが作れないと買い目そのものが組めないので、
#    配布漏れは商品の大半が消えることを意味する。**実際に起きている**——
#    `odds_tf_n7.txt` は 2026-08-13 に手で置かれたきり同期されず、
#    2026-08-28（PR#349）まで VPS の配布物が古いままだった。
#    `odds_tf_meta.json` には保守倍率（`conservative`）も入っており、欠けると
#    三連単の `odds_low`（下振れ表示）が黙って出なくなる。
