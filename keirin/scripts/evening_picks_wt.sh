...
# 2c. 🔴 **7T1 / 7T3 の候補も作り直す**（2026-08-26 追加）。
#     この2つだけ「朝1回で当日全開催ぶん」を作る設計で、夕方の再生成に乗って
#     いなかった。その結果、朝に入力（並び予想・AI印）が未取得だった開催では
#     **壊れた入力で選んだ買い目が、入力が届いた夕方にそのまま入稿される**。
#     実害（2026-08-26 熊本7R・7T1）: 朝は印ゼロ・ライン未取得で 1-2-4 の1点
#     （当時の予測 17.5倍）が選ばれ、morning/noon は入力欠落ガードで見送り、
#     18:00 に印が揃ってガードを通過した。ところが買い目は朝のままで、
#     現在の入力で引き直すと同じ目は 11.2倍＝想定払戻 111,654円。
#     7T1 の目標（15万円）に届かない商品が出ていた。作り直すと軸ごと変わり
#     1-7 の4点（61〜188倍）になった。
#     ⚠️ 他ランクは `wave-picks-wt` が `_night` を作るのでこの穴が無い。
#     ⚠️ 出力名は `_night` を挟むこと。`netkeirin_submit_wt._load_candidates` は
#        evening の波でこの名前を先に探し、無ければ朝の生成物へ落ちる。
#     ⚠️ 生成に失敗しても継続する。朝の候補が残るので商品ゼロにはならない。
