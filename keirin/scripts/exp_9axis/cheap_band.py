"""安い配当の帯で「点数を絞る／ワイド1点」が買いになるかを測る（9車・全網羅）。
...
from src.strategy_wt import (RANK_9C_LEG_P3_MIN, RANK_9C_LEGS_MIN,
                             RANK_9C_P3_SUM_MIN, rank_7c_select_legs)
...
for rk, g in P.groupby("race_key"):
    ...
    if not m or not es or not tb or not wb or m["n_entries"] != 9:
        continue
    ...
    c_legs = rank_7c_select_legs([f for f in order[2:]], p3, p3_min=RANK_9C_LEG_P3_MIN)
    passes = gate >= RANK_9C_P3_SUM_MIN and len(c_legs) >= RANK_9C_LEGS_MIN
    if passes:
...
D = pd.DataFrame(rows); D.to_pickle("/tmp/cheap9.pkl")
D["win_"] = D.date < "2026-01-01"
print(f"9車 {len(D):,}R（探索 {int(D.win_.sum()):,} / 確認 {int((~D.win_).sum()):,}）")
...
D["gb"] = pd.cut(D.gate, [0, 1.15, 1.25, 1.30, 1.40, 1.55, 3.0])
