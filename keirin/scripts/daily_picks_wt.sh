...
# 🔴 **既知の縮小（2026-08-19・未対応）**: 母集団は「この時点で
#    netkeirin_submissions にある行」。ランクのナイター/ミッドナイト分は
#    元から13:00/18:00 なので居らず、さらに同日、穴埋めも自分の波でしか
#    走らなくなった（`submit_marquee_wt.py`）。結果 **自信ありは朝の開催からしか
#    選ばれない**。実績5日のうち3日は「朝の穴埋めが入れたナイター開催
#    （松山・GI オールスター）」が選ばれており、それはもう起きない。
#    ⚠️ docstring の「朝の時点で当日全レースを見て」という設計意図とは食い違う
#       （予測オッズを使うのは夜開催を同じ土俵に載せるためだった）。
#    → 直すなら「各波の後にも未公開行だけ選び直す」か「朝に当日全候補から選び、
#      後の波の入稿時に付ける」。ユーザー判断で今回は据え置き。
