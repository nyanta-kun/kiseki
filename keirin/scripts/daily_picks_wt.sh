...
# 🟢 **母集団の縮小は 2026-08-21 に解消した。** 母集団が「この時点で
#    netkeirin_submissions にある行」であることは変わらないが、ランク入稿・
#    穴埋めが**前倒しできる夜の開催をこの回で出す**ようになったため、当日ほぼ
#    全開催がここに揃う（docstring の「朝の時点で当日全レースを見て」に追いついた）。
#    ⚠️ 残るのは前倒しできなかった開催（三連単ランク・予測オッズ不可）だけ。
#      それらは 13:00/18:00 に入稿されるが、自信ありは朝に決め切る（1日1件の
#      担保が2回選定で壊れるため）。前倒し率が下がった日は母集団も痩せる。
