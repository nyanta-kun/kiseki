...
#
#     🔴 2026-08-21: この朝の回は **当日全開催**を対象にする。ナイター・
#        ミッドナイトも「予測オッズが買う点すべてに作れる三連複」なら**前倒しで
#        入稿する**（`netkeirin_submit_wt._can_pull_forward`）。
#        2026-08-07〜08-20 は板が育つのを待って昼13:00・夕18:00 に回していたが、
#        その理由は失効した——賭け金の配分は `landing_weights` が**予測オッズを
#        最優先で単独採用**するようになり、実測でも 08-12 以降の夜の波の入稿は
#        noon 34/34・evening 67/67 が `predicted` で**板由来は1件も無い**。
#     🔴 前倒しできないものだけが波に残る: **三連単ランク**（ダッチ配分に実際の
#        板オッズが要る）と**予測オッズを作れないレース**（7車・9車以外＝3.7%）。
...
# 🔴 **2026-08-30: 従来ランクの入稿と看板穴埋めをここから外した**（ユーザー決定
#    「型ラボの入稿を7時の実行にし、従来の入稿は無効とする」）。
#
#    2026-08-28 に旧ランク（7S/7C/7B/7M1/9C/7T1…）を全て `enabled=false` にした
#    ので、`netkeirin_submit_wt.py morning` は毎朝「成功0件」を出すだけになり、
#    看板穴埋めは**存在しない 7S を呼び続けて全滅**していた
#    （実測 2026-08-30: 「埋まらなかった21件」・8/29 は15件）。
#
#    🔴 **穴埋めの役割は型ラボ側へ移した。** 9車の型F（決勝以外）を売るように
#       したので（`sell_plans_for`・PR #379）、看板の無商品は 10件 → 2件
#       （残りは8車＝型ラボが7車/9車しか扱わないため構造的）。
#
# 🔴 **型ラボは「当日データ収集」の後でなければ動かない**（自前でモデル推論する
#    ので `wave-picks-wt` には依存しないが、`wt_entries` は要る）。だから
#    別 cron の時刻合わせではなく**この波の中で順に呼ぶ**。
#    2026-08-12 に看板穴埋めを cron から波の中へ移したのと同じ理由で、
#    「20分あれば終わっているはず」という暗黙の仮定を持たない。
# ⚠️ 取りこぼしは `type_lab_wave.sh`（13:05 / 18:05）が拾う。
echo "[$(date '+%H:%M:%S')] 型ラボ（生成 → 入稿 → 自信あり → 採点）..."
bash scripts/type_lab_daily.sh \
