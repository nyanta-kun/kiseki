"""ライン先頭同士の比較・ライン内結束の特徴量 A/B（2026-08-19・ユーザー提起）。

## 仮説の出所

ユーザー提起:「逃げの競走得点が低いと別ラインから離されてしまうことがありそう」
「単騎・複数ライン・ラインの強さ（得点合計、得点差が少ないか＝ちぎれ）を
  適切に考慮できているか」。
...
実測（先頭が2人以上いる7車レース 33,293R・2025-01〜2026-08）:
    ...
    **−45.8pt・単調。** ライン員も道連れ（第1位の先頭のライン員 50.1% →
    第2位のライン員 36.8%）。

既存 `FEATURE_COLS_WT` の欠落:
  - `line_rp_max` は「ライン内の最大得点」で**先頭とは限らない**
    （逃げが弱く番手が強いラインでは番手を拾う）
  - `line_rp_sum` は合計なので「90+90」と「110+70」を区別できない
  - `is_line_leader` × `race_point` の**掛け合わせが無い**。木は交互作用までは
    学べても、**レース内で他の先頭と比べた相対順位**は作れない

## ⚠️ 条件付き成績と純増分は別物

上の 45.8pt は条件付き成績で、モデルが `race_point` 経由で間接的に
捉えている可能性がある。**このA/Bが測るのは純増分**。

検証方法は `exp_racetype_field_ab.py` と同一（2窓 × 5seed・deterministic=True・
AUC / 指数1位の勝率 / 3着内率）。DB書き込みなし。

使い方:
    PYTHONPATH=. .venv/bin/python scripts/exp_line_leader_ab.py [--arms all|leader|cohesion]
"""
...
def run_window(df: pd.DataFrame, test_from: str, test_to: str, arms: list) -> None:
    ...
    print("== 新特徴の分布（test） ==")
    print(test[LEADER_COLS + COHESION_COLS].describe()
    ...
    for arm, cols in arms:
        ...
        new = [c for c in cols if c in LEADER_COLS + COHESION_COLS]
...
def main() -> None:
    ...
    ap.add_argument("--arms", default="all", help="all | leader | cohesion")
    args = ap.parse_args()

    # 🔴 `build_features_wt` が race_type / field / line 系を**既に**付与する
    #    （導入時に本採用されたため）。旧A/Bハーネスの `add_*` を重ねて呼ぶと
    #    生列（race_type 等）を要求して落ちる。新特徴も
    #    `add_line_strength_features_wt` の末尾から付与済み。
    max_to = max(t for _, t in WINDOWS.values())
    df = build_features_wt(load_raw_data_wt(min_date=TRAIN_FROM, max_date=max_to))
    missing = [c for c in LEADER_COLS + COHESION_COLS if c not in df.columns]
    if missing:
        raise SystemExit(f"新特徴が付与されていません: {missing}")

    base = list(FEATURE_COLS_WT)
    arms = [("baseline", base)]
    if args.arms in ("all", "leader"):
        arms.append(("+先頭比較", base + LEADER_COLS))
    if args.arms in ("all", "cohesion"):
        arms.append(("+ライン内結束", base + COHESION_COLS))
    if args.arms == "all":
        arms.append(("+両方", base + LEADER_COLS + COHESION_COLS))
