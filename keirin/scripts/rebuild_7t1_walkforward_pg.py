#!/usr/bin/env python3
"""7T1(RANK_7T1) の honest 再構築（月次凍結vintageモデル使用）。

`rebuild_7c_walkforward_pg.py` の 7T1 版。7T1 は軸選定と点数決定に
...
⚠️ したがって 7T1 の再構築結果を他ランク（2024-01 から）と**期間を揃えずに
...
使い方:
    PYTHONPATH=. .venv/bin/python scripts/rebuild_7t1_walkforward_pg.py [--dry-run]
    PYTHONPATH=. .venv/bin/python scripts/rebuild_7t1_walkforward_pg.py --tail-only
"""
...
from scripts.backfill_7t1_rank_wt import ODDS_TF_TRAIN_END, build_rows
...
_RANK_LABEL = "RANK_7T1"
_DELETE_COND = "rank='RANK_7T1' AND race_key LIKE '%#7T1' AND race_date BETWEEN ? AND ?"
_SCRIPT_NAME = "rebuild_7t1_walkforward_pg.py"
...
def main() -> None:
    ...
    if not windows:
        print("[rebuild-7t1-pg] 対象の窓がありません（全窓がオッズモデルの学習期間内）。")
        sys.exit(1)

    # 7T1 は eval と win の両方を使う（bad は使わない）。
    ...
    if missing:
        ...
        if not args.skip_missing_models:
            ...
            print("[rebuild-7t1-pg] --skip-missing-models 未指定のため処理を中断します"
                  "（計算は一切行っていません）。")
            sys.exit(1)
        notify_discord_warning(
    ...
    windows = available
    if not windows:
        print("[rebuild-7t1-pg] 処理対象の窓がありません（全窓でモデル不足）。終了します。")
        sys.exit(1)

    per_window_rows: list[tuple[str, str, list[dict]]] = []
    all_rows: list[dict] = []
    for date_from, date_to, eval_model, win_model in windows:
        print(f"\n[rebuild-7t1-pg] {date_from}〜{date_to}  eval={eval_model} win={win_model}",
              flush=True)
        ...
        print(f"[rebuild-7t1-pg]   7T1: {len(rows)}R ({len(rows)/n_days:.1f}R/日) 的中{n_hit} "
    ...
    print("\n[rebuild-7t1-pg] ===== 全期間合計（2026-01 以降のみ）=====")
    print(f"[rebuild-7t1-pg] 7T1: {len(all_rows)}R 的中{total_hit} "
    ...
    # 7T1 は3ヘッド軸を使わない（軸は目的関数の argmax）。フラグを立ててはいけない。
