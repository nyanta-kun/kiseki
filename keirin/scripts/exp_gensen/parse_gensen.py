...
ROOT = Path(__file__).resolve().parent / "gensen"
DET = ROOT / "detail"
...
def parse(p: Path) -> dict | None:
    ...
    # ---- 厳選AI評価（本予想家固有） ----
    m = re.search(r'<h2 class="YosoKenkaiTitle">厳選：(\w+)</h2>', t)
    out["tier"] = m.group(1) if m else None
    out["confident"] = "自信あり" in t and bool(re.search(r"自信あり", t))
    ev = {}
    kt2 = re.search(r'厳選AI評価.*?<tbody>(.*?)</tbody>', t, re.S)
    if kt2:
        for tr in re.findall(r"<tr>(.*?)</tr>", kt2.group(1), re.S):
            tds = re.findall(r"<td[^>]*>(.*?)</td>", tr, re.S)
            if len(tds) >= 4 and tds[0].strip().isdigit():
                ev[int(tds[0].strip())] = {"grade": tds[2].strip(),
                                           "point": float(tds[3].strip()) if re.match(r"^[\d.]+$", tds[3].strip()) else None}
    out["ai_eval"] = ev
    return out
