"""9F(RANK_9F) の walk-forward 全期間再構築（VPS PG 直書き・2026-08-25 新設）。

`rebuild_9c_walkforward_pg.py` の穴埋め版。母集団は **9車 ∧ 9C のゲートを通らない**
レースで、`#9C` とは排他。union が「9車を開催まるごと網羅したときの姿」になる。
...
🔴 **`CURRENT_PAPER_RANKS` には登録していない**（kiseki Web の集計に混ぜないため）。
   分析は picks_history を rank='RANK_9F' で直接引く。

⚠️ 9F は軸も相手も pred_prob(3着内率) とライン情報だけで決まるので win/bad モデルは
   使わない（`build_rows` は signature を揃えるためだけに受け取り、無視する）。

使い方:
    PYTHONPATH=. .venv/bin/python scripts/rebuild_9f_walkforward_pg.py [--dry-run]
    PYTHONPATH=. .venv/bin/python scripts/rebuild_9f_walkforward_pg.py --tail-only
"""
...
from scripts.backfill_9f_rank_wt import build_rows
...
_RANK_LABEL = "RANK_9F"
_DELETE_COND = "rank='RANK_9F' AND race_key LIKE '%#9F' AND race_date BETWEEN ? AND ?"
_SCRIPT_NAME = "rebuild_9f_walkforward_pg.py"
...
def main() -> None:
    ...
    if missing:
        ...
        if not args.skip_missing_models:
            ...
            print(f"[rebuild-9f-pg] --skip-missing-models 未指定のため処理を中断します"
                  f"（計算は一切行っていません）。")
            sys.exit(1)
        notify_discord_warning(
        ...
        print(f"[rebuild-9f-pg] --skip-missing-models 指定のため{len(missing)}窓を"
              f"除外して続行します。")

    windows = available
    if not windows:
        print("[rebuild-9f-pg] 処理対象の窓がありません（全窓でモデル不足）。終了します。")
        sys.exit(1)

    per_window_rows: list[tuple[str, str, list[dict]]] = []
    all_rows: list[dict] = []
    for date_from, date_to, eval_model, win_model in windows:
        print(f"\n[rebuild-9f-pg] {date_from}〜{date_to}  eval={eval_model} win={win_model}", flush=True)
        ...
        print(f"[rebuild-9f-pg]   9F: {len(rows)}R ({len(rows)/n_days:.1f}R/日) 的中{n_hit} "
    ...
    print(f"\n[rebuild-9f-pg] ===== 全期間合計 =====")
    print(f"[rebuild-9f-pg] 9F: {len(all_rows)}R 的中{total_hit} "
