...
## 現行ランク体系（2026-08-22〜・**7S/7B/7C/7M1/7H1/7H2/7T1/7T2/9C/9H1 の10ペーパーランク**）

> **【2026-08-22・三連単の一撃枠 RANK_7T2 を新設（ペーパー並走中・入稿しない）】**
> ユーザー方針「1レース1万円固定・1日20件程度・1日1〜2件で15〜30万の払戻」への対応。
>
> **7T1 と買い目の形・選定関数は完全に同一**（`build_7t1_candidates.build` を
> パラメータ違いで呼ぶ）。違うのは3点だけ:
>
> | | 7T1（入稿中） | 7T2 |
> |---|---|---|
> | 母集団 | 決勝系×別ライン（13.1件/日） | **全7車**（64件/日） |
> | 日次上限 | 5 | **20** |
> | 目標払戻 | 15万円 | **20万円** |
>
> 🔴 **目標払戻 = 日次上限 × 1レース予算 は設計原理。** 1レース1万円・1日N件なら
> 1件の的中で日次回収率が100%を超える条件は「払戻 >= N万円」。**片方だけ動かさない。**
>
> 実測（honest walk-forward・出荷実装忠実・2026-01〜08・14,922R/234日・
> 現行全ランク構成との同一日ペアード bootstrap。一撃20件＋三連複10件の構成）:
>
> | | 現行(43.7件/日) | 7T2構成(30件/日) |
> |---|---|---|
> | 投資/日 | 43.5万円 | 30.0万円 |
...
> 効果は「100%を超える日の頻度」と「大きい払戻の頻度」の2つだけ。
>
> ❌ **検証して採らなかった案（再提案しない）**:
> - **軸2を2着・3着の両方に置く**（ユーザー提案）。軸を固定した簡易実装では
>   8/8セルで現行形を上回ったが、**出荷実装（軸1=3着内率上位2車 × 42順序対の
>   総当たり）では 100%超の日が逆転する**（36.8%→30.8%）。総当たりは軸2が3着に
>   来そうなレースで**別の順序対を選び直す**ことで既に拾っていた
> - 予測オッズの下振れマージン（m=1.33 等）。出荷実装では **m=1.0 が最良**
> - 日次選別の順序の工夫。**無作為が ev 降順とほぼ同じ**で、的中確率順・
>   オッズ順・軸1の3着内率順は無作為より悪い。**選別器を作らないこと**
>
> ⚠️ **入稿しない**（`RANK_CONFIGS` 未登録・`PaperRankSpec.paper_only=True`）。
> 入稿を始めるときは旗を下ろすのと `RANK_CONFIGS` 登録を**同時に**行うこと
> （片方だけだと `tests/test_rank_7t2.py` と `test_netkeirin_rank_coverage.py` が落ちる）。
> ⚠️ **7T1 の「決勝系レースに推奨を出す」ブランドは無い。** 文面を流用しないこと。
> 検証と設計の全文: `docs/strategy_rebuild_2026_08.md`。
> 検査: `tests/test_rank_7t2.py`
...
> （優先順位 **7H2 > 7S > 7B > 7C > 7T1 > 7H1 > 7M1**・`RANK_CONFIGS` の定義順が正本）。
