...
> （優先順位 **7H1 > 7H2 > 7SS > 7S > 7A > 7C > 7H3 > 7B**・`RANK_CONFIGS` の定義順が正本）。


> **【2026-08-12・RANK_7H3 新設（穴推奨・本命連対どまり型／三連単の高配当）】**
> 三連単で1レース10万円超の払戻を作る枠。設計と実測は `docs/rank_7h3_design.md`。
>
> ```
> 対象   7車 ∧ race_type が看板キーワード（決勝/特選/選抜/特秀）を1つも含まない
>        ＝「看板でも準決勝でもない」 ∧ 軸積(p3上位2車の積) >= 0.70
> 買い目 三連単フォーメーション  1着=相手 / 2着=軸2車 / 3着=軸2車（相手数×2点）
>        ＝「上位2車は3着以内に残るが、勝つのは別の車」だけを買う
> 配分   Plackett-Luce（1着率由来）→ allocate_budget。1レース10,000円
> ```
>
> 実測（honest walk-forward・7車13,795R・216日）: **5.5件/日・edge 1.59
> （探索1.67 / 確認1.51）・推定ROI 1.19 [0.95, 1.44]・10万円超が3.5日に1回**。
> 🔴 **ROI > 1.0 とは言えない**（CIが1.0を含む）。表示的中は約5%で**的中率商品ではない**。
> 🔴 **看板レースを三連単に置き換えてはいけない。** 看板には市場の歪みが無く
> （edge 1.02）、替えると表示的中が 29.0%→5〜14% に崩れるだけで ROI も改善しない。
> ⚠️ 母集団は「看板外の全7車」なので **7C とだけ重複する**（`overlap_expected`）。
