...
本ファイルで現物と合わないと確認された主な箇所:

- 「現行ランク体系」節（約550行）— **旧ランクは全て `enabled=false`。いま売っているのは型ラボ
  （`src/type_lab.py`）だけ**。同節は旧ランクの沿革・否定結果の記録として読むこと
- 特徴量は **70列**（本文の 60 は古い）／日次 cron は **07:00**（本文の 8:00 は古い）
- 「3着内率の後段較正」— 型ラボは**生の p3** を使う。較正の消費者は Web 表示と停止中の旧ランクだけ
- 型ラボの売り物の p3 は `lgbm_wt_eval`（`full_refit:false`）。pw は `lgbm_wt_win`（full-refit）
- DB に残る過去の予測値（full-refit モデル由来）から作った ROI・的中率は in-sample
...
- 🔴 **同着では3着以内の当たり目が複数になる。判定の正本は `src/result_top3.py`**（2026-08-22）。
  三連複は3着同着で2通り、三連単は1着/2着同着でも着順の入れ替えぶん増える。
  - それまでは全経路が `ORDER BY finish_order` の**先頭3件だけ**を正解にしており、
    もう一方を買っていた的中が `hit=0` で記録されていた（**例外もログも出ない**）。
    実データで **10件**（三連複8・三連単2・2024-01〜2026-08）
  - さらに `ORDER BY finish_order` に**タイブレークが無い**ため、
    再構築のたびに正解が入れ替わりうる＝台帳の再現性が壊れる。
    `TOP3_SQL`（`ORDER BY finish_order, frame_no`）を使うこと
  - ⚠️ **払戻は当たり目ごとに違う。** 的中したら必ず `hit_trio()` / `hit_trifecta()` が返す
    「**自分が買った当たり目**」で payout を引く。外れたときの記録用は `representative()`
  - 頻度: 3着同着 237レース(0.23%) / 1着・2着同着 492レース(0.48%)
  - 検査: `tests/test_result_top3.py`（採点経路が旧実装へ戻っていないことも固定）
...
### モデル（候補）の確認は Web に残してある（2026-08-25 追加）

入稿・Discord は売った商品だけに揃えたが、**ゲートの是非を後から追えないと困る**
（見送ったレースが当たっていたか分からない）ので、Web だけ両方見られる。

| 場所 | 出るもの |
|---|---|
| `/keirin` 一覧 | 売っていない行に `paper_hit` / `paper_payout` を「モデル」チップで出す |
| `/keirin/stats` | 「売った商品 / モデル」の切り替え（`?source=paper`） |

🔴 **売った成績と同じキー・同じ系列に混ぜないこと。** `hit` / `payout` /
   `bet_amount` は売った商品、`paper_*` は候補。合算もしない
   （候補の賭け金は「1万円賭けたことにしたら」の名目値で収支ではない）。

🔴 **`source=paper` の ROI を「モデルの実力」として読まないこと。** 実測
   （2026-08-01〜25）で sold 0.682 に対し paper 0.902 と 22pt も開くが、
   これは実力差ではなく**母集団と金額の定義差**:
   ① 候補は名目均等・入稿は傾斜配分 ② 看板の穴埋め（実測 ROI 0.333）は
   候補側に存在しない ③ 当月分は毎朝 walk-forward で組み直され live の判断とずれる。
   採否の判断は必ず walk-forward スクリプトで行う。

⚠️ したがって Web の候補系列で見てよいのは**日々の傾向**まで
（live と rebuild は母集団が構造的にずれる。下記「live と rebuild で
母集団がずれるのは仕様」）。
...
> 🔴 **2026-09-20 監査**: この節のランクは**全て無効化済み**（`netkeirin_settings.enabled=false`・候補生成も 2026-09-11 停止）。
> 現行の商品は型ラボ。現物の仕様は `docs/AUDIT_2026_09_20.md` §2 と `docs/RECOMMENDATION.md`。以下は沿革として残す。
...
> （優先順位 **7H2 > 7T1 > 7T3 > 7S > 7B > 7C > 7H1 > 7M1**・`RANK_CONFIGS` の定義順が正本）。
...
#### 🔴 値そのものが無効でも同じことが起きる（2026-08-29・型ラボ移行で再発）

`netkeirin_submit_type_lab.py` が存在しない `channel="keirin"` を渡していて、
**入稿は成功しているのに毎回 `ValueError` で落ちていた**。
`type_lab_daily.sh` は `if ! …` で受けているので入稿バッチは死なないが、

- Discord には型ラボの入稿通知が **1通も出ていない**（8/28 の移行から2日間）
- ログには「⚠️ 入稿に失敗」と残る（**実際は24件入稿できている**）

有効なキーは `src/notify/discord.py::_WEBHOOK_ENV_KEYS` の5つだけ。
`tests/test_discord_channels.py` が **`src/` と `scripts/` の
`channel="…"` リテラルを AST で拾って実在を検査する**（実行経路が日に数回しか
無いので、静的に縛らないと次も本番で気づくことになる）。
通知そのものも try/except で包み、**入稿の後始末で入稿を失敗扱いにしない**。
