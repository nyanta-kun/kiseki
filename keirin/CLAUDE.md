...
> （優先順位 **7H1 > 7H2 > 7SS > 7S > 7A > 7C > 7T1 > 7B**・`RANK_CONFIGS` の定義順が正本）。
...
>
> ```
> 対象   7車 ∧ **上位クラス戦**（決勝/準決勝/特選/選抜）∧ p3上位2車が**別ライン**
...
> ```
>
> 実測（honest walk-forward・7車13,749R・216日／上位クラス戦×別ライン）:
...
> ⚠️ 母集団が上位クラス戦なので **7C と同じレースを取り合う**（`overlap_expected`）。
