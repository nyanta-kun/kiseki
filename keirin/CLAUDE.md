...
> ⚠️ 当初の目標は20万で、**20万到達を落とさずに的中頻度を上げられた**ため15万へ
> 下げてある。本節の「20万超」表記は導入時の検証値なので読み替えないこと）。
> 旧 7H3 の枠（三連単フォーメーション・1点1行の入稿経路・Web登録）を
> そのまま引き継ぎ、**中身を全面的に置換**した。
>
> ```
> 対象   7車 ∧ **決勝系レース**（決勝/準決勝/特選/選抜）∧ p3上位2車が**別ライン**
>        ∧ **看板レースでない**（2026-08-17 追加。実質「準決勝」だけが残る）
...
> 🔴 **母集団の定義（`rank_7t1_is_target_race_type`）と看板判定は別物**。前者は
> 準決勝を**含み**、後者は**除外する**。検証は前者の定義で行った（準決勝は母集団の
> 33%で成績は本体と同等・20万超 2.17% vs 2.02%）。**2つを混同しないこと。**
>
> 🔴 **【2026-08-17】看板レースは出稿しない**（`RANK_7T1_SKIP_MARQUEE = True`・
> ユーザー判断）。母集団の定義は変えず、選出（`rank_7t1_daily_select`）で落とす。
> - **理由**: 7T1 の対象種別と看板キーワードは準決勝を除いてほぼ一致するため、
>   7T1 は構造的に看板へ寄る。実測（入稿 08-13〜）で**看板の入稿70件中30件(42.9%)が
>   7T1**、7T1入稿44件の68%が看板だった。看板は**当日売上の84%**が集中するレースで、
>   そこでの表示的中は 7S 32.5% / 7C 26.1% に対し **7T1 は 4.2%**
> - 🔴 **優先順位の変更では効かない**。入稿順は既に 7S > 7C > 7T1 なので、看板で
>   7T1 が出るのは**両方がゲートに落ちたレース**。母集団から外すしかない
> - 🟢 **看板に穴は空かない**（`submit_marquee_wt` の `RANK_BY_CARS = {7:"7S"}` が埋める）
> - 🔴 **代償**: 残るのは準決勝のみで **225→81件（36%）**、13.4本/日 → 概ね4〜5本/日。
>   15万超を出せるランクは 7T1 だけ（7S 0.00% / 7C 0.08%）で、しかも看板側のほうが
>   高い（看板 3.47% / 準決勝 1.23%）＝**一撃力の本体を手放している**
> - ⚠️ **種別別の ROI を判断に使わないこと**（決勝135.5%/n=48・準決勝44.4%/n=81 等）。
>   どのセルも的中1〜3件で1本の配当が支配する。確実なのは表示的中の差だけで、
