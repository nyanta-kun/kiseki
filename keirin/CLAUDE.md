...
- 🔴 **`netkeirin_settings.enabled` は「入稿の可否」と「Web の可視性」を兼ねている**（2026-08-18 判明）。
  ...
  **入稿を止めると画面からも消える**。検証中のペーパーランクを「売らずに画面で追う」ことが
  ...
  - ⚠️ 現在ペーパーのまま画面に出ないのは **7B / 7H2 / 9H1**。成績を見るには DB を直接引くか、
    可視性フラグを `enabled` から分離する改修が要る
  - ⚠️ 廃止済みランク（`ABOLISHED_PAPER_RANKS`）を画面に戻してはいけない。
    分離するなら `CURRENT_PAPER_RANKS` を基準にすること
