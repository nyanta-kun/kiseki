...
RANK_ALIASES = {"9S": "7S", "9A": "7A", "9H1": "7H1"}
...
SHAPE_TITLES: dict[str, dict[str, str]] = {
    ...
    # 7H3 — 本命連対どまり型・三連単の高配当。**上位2車は「来る」と読んでいる**
    # ので、7H1/7H2 のように「人気を疑う」「本命が飛ぶ」と書いてはいけない
    # （買い目は上位2車を2〜3着に必ず含むため、文面と商品が食い違う）。
    "7H3": {
        SHAPE_SOLO: "抜けた1車も勝ち切れない",
        SHAPE_DUO: "上位2車は連対どまり",
        SHAPE_LINE: "並びの主役を2〜3着に置く",
        SHAPE_SPLIT: "別線対決で上位2車が止まる",
        SHAPE_CLASH: "先行争いの決着を差し込む",
        SHAPE_MIXED: "1着だけが読めない",
    },
}
...
SHAPE_NOTES: dict[str, dict[str, str]] = {
    ...
    "7H3": {
        SHAPE_SOLO: "頭ひとつ抜けた1車がいます。当方はこれが2着か3着までと読みました。",
        SHAPE_DUO: "上位2車は堅いと見ています。ただし勝つのは別の選手と読みました。",
        SHAPE_LINE: "並びの中心は上位2車。その2車が勝ち切れないほうへ賭けます。",
        SHAPE_SPLIT: "有力どころが別ラインに分かれ、上位2車が止まりやすい一戦と見ました。",
        SHAPE_CLASH: "先行争いが激しく、上位2車の後ろから差し込まれる形を狙います。",
        SHAPE_MIXED: "上位2車は残ると見ています。1着だけが読みにくい一戦です。",
    },
}
...
SHAPE_NOTE_FALLBACK = {
    ...
    "7H3": "上位2車は残るが勝つのは別の選手、と読んだ高配当ねらいの一戦です。",
}
