...
SHAPE_FALLBACK = {
    ...
    "7H1": "本命バスト警報",
    "7H2": "人気薄から高配当",
    # ⚠️ 「看板」と書かない。7T1 の母集団は決勝系レースで、その23.8%は
    #    チャレンジ／ガールズ。クラスの高さを謳うと約1/4で事実に反する。
    "7T1": "3着だけを絞る",
    # 7T3（2026-08-24）。決勝 × 予測30倍以上の帯。🔴 **万車券を謳わない**
    #    （的中の1万円以上は 4.7%＝年6件・3万円超は0件）。
    "7T3": "決勝で中配当を狙う",
    # 7M1（中間層）。「混戦 × 公式印と不一致」が母集団なので、堅さを謳わない。
    "7M1": "評価が割れた混戦",
}
...
SHAPE_TITLES: dict[str, dict[str, str]] = {
    ...
    # 7M1 — 混戦 × 公式印と不一致。相手を指数下位3車へずらして配当を作る。
    # 🔴 **「堅い」「手堅く」の類は書かない。** 的中は12%前後で、7C とは狙いが逆。
    #    書くのは「評価が割れている」「相手を人気薄へずらした」の2点。
    "7M1": {
        SHAPE_SOLO: "1車は抜けたが印と割れた",
        SHAPE_DUO: "上位2車と印が食い違う",
        SHAPE_LINE: "ライン決着でも相手は薄目",
        SHAPE_SPLIT: "別線対決で3着が荒れる",
        SHAPE_CLASH: "先行争いで3着が入替わる",
        SHAPE_MIXED: "混戦の3着に妙味",
    },
    ...
    "7T1": {
        ...
        SHAPE_DUO: "上位2車の3着だけを絞る",
        SHAPE_SPLIT: "別線対決の決着順を読む",
        SHAPE_CLASH: "先行争いでも二軸を固定",
        SHAPE_MIXED: "混戦の3着を少点数で獲る",
    },
    # 7T3（2026-08-24）。🔴 **7T1 とは買い方が違う**。7T1 は上位2車を1着・2着に
    #    固定して3着を流すが、7T3 は**帯（予測30倍以上）の中から確率上位の
    #    決着順を5点**採るだけで、軸を固定しない（5点の1着が1車に揃うのは 7.0%）。
    #    したがって「1着・2着に固定」と書いてはいけない（買い目と文面が食い違う）。
    # ⚠️ ライン条件を持たないので **6構造すべてが発生しうる**（7T1 が line を
    #    欠くのとは逆）。実際には優先順位で 7T1 が別ラインを取るため line が多い。
    "7T3": {
        SHAPE_SOLO: "抜けた1車が絡む高配当だけ",
        SHAPE_DUO: "上位2車が絡む高配当だけ",
        SHAPE_LINE: "並びどおりの決着に賭ける",
        SHAPE_SPLIT: "別線決着の高配当をねらう",
        SHAPE_CLASH: "先行争いの決着に賭ける",
        SHAPE_MIXED: "混戦の決着順を絞る",
    },
}
...
SHAPE_NOTES: dict[str, dict[str, str]] = {
    ...
    "7M1": {
        SHAPE_SOLO: "1着率で1車が抜けていますが、当方の上位2車は公式予想の印と食い違っています。相手は人気どころを外しました。",
        SHAPE_DUO: "上位2車が公式予想の印と一致しません。評価が割れている分、同じ的中でも配当が付きます。",
        SHAPE_LINE: "軸2車は同じライン。並びは素直に見つつ、3着だけを人気薄へずらしました。",
        SHAPE_SPLIT: "軸2車は別ライン。3着が荒れやすいと見て、相手を指数下位の3車に絞りました。",
        SHAPE_CLASH: "先行争いが見込まれる一戦。3着が入れ替わりやすいと見て、人気どころを相手から外しました。",
        SHAPE_MIXED: "拮抗した一戦で、当方の評価は公式予想の印とも食い違っています。3着に妙味を求めました。",
    },
    "7SS": {
    ...
    # 🔴 **7T1 の見解文で軸の的中を約束しないこと。** 実測（honest walk-forward・
    #    決勝系×別ライン 2,898R）は 二軸とも3着内 39.7% / 軸1-軸2が1-2着 9.1% で、
    #    **オッズ板の人気上位2車（47.4%）より低い**。これは不具合ではなく、
    #    払戻15万円に届く目だけを買うという目的から出る当然の帰結
    #    （同じレースで素直に指数1-2位を軸にしても 43.3% しか行かない＝
    #    決勝系×別ラインという母集団自体が荒れる）。
    #    したがって「堅い」「読み切りました」「決まると見ています」の類は書かない。
    #    **狙いは的中数ではなく配当の大きさ**だと毎回述べること。
    "7T1": {
        SHAPE_SOLO: "頭ひとつ抜けた1車がいます。ここを1着に固定し、配当が伸びる3着だけを絞りました。",
        SHAPE_DUO: "上位2車を1着・2着に固定し、3着だけを絞りました。的中の回数より一撃の大きさを優先した組み立てです。",
        SHAPE_SPLIT: "上位2車が別ラインに分かれた一戦です。決着順まで踏み込んで固定し、高い配当のつく形だけを狙いました。",
        SHAPE_CLASH: "先行争いが激しい一戦ですが、上位2車を軸に構えました。狙うのは的中の回数ではなく配当の大きさです。",
        SHAPE_MIXED: "混戦の一戦です。上位2車で決着順を固定し、3着だけを少点数に絞って高配当を狙います。",
    },
    # 7T3（2026-08-24）。母集団は**決勝のみ**なので毎回そう述べてよい。
    # 🔴 **「固定」と書かない**（7T1 と違い軸を置かない）。
    # 🔴 **万車券・一撃を謳わない。** 的中の払戻は1万円以上（100倍超）が 4.7% で
    #    3万円超は0件。「週2〜3ヒット」と「万車券」は同時に成立しない。
    # 🔴 **「堅い」「読み切った」の類も書かない。** 的中は約10%で、
    #    狙いは的中の回数ではなく配当の大きさ（7T1 の注記と同じ理由）。
    "7T3": {
        SHAPE_SOLO: "頭ひとつ抜けた1車がいる決勝です。その1車が絡む決着順のうち、配当が大きくつく形だけを選びました。",
        SHAPE_DUO: "上位2車がはっきりした決勝です。その2車が絡む決着順から、配当が大きくつく形だけに絞りました。",
        SHAPE_LINE: "上位2車が同じラインの決勝です。並びどおりに決まる形の中から、配当のつく決着順を選びました。",
        SHAPE_SPLIT: "上位2車が別ラインに分かれた決勝です。どちらが主導権を取るかで決着順が変わるぶん、配当のつく形を狙いました。",
        SHAPE_CLASH: "先行争いが激しい決勝です。展開が読みにくいぶん配当がつきやすく、その中から狙う決着順を絞りました。",
        SHAPE_MIXED: "力が拮抗した決勝です。決着順の組み合わせが多いぶん配当がつきやすく、その中から見込みのある形だけを選びました。",
    },
}
...
SHAPE_NOTE_FALLBACK = {
    ...
    "7M1": "評価が割れた混戦で、相手を人気薄へずらした中穴ねらいの一戦です。",
    "7T3": "決勝戦のうち、配当が大きくつく決着順だけを選んだ高配当ねらいの一戦です。",
}
