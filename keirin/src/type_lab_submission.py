...
#: 🔴🔴 **2026-09-15 から7車は「型」ではなく「段」で売る**（ユーザー決定 2026-09-14）。
#:    軸信頼（p3上位2車の合計）で 硬い / やや波乱 / 波乱 の3段に分け、段ごとに買い方を固定する。
#:    🔴 **商品名（ランク名称）は既存と重ならない語にする**（ユーザー決定 2026-09-14）:
#:    硬い＝「固め」・やや波乱＝「広め」・波乱＝「荒れ」。旧キーの「本線」「押さえ」「高配当」は使わない
#:    （過去の入稿と名前が重なると、Discord・画面で新旧の商品を見分けられなくなる）。
#:    型A〜F は段と独立に決まるので、**段の商品に型の見解（例: 型A「二軸が堅い一戦」）を
#:    付けてはいけない**——「型Aの鉄板レースをやや波乱の商品として売る」組み合わせが出て、
#:    タイトルと中身が食い違う。見解・本文は下の `PLAN_VIEWS` / `PLAN_NOTES` / `PLAN_BODIES`
#:    で**段ごとに上書き**する（`build_title` / `build_comment` はプランの上書きを優先する）。
#: 例外は `A_ana`（型A かつ1着が読めない）で、段より先に判定して従来の文面のまま売る。
...
PLAN_NOTES: dict[str, str] = {
    ...
    "A_ana": ("指数の上位2車ははっきり抜けている一方で、1着がどれになるかは"
              ...
              "配当は大きくなります。"),
    # 🔴 段の商品は**型の見解を使わない**（`TIER_LABELS` の節）。買い方には触れない
    #    （買い方は `PLAN_BODIES`）。
    "T_firm": ("指数の上位がはっきり抜け、並びの入れ替わりまでで決まると読んだ一戦です。"),
    "T_mid": ("上位は見えているものの、崩れる余地も残る一戦と読みました。"),
    "T_axis": ("1着候補が一人抜けている一方、2着・3着は混戦と読み、相手を手広く押さえる一戦です。"),
    "T_upset": ("人気を集める選手のラインが崩れる余地の大きい一戦と読みました。"),
    # 🔴 逃げ先頭ライン（`L_lead`・2026-09-24・**検証中・入稿しない**）。
    #    売ると決めたときにそのまま使える文面として置いてある（全プランに文面を要求するテストのため）。
    "L_lead": ("人気の中心とは別のラインで、先行する選手がそのまま押し切り、番手が続く展開を"
               "狙う一戦です。"),
}
...
#: 一軸（1着を1着率1位に固定する段の商品）。軸は買い目の1着から拾う（`build_submission`）。
ONE_AXIS_PLAN = "T_axis"
...
# 🔴 **`T_upset` は人気1位の選手のラインを1・2着から外して組む**（2026-09-14）。
#    指数上位2車がそのラインに居ると買い目に入らないので、既定の
#    「二軸は◎○です」を出すと**買っていない車を軸として案内する**。
#    ◎は `marks_for` が買っている車から振り直すので、ここでも ◎○ を名指ししない。
PLAN_AXIS_NOTES["T_upset"] = (
    "【二軸】\n本レースの指数上位2車は {a1}番・{a2}番です。"
    "ただしこの商品は人気1位の選手のラインを1・2着から外して組んでいるため、"
    "この2車が買い目に入らないことがあります。印は買っている車だけに付けています。")

# 🔴 **`T_axis` の軸は「1着率1位」で、指数（3着内率）上位2車とは限らない**（2026-09-15）。
#    `build_submission` が買い目の1着（全点共通）を ◎、確率最上位の目の2着を ○ として渡す。
# 🔴🔴 **販売面では「一軸」と書かない**（2026-09-15 ユーザー指示・ブランド「二軸探偵」）。
#    見出しは他の商品と同じ【二軸】のまま、◎を1着に据えて相手を「手広く」押さえる形で説明する。
PLAN_AXIS_NOTES["T_axis"] = (
    "【二軸】\n本レースで照らし出した二軸は、◎{a1}番・○{a2}番です。"
    "1着は ◎{a1}番 に据え、2着・3着は ○{a2}番 を筆頭に、"
    "当方の指数で確率の高い相手を手広く押さえました。")
...
PLAN_VIEWS: dict[str, str] = {
    ...
    **{f"{_t}_big": "指数1位が3着にも残らない側" for _t in "ABCDEF"},
    # 🔴 段の商品は型の見解（`TYPE_VIEWS`）を使わない（`TIER_LABELS` の節）。
    "T_firm": "上位が抜けた本線決着の見立て",
    "T_mid": "崩れも考えるレース",
    "T_axis": "抜けた1着候補から相手を広げる見立て",
    "T_upset": "人気のラインが崩れる見立て",
    "L_lead": "別線の逃げが押し切る見立て",
}
...
TYPE_NOTES: dict[str, str] = {
    "A": ("指数の上位2車がはっきり抜け、レースが荒れる要素も見当たらない一戦です。"
          "着順まで踏み込んで狙えると読みました。"),
    # 🔴🔴 **2026-09-22 是正。** 旧文の2文目（B「3着の可能性がある目を広めに拾います」/
    #    C「配当の付く帯へ寄せています」）は**買い方の記述**で、`B_hit`/`C_hit` を
    #    計画払戻5万円のダッチへ替えた時点で事実と食い違った（B は広くない・C に帯は無い）。
    #    この節が自分で禁じている「`TYPE_NOTES` に買い方を書かない」に倣い、読みだけにする。
    "B": TYPE_READS["B"],
    "C": TYPE_READS["C"],
...
PLAN_TITLES: dict[str, str] = {
    # 🔴 **2026-09-22 に「本線」から改めた**（`type_lab.HIT_BAND_TARGET`）。
    #    買い方が「確率上位3点・信頼度傾斜」から「計画払戻5万円のダッチ」へ変わり、
    #    払戻中央が 19,660 → 31,000円 になったので「本線」では性格を説明できない。
    "A_hit": "中配当狙いの三連単",
    "A_pay": "一撃の三連単",
    ...
    "A_trio": "本線の三連複",
    "A_ana": "波乱狙いの三連単",
    # 🔴 **2026-09-22: A/B/C は買い方が同一になったので文言も揃える**
    #    （看板枠を6型とも同じ文言にしているのと同じ理由。型の違いは
    #    タイトル後半のレース見解と `TYPE_NOTES` が担う）。
    "B_hit": "中配当狙いの三連単",
    "C_hit": "中配当狙いの三連単",
    "D_hit": "混戦の三連複",
    "E_hit": "高配当の三連単",
    ...
    "F_line": "ライン狙いの三連複",
    # 🔴 **段の商品（2026-09-14）。名前は既存と重ならない語にする**（`TIER_LABELS` の節）。
    #    固め＝当てにいく／広め＝点数を広げて押さえる／荒れ狙い＝一撃。
    "T_firm": "固めの三連単",
    "T_mid": "広めの三連単",
    "T_axis": "手広く狙う三連単",
    "T_upset": "荒れ狙いの三連単",
...
PLAN_BODIES: dict[str, str] = {
    # 🔴 **2026-09-22 に構成が変わった**（`prob_top` 3点 → `signboard`＝計画払戻5万円のダッチ）。
    #    旧文面「3点に絞りました」は点数が可変になったので**事実と食い違う**。
    # 🔴 **A/B/C は同一文**（買い方が同じ）。配分の説明は `alloc_note` が実際の賭け金から
    #    書くので、ここで「オッズに応じて配分」と重ねて書かないこと。
    # 🔴 **「◎○が2着・3着へ回る目も含みます」と書かない。** 確率順に積む形では
    #    ◎○が1点も入らないレースがありうる（`axis_note` がそのとき別の文を出す）。
    "A_hit": ("買い目は三連単。点数を欲張らず、当たったときに手元へ残る額のほうを取る組み立てにしました。当方の指数で確率の高い目から順に、狙う払戻を割らない範囲まで積んでいます。着順は決め打ちしていません。"),
    ...
    "B_hit": ("買い目は三連単。点数を欲張らず、当たったときに手元へ残る額のほうを取る組み立てにしました。当方の指数で確率の高い目から順に、狙う払戻を割らない範囲まで積んでいます。着順は決め打ちしていません。"),
    # 🔴 **2026-09-22 に帯（15倍）が無くなった**ので「一定の配当が付く目に絞り」を外した。
    #    帯下の差込も無くなったため `UNDERBAND_BANDS` からも落としてある。
    "C_hit": ("買い目は三連単。点数を欲張らず、当たったときに手元へ残る額のほうを取る組み立てにしました。当方の指数で確率の高い目から順に、狙う払戻を割らない範囲まで積んでいます。着順は決め打ちしていません。"),
    ...
    # 🔴 **段の商品（2026-09-14）**。点数は書かない（`build_comment` が実数を入れる）。
    #    🔴 **金額を数字で書かない**（看板枠の節と同じ理由・`test_no_baked_in_numbers`）。
    #       計画払戻（固め/広め 約2.2万円・荒れ 約10万円）は当方の想定オッズでの目安で、
    #       確定オッズは下振れしうるので「当たれば◯円」と約束しない。
    #    どれも**払戻を揃える配分（ダッチ）**なので、配分の説明は `alloc_note` と矛盾しない。
    "T_firm": ("買い目は三連単。当方の指数で確率の高い目から、並びの入れ替わりまでを押さえて"
               "点数を絞りました。当方が想定する発走時オッズで、どの目が当たっても払戻が"
               "ほぼ同じ額になるよう賭け金を配分しています。"
               "◎○が2着・3着へ回る目も含みます。"),
    "T_mid": ("買い目は三連単。上位の崩れも考えて点数を広めに取り、当方の指数で確率の高い目から"
              "押さえました。配分は、当方が想定する発走時オッズでどの目が当たっても払戻が"
              "ほぼ同じ額になる形です。◎○が2着・3着へ回る目も含みます。"),
    "T_axis": ("買い目は三連単。1着は◎に据え、2着・3着は当方の指数で確率の高い相手を手広く、"
               "並びの入れ替わりまで押さえました。当方が想定する発走時オッズで、どの目が当たっても"
               "払戻がほぼ同じ額になるよう賭け金を配分しています。"),
    "T_upset": ("買い目は三連単。この商品は、人気1位と見込まれる選手とそのラインの選手を"
                "1着・2着から外しています。当方が想定する発走時オッズで配当の付く目に絞り、"
                "どの目が当たっても大きな払戻がほぼ同じ額でそろうよう賭け金を配分しました。"
                "当たる回数は少なく、狙いは一撃の大きさです。"),
    # 🔴 逃げ先頭ライン: 配分は**均等**（ダッチではない）ので「払戻が揃う」とは書かない。
    "L_lead": ("買い目は三連単。人気の中心とは別のラインの先行選手を1着、その番手を2着に固定し、"
               "3着は届きにくい選手を外して流しました。賭け金は各点同額です。"
               "当たる回数は少なく、狙いは一撃の大きさです。"),
}
...
#: 🔴 **2026-09-22 に空になった。** `C_hit` が帯15倍をやめて計画払戻5万円のダッチへ
#:    変わり、帯も差込も持たなくなったため（`type_lab.HIT_BAND_TARGET`）。
#:    残したまま `min_odds=0` の商品へ当てると、**通常の買い目を「押さえの1点」と
#:    説明する**ことになる。上の一致テストがそれを止める。
UNDERBAND_BANDS: dict[str, float] = {}
...
def underband_note(plan_key: str, legs: Sequence[Mapping]) -> str:
    ...
    🔴 **2026-09-22 から `UNDERBAND_BANDS` は空**（型C が帯を持たなくなった）ので、
       いまはどのプランでも空文字を返す。仕組みは型F のゲート落ちの受け皿のために残す。
...
def build_submission(row: Mapping, entry_table_html: str | None = None) -> dict:
    ...
    # 🔴 **一軸は買い目の1着（全点共通）が軸**、相手は確率最上位の目の2着（2026-09-15）。
    #    `axis1/axis2` は3着内率の上位2車で、1着率1位と一致しないことがある（約15%）。
    if plan_key == ONE_AXIS_PLAN and legs:
        firsts = {str(lg["combo"]).split("-")[0] for lg in legs}
        if len(firsts) == 1:
            top = max(legs, key=lambda lg: float(lg.get("prob") or 0.0))
            parts = str(top["combo"]).split("-")
