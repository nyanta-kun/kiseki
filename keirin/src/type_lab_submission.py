...
for _t in "ABCDEF":
    ...
    PLAN_TITLES[f"{_t}_big"] = "特大狙いの三連単"
del _t
...
for _t in "ABCDEF":
    PLAN_BODIES[f"{_t}_sign"] = (
        ...
        "◎○が2着・3着へ回る目も含みます。"
        "そのぶん外れる回数は多くなります。")
    # 🔴 **`_big` は「軸1を買わない」ことを必ず書く。** 印は付くのに◎絡みの目が
    #    1点も無い商品なので、書かないと「印と買い目が違う」と読まれる。
    PLAN_BODIES[f"{_t}_big"] = (
        "買い目は三連単。この商品は「指数1位が3着にも入らない」側に賭けています。"
        "そのため指数1位が絡む目は1点も買っていません。"
        "当方が想定する発走時オッズで特に配当の大きい目に絞り、"
        "どの目が当たっても払戻がほぼ同じ額になるよう賭け金を配分しました。"
        "当たる回数は少なく、狙いは一撃のみです。")
del _t
