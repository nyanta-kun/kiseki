...
PLAN_NOTES: dict[str, str] = {
    ...
    "T_axis": ("1着候補が一人抜けている一方、2着・3着は混戦と読んだ一戦です。"),
...
# 🔴 **一軸（`T_axis`）の軸は「1着率1位」で、指数（3着内率）上位2車とは限らない**（2026-09-15）。
#    `build_submission` が買い目の1着（全点共通）を ◎、確率最上位の目の2着を ○ として渡す。
PLAN_AXIS_NOTES["T_axis"] = (
    "【一軸】\n本レースは1着を ◎{a1}番 に絞った一軸の組み立てです。"
    "全ての買い目で ◎{a1}番 が1着です。2着・3着は混戦と見て、"
    "当方の指数で確率の高い相手から並べました（相手筆頭は ○{a2}番）。")
...
PLAN_BODIES: dict[str, str] = {
    ...
    "T_axis": ("買い目は三連単。1着は◎に固定し、2着・3着は当方の指数で確率の高い組み合わせから、"
               "並びの入れ替わりまでを押さえました。当方が想定する発走時オッズで、どの目が当たっても"
