...
#: 🔴🔴 **当てにいく商品のうち「堅い3型（A/B/C）」が狙う計画払戻**（2026-09-22・ユーザー判断）。
#:
#: 発端はユーザー観察「当たらないし、払い戻しが下がりすぎ。4万以上の払い戻しを取りたい」。
#: 実売を分解すると、当てにいく商品の計画払戻中央が 30,100 → 24,440円（−19%）へ落ちており、
#: 9/08〜9/21 の5つの変更（型C の帯下差込・型F の下限・τ適応・並び違いの押さえ）が
#: **すべて「払戻を下げて表示的中を買う」向き**だった。1つずつは小さく、全部同じ向き。
#:
#: 🔴 **元へ戻すだけでは届かない。** 5変更を revert した腕は 4万+ が 1.23 → 1.46件/日
#:    （+19%）止まりで、表示的中 −1.5pt・件数 −0.9件/日 を払う。
#:
#: 🔴 **予算は netkeirin の上限に張り付いている**（`docs/netkeirin-input-api-spec.md` §7.3・
#:    合計買い目金額は 1,000〜10,000円）。だから「単位を上げて払戻を倍にする」手は無く、
#:    ダッチの恒等式 `P(払戻 >= X) ≒ ROI × 予算 / X` がそのまま効く。回収率は控除率で
#:    78〜82% に張り付いている（`docs/AUDIT_2026_09_20.md` §3）ので、
#:    **残る自由度は「1商品で狙う払戻」ひとつだけ**＝表示的中との1本のダイヤルになる。
#:
#: 掃引（本番コードのまま買い方だけ差し替え・確認 2026-01〜08-25 / 探索 2024-07〜2025-12。
#: `scripts/exp_type_lab/lineup_arms.py`。高額枠の是正（`HIGHPAY_BIG_SLOTS=∅`）込み）:
#:
#:   狙う額   件/日   表示的中(確認/探索)   ROI        払戻中央   3万+/日      4万+/日
#:   現行     31.3   23.69 / 23.19%      83.1/80.9  22,960    2.16/2.00   1.23/1.11
#:   4万      32.7   20.69 / 20.52%      82.3/81.4  27,010    2.89/2.66   1.71/1.53
#:   **5万**  32.7   19.40 / 19.27%      82.0/79.7  28,800    3.00/2.80   **1.81/1.70**
#:   6.5万    32.7   18.31 / 17.93%      83.1/79.3  30,550    3.04/2.83   2.06/1.85
#:
#: 対比較（日ブートストラップ2,000本・5万狙い ↔ 現行）:
#:   Δ表示的中 **−4.30pt [−5.08,−3.54] / −3.92 [−4.42,−3.44]**
#:   Δ4万+/日  **+0.59 [+0.47,+0.71] / +0.59 [+0.52,+0.67]**
#:   Δ3万+/日  +0.83 [+0.67,+1.00] / +0.79 [+0.69,+0.90]
#:   Δ件/日    +1.35 [+1.19,+1.51]（ダッチのほうが入稿ゲートを通りやすい）
#:   ΔROI      −1.07 [−4.92,+2.88] / −1.20 [−4.35,+1.75]（**判別不能＝壁のまま**）
#:
#: 🔴 **堅い3型だけに当てること。** 同じ操作を混戦側（型D/E/F）に当てても 4万+ は
#:    1.34件/日 までしか伸びない（あちらは既に高い帯を買っている）。6型全部に当てる腕は
#:    表示的中が 3.4pt 低いのに 4万+ は同等＝効率境界の内側に落ちる。
#: 🔴 **6.5万以上へ上げない。** 4万+ は 6〜8万で頭打ちになり、そこから先は表示的中だけが減る。
#: ⚠️ **2026-09-14 に置いた「全体の表示的中30%維持」はこの変更と正面から衝突する。**
...
#: 🔴🔴 **2026-09-22 に空へした**（＝5本とも計画15万の `{型}_sign`）。監査 §6.2 の H1。
#:    実売（2026-08-07〜09-21）で `_big` は **32件・的中0・ROI 0%**、売上も
#:    `_sign` 1,288pt/R ↔ `_big` 1,200pt/R で優位が無い。台で測っても全指標が非負:
#:      Δ表示的中 +0.21 [+0.07,+0.35] / +0.13 [+0.04,+0.23]（確認 / 探索）
#:      Δ4万+/日  +0.06 [+0.02,+0.11] / +0.04 [+0.01,+0.07]
#:      Δ10万+/日 +0.04 [+0.00,+0.07] / +0.03 [+0.00,+0.05]   件数は1件も減らない
#:    ⚠️ 失うのは **30万超の払戻**（`bust=True` のこの形でしか出ない）。戻すなら
#:       `frozenset({2, 4})` へ戻すだけでよい（`{型}_big` の定義は残してある）。
HIGHPAY_BIG_SLOTS: frozenset[int] = frozenset()
...
PLANS: dict[str, Plan] = {
    # 型A 鉄板 — **2026-09-22 に「確率上位3点・信頼度傾斜」から
    #            「どの目が来ても `HIT_BAND_TARGET` 円のダッチ」へ替えた**（`HIT_BAND_TARGET` の節）。
    # 🔴 旧構成は表示的中 38.0% と全プラン最良だったが、**払戻中央 19,660円・
    #    4万+ 0.04件/日**＝1日3件売って4万円以上をほとんど作れていなかった。
    # ⚠️ 点数は固定しない（Σ(1/予測オッズ) <= 予算/目標 に詰められるだけ。実測 平均5.0点）。
    "A_hit": Plan("A_hit", "A", "trifecta", "signboard", 0,
                  max_odds=SIGNBOARD_MAX_ODDS, alloc="dutch",
                  floor_mult=MIN_PAYOUT_MULT, target=HIT_BAND_TARGET,
                  note="確率順に Σ(1/予測オッズ) <= 予算/5万 まで（どの目でも約5万円）"),
    ...
    # 型B 堅い・中 — **2026-09-22 に `sigma_max=1/3`（実質3万円の床）＋信頼度傾斜から
    #            `HIT_BAND_TARGET` 円のダッチへ替えた**（`HIT_BAND_TARGET` の節）。
    # 🔴 `sigma_max` とダッチの組み合わせは**同じ式の両側**（平均想定払戻 = 予算 ÷ Σ）なので、
    #    やっていることは「床を 3万 → 5万 へ上げ、配分を傾斜からダッチへ揃えた」だけ。
    #    傾斜だと当たりやすい安い点に厚く乗るので、**当たったときの払戻が床を大きく下回る**
    #    （実測 計画中央 25,730円に対し払戻中央 21,450円）。
    "B_hit": Plan("B_hit", "B", "trifecta", "signboard", 0,
                  max_odds=SIGNBOARD_MAX_ODDS, alloc="dutch",
                  floor_mult=MIN_PAYOUT_MULT, target=HIT_BAND_TARGET,
                  note="確率順に Σ(1/予測オッズ) <= 予算/5万 まで（どの目でも約5万円）"),
    ...
    # 🔴🔴 **2026-09-22: 帯15倍・τ適応・帯下差込をやめ、`HIT_BAND_TARGET` 円のダッチへ替えた**
    #    （`HIT_BAND_TARGET` の節）。上の 09-01 / 09-08 / 09-11 の3つはいずれも
    #    **払戻を下げて表示的中を買う**操作で、累積すると計画払戻中央が
    #    33,670 → 20,900円（−38%）まで落ちていた。**4万+ は 0.06件/日**。
    #    ⚠️ 上の実測表（帯20→15・帯下差込・τ適応）は**この置き換えで全て無効になる**。
    #       戻すときは `GATE_FALLBACK["C_hit"]` の定義がそのまま旧構成なので流用できる。
    "C_hit": Plan("C_hit", "C", "trifecta", "signboard", 0,
                  max_odds=SIGNBOARD_MAX_ODDS, alloc="dutch",
                  floor_mult=MIN_PAYOUT_MULT, target=HIT_BAND_TARGET,
                  note="確率順に Σ(1/予測オッズ) <= 予算/5万 まで（どの目でも約5万円）"),
...
#:
#: 🔴🔴 **2026-09-19: 10 → 5 へ戻した**（ユーザー判断）。発端は「全体の的中率が
#:    上がらない」という観察。**枠を増やした側の代償（表示的中）が、得られた
#:    看板（10万+）に見合っていなかった。**
#:
#:    前向き実測（実入稿・2026-09-12〜09-18・7日・採点済 396件。6本目以降を
#:    落として再集計した同一母集団の対比較）:
#:
#:      腕          件数  件/日   表示的中   10万+   ROI
#:      現行(10本)   396   56.6   22.47%      2件   56.5%
#:      **5本**      375   53.6   **23.47%**  1件   56.3%
#:
#:    ＝ **表示的中 +1.00pt / 10万+ −1件 / ROI 不変 / 件・日 −3.0**。
#:    高額枠そのものの表示的中は **4.1%**（73件中3件・2026-08-29〜09-18）で、
#:    入稿の 13% を占めていた。9/17 は通常商品だけなら 31.0% なのに全体 25.0% と、
#:    **希釈が 6pt** 出ていた。
#:
#: 🟢 **供給源（軸信頼ゲート落ち）は残す。** 戻したのは本数だけ。doc の②
#:    「供給源を足すだけ（5本）」に相当し、Δ10万+/日 +0.032〜+0.091 は
#:    表示的中をほとんど払わずに取れる側だから。
#: ⚠️ n=396・7日で、10万+ は 2件 ↔ 1件。**看板側の差は Poisson の揺らぎの内**。
#:    確かなのは表示的中と件数の方で、10万+ が本当に半減するかは前向きに見ること。
...
def highpay_plan_for(type_label: str | None, n_entries: int | None,
    ...
    >>> [highpay_plan_for("B", 7, i) for i in range(5)]   # HIGHPAY_BIG_SLOTS=∅（2026-09-22）
    ['B_sign', 'B_sign', 'B_sign', 'B_sign', 'B_sign']
...
GATE_FALLBACK: dict[str, tuple[Plan, ...]] = {
    ...
    #       外すなら測り直すこと。
    # 🔴 **2026-09-22: `replace(PLANS["C_hit"], ...)` をやめ、旧構成を明示で持つ。**
    #    本命が「5万円のダッチ」になったので、`replace` だと代替が本命と同じものになり
    #    **フォールバックが意味を失う**（ゲートに落ちたレースの在庫がそのまま消える）。
    #    ここに残すのは 2026-09-21 までの C_hit そのもの（帯15倍・τ適応・差込なし）で、
    #    5万円のダッチが組めないレースだけ従来の買い方で売る＝在庫は1件も減らない。
    "C_hit": (Plan("C_hit", "C", "trifecta", "prob_top", 0, min_odds=15.0,
                   ...
                   note="C_hit がゲートに落ちたとき: "
                        "予測15倍以上から想定2万を割らない点数（2026-09-21 までの構成）"),),
}
...
ALLOC_BEFORE_MIN_PAYOUT: dict[str, tuple[str, float]] = {
    # 🔴 **2026-09-22 に `A_hit` / `B_hit` / `C_hit` を外した。** 3つともダッチへ移り、
    #    `_allocate_once` はダッチでは床（`floor_mult`）を一切見ないので、この代替は
    #    構造的に発火しない。残すと「ダッチの商品が信頼度傾斜へ落ちうる」と読める
    #    死んだ分岐になる（`alloc_fallback` は alloc か floor_mult が違えば返してしまう）。
