...
## 欠測時に型判定が退化することの記録（2026-09-02）

見送った側の `type_lab_picks.plan_key` / `submission_skips.rank_key` は
**そのレースの型ではない**。欠測入力から出た偽の型なので、集計に使わないこと。

`type_lab` の荒れ度は

    arare = ライン構成 + ラインの維持 + 先頭の脚質 + (開催日目 − 2) + 番手の得点

の5項で、**うち4項が並び依存**。並びが無いと「全員が同じ1本のライン」として
読まれて4項が全レース同値になり、`arare` は実質 **「開催日目 − 2」だけの関数**へ
退化する。印が全車ゼロだと `axis_sum` も下がるので必ず混戦側（D/E/F）へ落ちる。
結果、**同じ回の欠測レースが全部同じ型**になる:

    2026-08-29 morning（開催日目3）  18件すべて F
    2026-08-31 morning（開催日目2）  19件中18件が E
    2026-09-01 morning（開催日目3）  16件すべて F

後の波で並びが取れると型はばらける（2026-09-01 は16件中12件が別の型になった）。
見送りを型別に集計すると「特定の型だけ取りこぼしている」と読み違えるので、
**集計は `reason_code` だけで行う**（2026-09-02 に実際に読み違えた）。

🔴 商品そのものへの影響は無い。この関数が入稿前に止めており、「自信あり」の選定も
   `netkeirin_submissions` と突き合わせるので未入稿のレースは対象にならない。
   日次上限の分母からも外れている（`netkeirin_submit_type_lab._reject` が
   `judged=False` を返す）。**壊れるのは記録の読み方だけ。**
"""
