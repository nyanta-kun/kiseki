...
# ═══════════════════════════════════════════════════════════════════════════
# 【2026-08-26 改定】判定する数字を「素の予測オッズ」→「下限包絡」へ替えた
# ═══════════════════════════════════════════════════════════════════════════
#
# 🔴 **閾値 1.5 は据え置き。変えたのは「何を 1.5 と比べるか」。**
#    `netkeirin_submit_wt._expected_payout_floor_for` が予測オッズへ
#    保守倍率 c(p25)（7車 0.843 / 9車 0.859）を掛けてから最小値を取る。
#
# 素の予測オッズで測ると**下限を系統的に高く見積もる**。原因は2つとも構造的:
#   ① 整合化（Σ(1/o) を定数へ再スケール）が素の点予測を約8%下げる方向に効くので、
#      買う点は確定オッズのほうが約7%高い（実測 中央 1.07）
#   ② 下限は買う点の**最小**＝順序統計量なので、目ごとの独立誤差がある限り
#      「予測の最小」より「確定の最小」は必ず小さい。傾斜配分が払戻をそろえる
#      ほど5点が接近し、最小はより深く食い込む
#
# 実測（vintage・2026-01〜08 の 14,748R）: **確定の下限 ÷ 予測の下限 = 0.78**
# （月次 0.766〜0.789）。実入稿 8/05〜8/25 の 7C/7S 424件では:
#
#   判定に使う数字        通す件数        通した中で実際に1.5倍以上  切った中で実は1.5倍以上
#   素の予測オッズ（旧）  215件(11.3/日)  **57.7%**                 9.1%
#   **c(p25)×予測（新）** 136件( 7.2/日)  **75.7%**                13.9%
#
# 新たに落ちる79件のうち、実際に1.5倍以上あったのは21件（27%）。
# **件数は減る（-4.2件/日）。ROI は動かない。**買うのは「最低1.5倍」という
# 看板の確からしさ（57.7→75.7%）で、ユーザー方針「最低限の希望オッズは1.5倍」
# （2026-08-21）を素直に実装し直したもの。
#
# 🔴 **`bet_detail.odds_low`（= min(板, c×予測)）で判定してはいけない。**
#    同じ314件で 通す97件・達成66.0%・取りこぼし26.3% と**両方悪い**。
#    板は買う帯で系統的に低い（中央 確定/板 0.86）ため判定材料として濁る。
#    表示は min 側・判定は c×予測、と**用途で数字を替える**
#    （[[keirin_odds_prediction_model_2026_08_11]] の原則）。
#
# ⚠️ 保守倍率はレース内で一律なので**配分の比率は変わらない**（水準だけの話）。
# ⚠️ 検証と再現: `docs/oddspred_gap_2026_08_26.md` /
#    `scripts/exp_oddspred_gap/`（01=実入稿・02=長い窓・03=不採用にした改善案5本）。
