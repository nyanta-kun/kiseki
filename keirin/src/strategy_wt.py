...
# 🔴 **カット後がこの点数以下なら、削らず総流しへ戻す**（2026-08-15・ユーザー判断）。
#
...
#
#     カット後  R数   割合  │ 的中(カット)  ROI  │ 的中(総流し)  ROI
#     1点     2355  43.7% │      35.0% 78.1% │      64.4% 75.6%
#     2点      924  17.2% │      46.5% 75.9% │      61.4% 77.2%
#     3点      286   5.3% │      58.0% 83.3% │      61.9% 71.6%
#     4点     1078  20.0% │      68.0% 84.3% │      68.1% 84.4%
#     5点      744  13.8% │      65.3% 79.7% │      65.3% 79.7%
#
#   ＝ **1点まで縮むのは母集団の 43.7%**で、そこでは的中が 64.4% → 35.0%
#   （−29.4pt）落ちるのに ROI は +2.5pt しか増えない。
#
#   方針別（レース単位 paired bootstrap 2,000回）:
#     現行（常にカット）      的中 49.0%  ROI 79.5%
#     1点なら総流しへ戻す     的中 61.8%  ROI 78.4%  差 −1.09pt [−4.11, +1.94]
#     **2点以下なら総流し**   的中 64.4%  ROI 78.6%  差 −0.85pt [−4.38, +2.46]
#     カットしない           的中 64.6%  ROI 78.0%  差 −1.45pt [−4.84, +2.05]
#
#   🔴 **ROI 差はどれも有意でない**（CI が 0 を跨ぐ）。落差カットは
#      **ROI を買えていないのに的中率を 13〜16pt 払っていた**。7C は的中体験の
#      商品（既存の高配当枠は 7H1/7T1 が担う）なので、商品の性格と逆を向いていた。
#
#   ⚠️ **見送りにはしない。** 1点まで縮むレースも総流しなら的中 64.4%・ROI 75.6% の
#      普通のレースで、母集団から落とす理由が無い（案として出たが不採用）。
#   ⚠️ 判定は `rank_7c_cut_legs_by_gap` の**中**で行う。呼び出し側（候補生成・
#      発走前判定・再構築）でそれぞれ下限を掛ける形にすると、1つ忘れた経路だけが
...
def rank_7c_cut_legs_by_gap(
    ...
    🔴 **削った結果が `legs_floor` 点以下なら、削らず総流しへ戻す**
       （2026-08-15・`RANK_7C_TRIO_LEGS_FLOOR` の定義部に実測表）。
       1点まで縮むのは母集団の43.7%で、そこは的中が 64.4% → 35.0% に落ちる一方
       ROI は有意に増えない。**ここで戻すのが単一正本**——呼び出し側で下限を
       掛ける形にすると、忘れた経路だけが1点買いを出し続ける。
    """
    if not legs:
        return []
    out = [legs[0]]
    for prev, cur in zip(legs, legs[1:]):
        if top3_probs.get(prev, 0.0) - top3_probs.get(cur, 0.0) >= gap_min:
            break
        out.append(cur)
    return list(legs) if len(out) <= legs_floor else out
