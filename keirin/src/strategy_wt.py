...
def rank_7m1_select_legs(
    ...
    """7M1の相手選択。**`ev` があれば EV 順の上位 n 点**、無ければ従来の下位3車。
    ...
    🔴 **対照2本が必要だった**。オッズだけでは現行より悪く、確率だけでは払戻中央が
       ...
       **積でないと払戻帯が保てない。**
    ⚠️ **点数を増やすと悪化する**（EV上位4/5 は 2倍+ が 1.51/1.36 へ低下）。
       EV 順なら **3点が最適**。単純な点数増しは別途測って不可と確定している
       （追加分の払戻中央 1.0倍台・半分がガミ・2倍超は 11〜15% しかない）。
    ...
    if ev is not None and all(f in ev for f in others):
        # EV 順（上位 n 点）。足切りも min_legs の戻しも掛けない（上記）。
        return sorted(others, key=lambda x: (-ev[x], x))[:n]
    ranked = sorted(others, key=lambda x: (-top3_probs.get(x, 0.0), x))
