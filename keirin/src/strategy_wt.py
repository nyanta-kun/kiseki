...
RANK_7M1_LEGS_MIN = 2
# 相手の並べ方（2026-08-21・ユーザー提案で "position" → "ev"）。
# 🔴 **`rank_rule_version("7M1")` はこの定数を拾う。** 値を変えると版が変わり、
#    `picks_history.rule_version` で新旧の行が自動的に分かれる。
#    "position"（下位3車固定）へ戻す場合もここだけを変えること。
# ⚠️ 実際に EV 順になるのは `rank_7m1_select_legs(ev=...)` に値が渡ったときだけ。
#    予測オッズが作れないレース（7車・9車以外＝実測3.7%）は従来規則へ落ちる。
RANK_7M1_LEG_ORDER = "ev"
# ═══════════════════════════════════════════════════════════════════════════
...
def rank_7m1_select_legs(
    ...
    """7M1の相手選択。**`ev` があれば EV 順の上位 n 点**、無ければ従来の下位車。
    ...
    returns 買う相手のリスト（pred_top3 の降順）。
    """
    if ev is not None and all(f in ev for f in others):
        maru = None
        if marks:
