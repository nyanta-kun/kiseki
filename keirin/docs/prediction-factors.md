...
> 🔴 **2026-09-20 監査（正本 `AUDIT_2026_09_20.md`）**: 以下のヘッダは現物と合わない。①特徴量は **70列**（46 ではない）②型ラボの売り物の p3 は **`lgbm_wt_eval`**（「評価専用」ではない）③「現行戦略 S1/SS+/SS/S」は全廃済みで現行は型ラボ④race_point 汚染は **2026-06-12 の 43 会場・210 行が未修復**（再取得の窓 6/18〜7/23 の外）。
...
> **【2026-08-18・7C のレース選別を4特徴の学習スコアへ替える案 → 不採用】** 7C の選別「モデル3着内率の**上位2車の合計** >= 1.44」を、目的関数である**二軸的中**を直接学習したロジスティック回帰（`sum2` / `gap23`(2位と3位の差) / `same_line`(軸2車が同ライン) / `p_ent`(レース内エントロピー)・入力は較正後の3着内率）へ置き換える案を検証した。**オフラインでは同一件数で二軸的中 +1.2〜2.4pt**（VAL/TEST/確認窓とも CI がゼロを跨がない）だったが、月次凍結 vintage の walk-forward A/B（14窓・本番の買い方の正本を経由）では **件数を揃えた時点で差が消えた**（3,502件へ外挿して Δ表示的中 ≒ −0.09pt）。🔴 **改善の正体は「良いレースを選んだ」ではなく「少なく賭けた」だった**（閾値 0.1099 で件数 −10.0%・表示的中 +0.50pt / 閾値 0.0130 で件数 −6.5%・+0.26pt(ns)）。下流の `RANK_7C_LEGS_MIN`（相手4点以上）と `lowpay_pattern` が**同じ情報を既に拾っている**ためで、`gap23` の大きいレース＝上位2車が抜けている＝相手が弱い＝低配当フィルタが落とす対象。**`RANK_7C_P3_SUM_MIN = 1.44` は据え置き**（本番コードは変更なし）。判定式は `src/race_gate_7c.py` に、係数の引き直しは `scripts/fit_race_gate_7c.py`、再検証ハーネスは `scripts/exp_gate7c_walkforward_ab.py` に残してある。詳細 `docs/analysis/56-race-selection-meta.md`。
...
| 日付 | 内容 |
|------|------|
| 2026-09-24 | **型ラボに検証ランク `L_lead`（逃げ先頭ライン）を追加・入稿しない**。特徴量・モデルは変更なし。条件は発走前の出走表（脚質・ライン・競走得点）と本番の3着内率の順位だけで決まる（`src/type_lab.py::line_lead_legs`）。得点1位の居ないラインの「逃げ先頭→番手」の決着は、市場の見込みの約1.2倍起きている（発生倍率 1.19 / 1.20・両窓）。`rule_version` からは外し、専用の `line_lead_rule_version()` を持たせた（既存の版を割らない）。詳細 `docs/type_lab/line_lead_2026_09_24.md` |
| 2026-09-23 | **軸信頼ゲート `AXIS_GATE_MIN` を p20 で引き直す（ユーザー指示）**: 特徴量・モデルは変更していない。9/22 に堅い3型がダッチへ移って母集団が変わり、旧の8値が **p19〜p44** とばらばらの分位を意味していた（`B_hit` p44 / `C_hit` p39 ↔ `F_hit` p19）。原典と同じ引き方（入稿ゲート通過後のプラン内分位・`scripts/exp_type_lab/axis_gate_redraw.py`）で p20 へ揃えた。実測（確認 / 探索）: 件/日 +1.50 / +1.48・**4万+/日 +0.08 [+0.04,+0.13] / +0.07 [+0.04,+0.10]**・3万+/日 +0.14 / +0.15・**表示的中は据え置き**（−0.08 / −0.03）・ROI 判別不能。🔴 p40〜ゲート無しまで掃引しても表示的中と ROI は 0.3pt 以内で動かず窓で符号も一致しない＝**精度のゲートではなく量のダイヤル**。発端はユーザー観察「買ってる側より買ってない側に的中・回収がある」で、その検証も同じ文書に記録した。詳細は `docs/type_lab/axis_gate_redraw_2026_09_23.md`。 |
| 2026-09-22 | **堅い3型（A/B/C）を「どの目が来ても5万円」のダッチへ替え、高額枠の `_big`（計画40万）を廃止（ユーザー決定）**: 特徴量・モデルは変更していない。発端はユーザー観察「当たらないし払戻が下がりすぎ。4万以上の払戻を取りたい」。実売で当てにいく商品の計画払戻中央が 30,100→24,440円（−19%）へ落ちており、9/08〜9/21 の5変更がすべて「払戻を下げて表示的中を買う」向きだった。本番コードのまま買い方だけ差し替えた台（確認 2026-01〜08-25 / 探索 2024-07〜2025-12）で、4万+ 1.23→1.81件/日・3万+ 2.16→3.00・件数 +1.35件/日・**ROI は判別不能（80〜83%のまま）**・表示的中 23.7→19.4%（Δ −4.30pt CI[−5.08,−3.54]・両窓一致）。`src/type_lab.py` の `HIT_BAND_TARGET=50_000` と `HIGHPAY_BIG_SLOTS=frozenset()`。同日に「全体の表示的中30%維持」の運用目標を下ろした。詳細は `docs/type_lab/product_redesign_2026_09_22.md` と `docs/RECOMMENDATION.md` 2.0節。 |
| 2026-09-16 | **段の販売を停止し 9/14 までの規則へ戻す（2026-09-15 実績とユーザー決定）**: 特徴量・モデルは変更していない。2026-09-15 の確定57Rで段の実売が 表示的中 21.1%・回収率 37.3%（荒れ 0/19）、同日の行を e018af8 の入稿スクリプトで dry-run した反実仮想が 31.2%・78.1% と明確に悪く、ユーザーが「明朝から昨日までの規則に戻す。段商品は検証を続ける」と決定。`src/type_lab.py` の `TIER_SELL_ENABLED=False` で入稿（販売）の判断（`sell_plans_for` の段分岐・段と `A_ana` の日次上限免除・一軸の判定・段の自信あり規則）を e018af8 と同じにした。段の行は検証用に生成・採点を継続（`rule_version` は変えない）。詳細は `docs/RECOMMENDATION.md` 冒頭の節。 |
| 2026-09-15 | **7車の段（固め・広め・手広く）の入稿ゲートを「当たったときの払戻の悪い側の見込み」へ変更（ユーザー決定）**: 特徴量・モデルは変更していない。当たった目の確定オッズが予測より系統的に低い（予測帯が低いほど大きい）ため、各点の 賭け金×予測オッズ×下振れ係数 の最小 < 15,000円なら見送る。係数は当たり目の 確定÷予測 の下側25%点（paper 2025・予測帯×種別×段・n>=30 のセルのみ・`src/type_lab.py` の `TIER_REALIZED_FACTOR`）。確認窓 2026-01-01〜08-04 の実測: 見送り 11.96件/日、残す段の表示的中 31.78→27.51%、1.5万円未満の当たり 2.42→0.68件/日、全体の表示的中 25.58→20.76%、回収率 86.1→88.2%。詳細は `docs/RECOMMENDATION.md` 冒頭の節。 |
| 2026-09-15 | **7車の段に「手広く」（`T_axis`・1着を固定して相手を広く買う）を追加**: 特徴量・モデルは変更していない。荒れの条件（軸信頼 ≤ 1.353）のうち1着率1位−2位 ≥ 0.30 のレースは、1着を1着率1位に固定し2・3着を確率順（合成2.2倍・3〜8点・払戻をそろえる配分）で組む。`RaceShape` に `win_top`/`win_gap` を追加。`T_axis` の行が入稿ゲートを通らなければ荒れで売る。販売面では「一軸」と書かない（ブランド「二軸探偵」）。詳細は `docs/RECOMMENDATION.md` 冒頭の表。 |
| 2026-09-14 | **7車の売り方を型A〜F別から「段」（固め／広め／荒れ）へ変更（2026-09-15 入稿から）**: 特徴量・モデルは変更していない。軸信頼 `axis_sum`（3着内率上位2車の合計）を探索窓の7車四分位で固定した境目（1.464 / 1.353）で3段に分け、固め=確率順に合成2.2倍まで3〜5点、広め=同3〜8点、荒れ=人気1位（予測オッズ盤の1着人気）のラインを1-2着から外した15〜600倍を計画10万円（いずれも払戻をそろえる配分）。型A∧`pw_ent`上位10%の `A_ana` は段より先に見て残す。9車は従来どおり。日次上限は7車の段と A_ana に掛けない。詳細は `docs/RECOMMENDATION.md` 冒頭の節と kiseki 側 memory `keirin-firm-upset-policy-2026-09-14`。 |
...
| 2026-08-04 | **落車・失格(DNF)の学習上の扱いを3段階で検証 → 対処なしで確定（否定結果）**: ユーザー指摘「落車で棚ぼた的に勝った選手が過大評価され、巻き込まれた側は過小評価される。レースごと無かったことにすべきでは」を A→B→C で検証した。**A（汚染の実規模）**: honest 22,834レースで消えた選手の発走前モデル順位別に勝者の順位分布を見ると、**得点1-2位が消えたレースだけ**勝者が1位評価である率が **45.5%→27.1%**（勝者順位平均 2.26→3.09）に落ちる＝**棚ぼたは実在する**。ただし該当は**全レースの2.2%**のみで、消えたのが3位以下ならクリーンなレースと変わらない（むしろ本命が勝ちやすい52.2%）。🔴 **2026-08-31 訂正: 「DNFの98%は `final_half` を持つ＝走り切った上での失格」は誤り。** 0.0 のプレースホルダを「値がある」と数えていた。実測（2024年以降・`cancel=0`・DNF 6,002行）は **`final_half` 非NULL 5,966 のうち 5,963 が 0.0 で、正の値はわずか3行**。`res_standing`/`res_back` も `int(bool(...))` なので DNF 行には 0 が入るだけ。実態は逆で、**DNF はほぼ全員が上がりタイムを持たない＝走り切っていない**。検算: `scripts/exp_accident_advancement/probe_dnf_history.py`。**B（母集団の扱い・3seed・test 22,841レース）**: 汚染レース除外 ΔAUC −0.00002 / 重み0.2 +0.00010 / 重み0.5 +0.00007 / DNFレース全除外 +0.00013 で**全て seed間ばらつき（±0.00014）と同オーダー**。軸1のみ全案で +0.15〜0.26pt と方向が一貫するが、paired bootstrap で有意なのは重み0.5の1案だけ（CI下限+0.0004）＝多重比較の範囲。**C-1（落車リスク特徴の追加）**: `dnf_rate_180`（選手の180日ローリング落車率）+ レース単位 max/mean を追加して ΔAUC +0.00008、NDCG@3・Spearman はむしろ悪化。**`dnf_rate_180` 単体の落車予測 AUC は 0.5459** ＝ほぼコイン投げで重要度も51特徴中24/34/39位。🔴 **2026-08-31 訂正: ここから「落車しやすい選手という性質は存在しない」と結論したのは誤り。窓が短すぎただけ。** 180日 ≒ 15〜25走では期待DNFが0.2件しかなく測定できない。生涯窓で測ると性質は**実在する**（100走以上の2,428人で過分散 chi2/dof=**2.369**、as-of 生涯落車率5分位 → そのレースのDNF率 0.696→0.767→1.025→1.315→**1.897%** と単調・z=+23.0）。**それでも不採用の結論は変わらない**——`pred_top3_pct` 十分位で統制すると3着内率の増分は **+0.06pt** で十分位ごとの符号もばらばら、軸2車の落車率5分位で二軸そろい率を見ても `axis_sum` 5帯すべて**逆符号**。＝性質は在るが目的変数に翻訳されない。**効かない理由**: 落車は「全行の1.17%・平均的な実力の選手に・予測不能に」起きる。**稀・無偏り・予測不能の3条件が揃うと母集団も特徴も効かない**。**残した指針**: ①母集団調整は**除外ではなく重み付け**で行う（重み案は予測平均0.4304と現行から動かず `RANK_7S_AXIS_SUM_MAX`/`*_ENTROPY_MAX` の絶対閾値を再較正せずに済む。過去の「DNFレース除外でROI 122.8%→87.9%」はハード除外による較正ずれ＋選択バイアスであり重みなら回避できる）②「1人でも落車したら除外」は粗すぎる（汚染は2.2%に限定され残る3%は無傷。旧検証はこの3%を巻き添えにしていた）。対比として同日採用した**隊列推定位置は ΔAUC +0.00303（20倍以上）**であり、**探すべきは稀な事象の補正ではなく、全レースに効いていて未表現の構造**。詳細は CLAUDE.md「検証済みの否定結果」。 |
...
| 2026-08-31 | **直接対決・ライン位置別・種別別・時間帯別の「自前履歴からの実績」4種20列を検証 → 4種とも不採用（否定結果）**: Web調査で外部実装が採用していた特徴を、スクレイピングせず `wt_entries` の全履歴（728,567行・2022-12〜）から point-in-time で作って A/B した。①**直接対決**（`h2h_*` に「相手ごとの勝率の平均/最小」を追加）②**ライン位置別実績**（今回のライン位置＝単騎/先頭/番手/3番手以降で過去どうだったか。`line_pos`「今回どこか」とは別物）③**レース種別別実績**（`rtc_*`。既存の `rt_is_*` フラグと紛らわしいので接頭辞を分けた）④**時間帯別実績**（モーニング/デイ/ナイター/ミッドナイト）。②③④は2段の階層縮小（定数事前確率 3/7 → 通算 → バケット・K=10）で平滑化し、母数 `n_B` と `lift_B`（バケット率−通算率）も渡す＝**生の率は一切使っていない**（母数の薄い率がノイズになる既知の問題への対処）。**結果（4窓×5seed・baseline 66特徴）**: 事前に固定した採用ライン（確認窓w3/w4 の両方で符号一致 かつ 平均 Δ1位3着内 ≥ +0.30pt・`exp_form_features_ab.py` と同一）を**どのアームも満たさない**。Δ1位3着内(確認平均) h2h +0.03 / linepos +0.07 / racetype +0.05 / tod +0.16 / 全部乗せ +0.23pt。窓あたり n≈6,000レースで3着内率79%の標準誤差は約 **0.53pt** なので、観測された差はすべて 1SE の内側。**`+h2h` の ΔAUC だけは4窓とも正**（+0.00026〜+0.00058）で 2026-07-28 の所見を再現したが、採用済み特徴の1/5以下（レース種別+ライン実力 +0.00302・隊列推定位置 +0.00303）。②③④の ΔAUC は確認窓でゼロ〜負。🔴 **壊れているのではなく冗長**: 確認窓97,810行の層別で全20列がきれいに単調（例 `h2h_win_rate` Q1 26.5%→Q5 60.0% / `rtc_top3_lift` 32.1%→53.0% / `lp_top3_lift` 29.2%→51.5%）＝計算は正しく周辺情報としては本物だが、`win_3m`/`top3_3m`/`win_6m`/`top3_6m`/**`venue_wr`（会場別の過去勝率＝同型が既にある）**/`race_point`/`rt_is_*`/`line_pos`/`is_line_leader` が同じ情報を先に持っている。決まり手の自前集計（2026-08-04・不採用）と同じ構図。🔴 **feature importance は全20列が上位に入る**（`lp_n` 2位・`rtc_top3_rate` 2位・`h2h_opp_mean_rate` 6位）が性能は上がらない＝**「分割重要度が高い」と「性能が上がる」は別**（2026-08-04 のレース単位集約と同じ教訓）。**point-in-time はテスト6本で固定**——未来非依存/自己排他/**過去依存**（T1が「そもそも履歴を見ていないバグ」で自明に通るのを防ぐ）/既存 `add_h2h_features_wt()` との数値一致/同日順序/初出走2,722人の母数ゼロ。⚠️ **ROI は測っていない**（①の 2026-07-28 の撤回理由はまさに ROI だったので、指標が通っていても ROI 検証が別途必要だった）。⚠️ 履歴が 2022-12 始まりなので「通算」は最大3年9ヶ月でベテランの真のキャリアは表現できていない。スクリプト: `scripts/exp_h2h_line_records_ab.py`（`--selftest` 付き・未コミット）。 |
