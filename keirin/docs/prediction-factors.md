...
| 日付 | 内容 |
|------|------|
| 2026-09-15 | **7車の段に「一軸」（`T_axis`）を追加**: 特徴量・モデルは変更していない。荒れの条件（軸信頼 ≤ 1.353）のうち1着率1位−2位 ≥ 0.30 のレースは、1着を1着率1位に固定し2・3着を確率順（合成2.2倍・3〜8点・払戻をそろえる配分）で組む。`RaceShape` に `win_top`/`win_gap` を追加。一軸の行が入稿ゲートを通らなければ荒れで売る。詳細は `docs/RECOMMENDATION.md` 冒頭の表。 |
