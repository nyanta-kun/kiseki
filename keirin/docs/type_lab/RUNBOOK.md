...
🔴🔴 **軸が1行でも食い違ったら、その範囲は1行も書かない**（スクリプトがそう動く）。
突き合わせられるのは `axis1`/`axis2` ＝ **並びの先頭2つだけ**で、3位以下は照合できない。
そして「先頭2つが合っていれば3位以下も合っている」は**成り立たない**:

    2025-07-15 実測 — 別ソースで復元した 47 行のうち
      完全一致 24 / **先頭2つだけ一致 23** / 不一致 0

3位以下こそが「順当（3着が指数3〜4位）」と「軸2+穴（指数5〜7位）」を分ける情報なので、
先頭2つだけ見て部分的に書くと土台が静かに壊れる。
食い違いが出たら**その期間の行はそのソースで作られていない**ということ。
範囲を狭めるか `--order-from-models` を使う。
...
| mode | 期間 | `p3_order` | 由来 |
|---|---|---|---|
| `paper` | 2026-01-01〜08-04 | ✅ 15,978行 | `/tmp/race_type_board.npz`（軸不一致 0） |
| `paper` | 2026-08-05〜08-26 | ✅ 2,121行 | 月次 vintage（22日とも軸不一致 0） |
| `paper` | **2025 全期間** | ❌ 26,189行 未充填 | 板と食い違う＝月次 vintage 由来。要 `--order-from-models` |
| `paper9` | 2025 | ❌ 未充填 | 9車。板は7車のみなので別途 |
| `live` | 2026-08-27〜 | ✅ | 生成時に自動で入る（バックフィル不要） |
