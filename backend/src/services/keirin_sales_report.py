...
#: 表示的中率の**運用フロア**（%）。2026-09-14 にユーザー判断で 22% → 30% へ変更。
#: 全部を荒れ前提にしない商品なので、一撃の回収だけでなくコンスタントな的中が要る。
#: 高額枠（表示的中 約5%）を抱えるので本線（`*_hit`）は約35〜40% が必要になる。
#: 🔴 値を変えるなら `keirin/docs/PREREG_SALES_ALLOCATION_2026_09_14.md` と同時に変えること。
HIT_RATE_FLOOR_PCT = 30.0
