"""3着内率の後段較正とレース信頼度（**正本**・2026-08-25 に keirin から移設）。

## なぜ kiseki 側が正本なのか

`keirin_marquee.py` / `keirin_cup_grade.py` と**同じ向き**に揃えた。kiseki の
backend イメージは `./backend` だけをビルドコンテキストにするため、
**コンテナの中に `keirin/` は存在しない**。keirin 側を正本にすると Web から
参照できず、実際 2026-08-25 のデプロイで backend コンテナが起動に失敗した。

keirin 側（`keirin/src/p3_calibration.py`）はこのファイルを**読み込んで束縛する**。

🔴 **標準ライブラリ以外を import しないこと。** keirin は自分の venv
   （FastAPI も SQLAlchemy も無い）からこのファイルを直接読む。
