...
# 🔴 **平均払戻が安いレースの判定は API から消した**（2026-08-26）。
#    以前は `mean_payout` / `cheap_mean_payout` を返し、レビュー画面が一括取消の
#    候補を作っていたが、判定は**入稿データを作る時点の自動ゲート**へ一本化した
#    （`keirin/src/stake_allocation.py::MIN_MEAN_PAYOUT` /
#     `keirin/scripts/netkeirin_submit_wt.py::_mean_payout_too_low`）。
