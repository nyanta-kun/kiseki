...
_PAPER_RANK_LABELS: dict[str, str] = {
    ...
    # 手動入稿（_MANUAL_RANK_KEYS）は軸2車を選ぶUIのため**対象外**（7H1 と同じ理由）。
    "RANK_7H2": "7H2",
    # RANK_7H3: 2026-08-12 新設の**穴推奨**（本命連対どまり型・三連単の高配当）。
    #    「指数上位2車は3着以内に来るが、勝ち切るのは別の車」という決着だけを買う
    #    （軸2車を2着・3着に置き、1着は相手に任せる三連単フォーメーション）。
    #    母集団は **看板でも準決勝でもない7車立て** ∧ 軸積 >= 0.70 に限定される
    #    ので、看板+準決勝を対象とする的中率商品とは排他。7C（全7車）とだけ重なる。
    # ⚠️ 三連単の単一券種。買い目文字列が "相手-軸1-軸2" の順序つきなので、
