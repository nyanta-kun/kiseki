...
SIGNAL_HEIHACHI (平八、2026-09-06追加 [[jra_heihachi_badge]]):
  平地OP特別以上 ∧ composite順位≤3 ∧ 単勝オッズ15〜40倍 ∧ 複勝確率≥0.40。

  🔴 **これは「回収率」ではなく「3着内率の分離」を目的とするタグ**（「穴」と同じ性格）。
  2026-09-06 の頑健性評価でそう位置づけ直した
  （docs/jra_heihachi_threshold_sweep_2026_09_06.md）:

  - VAL 2窓(25H2/26H1)のスイープには選別力が無かった。1,392通りの候補のうち
    **47%** が「2窓とも複勝ROI>1」を通ってしまう（VAL が単に good period だった）
  - TEST(2026Q3)を開けたら**全候補が複勝ROI 1 を割った**。本条件は n=9・複0.19、
    旧条件(10-40倍/30%)でも n=64・複0.77、最も広い候補でも n=262・複0.98
  - 一方 **3着内率の分離は TEST でも残る**（29.8% vs ベースライン22.8%）

  🔴🔴 **v28 本番モデルの refit 期間は 2023-05-06〜2026-06-28**
  （models/v28_iswin_calib_metrics.json の `refit_period`）。`inference_v28.py` は
  この refit モデルでバックフィルするので、**2026-06-28 以前の指数値は全部 in-sample**。
  真に out-of-sample なのは 2026-06-29 以降だけ。

    in-sample  2025-07〜2026-06 : n=26  3着内 53.8%（ベース21.5%）
    **OOS**    2026-06-29〜09-06 : n= 9  3着内 11.1%（ベース22.8%）

  つまり **本タグの有効性は out-of-sample では確認できていない**（n=9 なので
  否定もできない）。in-sample の 53.8% は当てはまりの良さであって将来の性能ではない。
  なお指数そのものは OOS でも効いている（指数1位の3着内率は OOS で 56.1%）。
  崩れているのは「オッズ帯で切った妙味」の部分。

  由来: note の予想家「平八」の馬印 19,429件（198開催日・4,964レース・
  2024-01〜2026-08）を OCR 復元して逆解析した（data/heihachi/、scripts/heihachi/）。
  ◎は「人気5位以下 × JV-Next DM対戦型上位」で composite と強く重なり、
  対照実験では印を使っても使わなくても成績がほぼ同じ＝**印に独立情報は無い**。
  そこで印は使わず、平八が示していた「穴サイドで3着内率が高いゾーン」を
  自前の指数だけで再現したのが本タグ。`jvan_battle_dm`/`jvan_time_dm` は既に
  v28 の38列特徴に入っているので、DM で追加フィルタしても改善しない（実測で悪化）。

  既定値は 2026-09-06 にユーザー指示で 10-40倍/30% → 15-40倍/40% へ変更した。
  絞ったぶん頻度は約0.36件/開催日まで落ちる。しきい値は推奨ページのスライダーで
  変更でき、レース詳細のバッジ表示もそれに追随する（判定の真実源は
  frontend/src/lib/heihachi.ts）。しきい値を選び直すなら TEST を焼かないよう
  2026Q4 を独立窓として使うこと。
"""
...
HEIHACHI_MIN_ODDS = 15.0
HEIHACHI_MAX_ODDS = 40.0
HEIHACHI_MIN_PLACE_PROB = 0.40
