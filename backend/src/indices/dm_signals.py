...
SIGNAL_HEIHACHI (平八、2026-09-06追加 [[jra_heihachi_badge]]):
  ...
  実測 (2023-01〜2026-09, n=128, 約0.36件/開催日):
    3着内率 32.8% / 複勝ROI 1.138 / 単勝ROI 1.223
    年別 複勝ROI 2023:0.83(n=31) / 2024:0.96(n=42) / 2025:1.48(n=33) / 2026:1.41(n=22)

  🔴 既定値は 2026-09-06 にユーザー指示で 10-40倍/複勝率30% から
     15-40倍/複勝率40% へ変更した。**絞ったぶん年あたり n が22〜42頭まで落ち、
     年別の複勝ROIが2023:0.83・2024:0.96 と2年ぶん1を割る。**
     旧既定(10-40倍/30%)は n=1,017・複勝ROI 1.147 で4年とも1超だった。
     全期間の平均が1を超えているのは2025-2026の2年が牽引しているためで、
     この既定値は年次の頑健性という点では旧既定より弱い。
     しきい値は推奨ページのスライダーで動かせるので、緩める判断もそこで行う。

  v28 は2023-2025にバックフィルされており学習期間と重なるため、完全な OOS は
  2026のみである点にも注意。
"""
