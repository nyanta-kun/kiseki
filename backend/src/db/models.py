...
class Horse(Base):
    ...
    jravan_code: Mapped[str | None] = mapped_column(String(20), unique=True, index=True, comment="JRA-VAN馬コード（10桁）")
...
class Jockey(Base):
    ...
    jravan_code: Mapped[str | None] = mapped_column(String(10), unique=True, index=True, comment="JRA-VAN騎手コード（5桁）")
...
class Trainer(Base):
    ...
    jravan_code: Mapped[str | None] = mapped_column(String(10), unique=True, index=True, comment="JRA-VAN調教師コード（5桁）")
...
class Race(Base):
    ...
    course: Mapped[str] = mapped_column(String(10), nullable=False, comment="競馬場コード（コード表2001: 01=札幌,05=東京,06=中山等）")
    ...
    registered_count: Mapped[int | None] = mapped_column(Integer, comment="登録頭数（取消前の出走予定頭数）")
    finishers_count: Mapped[int | None] = mapped_column(Integer, comment="入線頭数（出走頭数から競走中止を除く）")
    race_type_code: Mapped[str | None] = mapped_column(String(2), comment="競走種別コード（コード表2005: 11=2歳,12=3歳,13=3歳以上,14=4歳以上,20=障害等）")
    weight_type_code: Mapped[str | None] = mapped_column(String(1), comment="重量種別コード（コード表2008: 1=馬齢,2=定量,3=別定,4=ハンデ）")
    ...
    first_3f: Mapped[Decimal | None] = mapped_column(Numeric(4, 1), comment="前3ハロン通過タイム（秒、SST形式変換後）")
    last_3f_race: Mapped[Decimal | None] = mapped_column(Numeric(4, 1), comment="レース後3ハロンタイム（秒、SEのlast_3fは個馬別）")
    lap_times: Mapped[str | None] = mapped_column(String(75), comment="ラップタイム生データ（25F分×3バイトSST形式、平地のみ）")
    record_update_type: Mapped[str | None] = mapped_column(String(1), comment="レコード更新区分（0:初期値,1:コース基準更新,2:コースレコード更新）")
    prev_distance: Mapped[int | None] = mapped_column(Integer, comment="変更前距離（距離変更時のみ設定、単位m）")
    prev_track_code: Mapped[str | None] = mapped_column(String(2), comment="変更前トラックコード（トラック変更時のみ）")
    prev_grade_code: Mapped[str | None] = mapped_column(String(1), comment="変更前グレードコード（グレード変更時のみ）")
    prev_post_time: Mapped[str | None] = mapped_column(String(4), comment="変更前発走時刻（hhmm形式、発走時刻変更時のみ）")
    post_time: Mapped[str | None] = mapped_column(String(4), comment="発走時刻（hhmm形式、例: '1025' = 10:25）")
...
class RaceEntry(Base):
    ...
    horse_weight: Mapped[int | None] = mapped_column(Integer, comment="馬体重（kg、計不明時はNone）")
    ...
    east_west_code: Mapped[str | None] = mapped_column(String(1), comment="東西所属コード（1:東,2:西,3:地方,4:海外）")
    prev_weight_carried: Mapped[Decimal | None] = mapped_column(Numeric(4, 1), comment="変更前負担重量（kg、斤量変更時のみ設定）")
    blinker: Mapped[bool | None] = mapped_column(Boolean, comment="ブリンカー使用フラグ（True:使用,False:未使用）")
    prev_jockey_code: Mapped[str | None] = mapped_column(String(5), comment="変更前騎手コード（騎手変更時のみ設定）")
    jockey_apprentice_code: Mapped[str | None] = mapped_column(String(1), comment="騎手見習コード（0:なし,1=5kg減,2=3kg減,3=1kg減）")
...
class RaceResult(Base):
    ...
    last_3f: Mapped[Decimal | None] = mapped_column(Numeric(3, 1), comment="後3ハロンタイム（秒、個馬別）")
    abnormality_code: Mapped[int | None] = mapped_column(Integer, default=0, comment="異常区分（1:出走取消,2:発走除外,3:競走中止,4:失格,5:降着）")
    arrival_position: Mapped[int | None] = mapped_column(Integer, comment="入線順位（失格・降着確定前の順位）")
    dead_heat: Mapped[bool | None] = mapped_column(Boolean, comment="同着フラグ（True:同着あり）")
    margin_code: Mapped[str | None] = mapped_column(String(3), comment="着差コード（コード表2102: '000'=ハナ差等）")
    win_odds: Mapped[Decimal | None] = mapped_column(Numeric(5, 1), comment="確定単勝オッズ（倍、期待値計算の基準値）")
    win_popularity: Mapped[int | None] = mapped_column(Integer, comment="単勝人気順位（1位が最低オッズ）")
    ...
    time_diff: Mapped[Decimal | None] = mapped_column(Numeric(4, 1), comment="1着とのタイム差（秒、マイナスは1着馬が速い）")
    running_style: Mapped[str | None] = mapped_column(String(1), comment="JRA判定脚質（1:逃,2:先,3:差,4:追）")
...
class TrackCondition(Base):
    ...
    distance: Mapped[int] = mapped_column(Integer, comment="距離（m）")
    condition: Mapped[str | None] = mapped_column(String(5), comment="馬場状態（良/稍/重/不）")
    bias_value: Mapped[Decimal | None] = mapped_column(Numeric(5, 2), comment="馬場差値（基準タイムとの差、秒）")
...
class CalculatedIndex(Base):
    ...
    race_id: Mapped[int] = mapped_column(ForeignKey(f"{SCHEMA}.races.id"), index=True)
    horse_id: Mapped[int] = mapped_column(ForeignKey(f"{SCHEMA}.horses.id"))
    version: Mapped[int] = mapped_column(Integer, default=1, comment="算出バージョン（指数ロジック変更時にインクリメント）")
    # スピード指数系
    speed_index: Mapped[Decimal | None] = mapped_column(Numeric(5, 1), comment="スピード指数（基準50、標準偏差10）")
    adjusted_speed_index: Mapped[Decimal | None] = mapped_column(Numeric(5, 1), comment="補正スピード指数（斤量・馬場差補正後）")
    ...
    position_advantage: Mapped[Decimal | None] = mapped_column(Numeric(5, 1), comment="枠順有利不利指数")
    # 能力指数系
    ...
    # 総合
    composite_index: Mapped[Decimal | None] = mapped_column(Numeric(5, 1), comment="総合指数（全指数の加重平均）")
    ...
    calculated_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), comment="指数算出日時")
...
class EntryChange(Base):
    ...
    change_type: Mapped[str] = mapped_column(String(20), comment="変更種別（scratch/jockey_change/weight_change等）")
...
class OddsHistory(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    race_id: Mapped[int] = mapped_column(ForeignKey(f"{SCHEMA}.races.id"), index=True)
    bet_type: Mapped[str] = mapped_column(String(20), comment="賭式（win/place/quinella/trio/trifecta等）")
    combination: Mapped[str] = mapped_column(String(50), comment="馬番組み合わせ（単: '3', 連: '3-7', 3連: '3-7-12'）")
    odds: Mapped[Decimal | None] = mapped_column(Numeric(10, 1), comment="オッズ（倍）")
    fetched_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), comment="取得日時")
