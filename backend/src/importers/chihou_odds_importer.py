...
O1-O8レコードをパースしてchihou.odds_historyテーブルへ格納する。
各券種のオッズは固定長テキスト内に連続して格納されているため、
種別ごとにスライス幅を変えて展開する。
umaconn_race_id でレースを紐付ける点が keiba スキーマ版と異なる。
"""
...
from ..db.chihou_models import ChihouOddsHistory, ChihouRace
...
class ChihouOddsImporter:
    ...
    keiba スキーマ版 OddsImporter と同一ロジックで、
    Race の検索に umaconn_race_id を使用する点のみ異なる。
    """
    ...
    async def _get_race_id(self, umaconn_race_id: str) -> int | None:
        """umaconn_race_id からDBのChihouRace.idを取得する。

        Args:
            umaconn_race_id: UmaConn レースID（パーサー出力の jravan_race_id フィールドの値）

        Returns:
            chihou.races.id または None
        """
        result = await self.db.execute(
            select(ChihouRace.id).where(ChihouRace.umaconn_race_id == umaconn_race_id)
        )
