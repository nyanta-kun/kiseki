...
def _build_calculator(
    ...
    _estimate_speed_score_sync が DB を呼ばないよう None を返すようにモック。
    """
    db = AsyncMock()
...
class TestCalculateSingleHorse:
    ...
    async def test_first_run_returns_default(self) -> None:
    ...
    async def test_ideal_interval_high_score(self) -> None:
    ...
    async def test_extreme_short_interval_low_score(self) -> None:
    ...
    async def test_long_rest_low_score(self) -> None:
    ...
    async def test_first_place_bonus_applied(self) -> None:
        """前走1着ボーナス（+20）が加算される"""
        target_race = _make_mock_race(1, "20260322")
    ...
    async def test_unknown_horse_returns_default(self) -> None:
        ...
        target_race = _make_mock_race(1, "20260322")
        calc = _build_calculator(target_race, [])
        result = await calc.calculate(race_id=1, horse_id=9999)
        assert result == DEFAULT_SCORE
...
class TestCalculateBatch:
    ...
    async def test_batch_empty_race_id_returns_empty(self) -> None:
        """存在しない race_id → 空dict"""
        db = AsyncMock()
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = None
    ...
    async def test_batch_no_entries_returns_empty(self) -> None:
        """エントリなしのレース → 空dict"""
        db = AsyncMock()
        target_race = _make_mock_race(1, "20260322")
        mock_race_result = MagicMock()
    ...
    async def test_batch_first_run_returns_default(self) -> None:
    ...
    async def test_batch_first_place_bonus(self) -> None:
