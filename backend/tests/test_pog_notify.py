...
def test_通知対象は指名がある最新年度に限る():
    """🔴 POG の年度は**ダービー週**で入れ替わる（2026-09-10 ユーザー決定）。

    実測（sekito の指名 created_at）:

        2025年度の指名  2025-05-28 〜        ← ダービー週
        2026年度の指名  2026-05-28 〜 05-31  ← ダービー週

    グループ行は前もって作られることがある（2026年度は 5/4 作成・指名は 5/28）
    ので、**行の有無ではなく指名の有無**で最新年度を決める。ここを行の有無に
    変えると、ドラフト前から新年度へ切り替わって旧年度の通知が止まる。

    ⚠️ 2025 と 2026 は同じ Discord チャンネルへ送る設定。全グループへ送ると
       去年の指名馬の通知が現役の年度に混ざる。
    """
    from src.services.pog_notify import _GROUPS_SQL

    sql = str(_GROUPS_SQL)
    assert "max(g.year)" in sql, "最新年度に絞っていない"
    assert "pog_picks" in sql and "pick_order > 0" in sql, (
        "指名の有無で判定していない（グループ行の有無だとドラフト前に切り替わる）"
    )
