...
def _make_calculator(
    ...
    mock_race_result.scalar_one_or_none.return_value = default_race
    mock_entries_result = MagicMock()
    mock_entries_result.scalars.return_value.all.return_value = default_entries
...
class TestCalculate:
    ...
    async def test_race_not_found_returns_mean(self) -> None:
        db = AsyncMock()
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = None
        db.execute.return_value = mock_result
        calc = Last3FIndexCalculator(db)
        result = await calc.calculate(99, 1)
        assert result == SPEED_INDEX_MEAN

    async def test_no_valid_last3f_returns_mean(self) -> None:
        ...
        # past results クエリ: 空のリスト
        past_rows_result = MagicMock()
        past_rows_result.all.return_value = []

        db.execute.side_effect = [mock_race_result, past_rows_result]
...
class TestCalculateBatch:
    ...
    async def test_race_not_found_returns_empty(self) -> None:
        ...
        db.execute.return_value = mock_result
        calc = Last3FIndexCalculator(db)
        result = await calc.calculate_batch(99)
        assert result == {}
...
# ---------------------------------------------------------------------------
# _get_field_stats_batch のテスト
# ---------------------------------------------------------------------------


class TestFieldStats:
    """フィールド統計取得のテスト（_get_field_stats_batch を使用）。"""

    async def test_insufficient_sample_returns_none(self) -> None:
        ...
        db.execute.return_value = mock_result
        calc = Last3FIndexCalculator(db)
        result = await calc._get_field_stats_batch({10})
        assert result[10] is None

    async def test_sufficient_sample_returns_stats(self) -> None:
        """サンプル数が十分なら (mean, std) を返す。"""
        db = AsyncMock()
        vals = [34.0, 35.0, 35.5, 36.0, 36.5]
        mock_result = MagicMock()
        mock_result.all.return_value = [(10, Decimal(str(v))) for v in vals]
        db.execute.return_value = mock_result
        calc = Last3FIndexCalculator(db)
        result = await calc._get_field_stats_batch({10})
    ...
    async def test_cache_prevents_duplicate_query(self) -> None:
        ...
        db.execute.return_value = mock_result
        calc = Last3FIndexCalculator(db)
        await calc._get_field_stats_batch({10})
        call_count_first = db.execute.call_count

        await calc._get_field_stats_batch({10})  # 2回目
        assert db.execute.call_count == call_count_first  # クエリ増えない

    async def test_zero_std_returns_none(self) -> None:
        """全馬が同一 last_3f（std≈0）なら None。"""
        db = AsyncMock()
        mock_result = MagicMock()
        mock_result.all.return_value = [(10, Decimal("35.0")) for _ in range(MIN_FIELD_SAMPLE)]
        db.execute.return_value = mock_result
        calc = Last3FIndexCalculator(db)
        result = await calc._get_field_stats_batch({10})
        assert result[10] is None
