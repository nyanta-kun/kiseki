...
# ---------------------------------------------------------------------------
# 欠車返還（2026-09-20 監査 item2）
# ---------------------------------------------------------------------------
#
# 買い目を組んだ後に出走取消が出ると、その車番を含む leg は**構造的に当たらない**
# （出走していない車番が3着以内に入ることはない）ので、本来は返還される。
# 旧実装は欠車の概念を持たず、返還ぶんまで投資額へ全額計上していた
# （実測 2026-08-27〜09-19 の live で 9行・6レース・25,400円）。
#
# 🔴 **動くのは投資額（分母）だけ。** 的中判定・払戻は元から無関係なので変えない。
# 🔴 **`valid_cars` を渡さなければ従来どおり**（後方互換）。出走表が引けないときに
#    全部を返還にしてしまうより、何もしない方が安全。

def test_欠車を含む買い目は投資額から外して返還に計上する():
    bet = _bet(("1=2=4", 3000), ("1=2=5", 2000))      # 4番が欠車
    res = settle(bet, [(1, 1), (2, 2), (3, 5)], {"1=2=5": 800},
                 valid_cars={1, 2, 3, 5, 6, 7})
    assert res.bet == 2000            # 欠車を含む 3,000円は賭かっていない
    assert res.void_refund == 3000
    assert res.hit is True            # 払戻の判定は不変
    assert res.payout == 16000        # 2,000円 × 8.0倍


def test_欠車を含む買い目でも的中判定は変わらない():
    """返還した leg は元から当たりえないので、hit/payout は据え置き。"""
    bet = _bet(("1=2=4", 3000), ("1=2=5", 2000))
    fin, pm = [(1, 1), (2, 2), (3, 5)], {"1=2=5": 800}
    with_void = settle(bet, fin, pm, valid_cars={1, 2, 3, 5, 6, 7})
    without = settle(bet, fin, pm)
    assert (with_void.hit, with_void.payout) == (without.hit, without.payout)
    assert without.void_refund == 0    # 渡さなければ従来どおり
    assert without.bet == 5000


def test_出走表を渡さなければ欠車判定をしない():
    """🔴 出走表が引けない＝「全部欠車」ではない。fail-safe は「何もしない」側。"""
    res = settle(_bet(("1=2=4", 3000)), [(1, 1), (2, 2), (3, 5)], {})
    assert res.bet == 3000 and res.void_refund == 0


def test_全部が欠車を含む場合は投資額がゼロになる():
    res = settle(_bet(("1=2=4", 3000)), [(1, 1), (2, 2), (3, 5)], {},
                 valid_cars={1, 2, 3, 5, 6, 7})
    assert res.bet == 0 and res.void_refund == 3000
    assert res.n_combos == 0


def test_買い目の表示は返還ぶんも含めて出す():
    """🔴 `pred_combo` は「実際に買った全部」。返還で消すと何を買ったか分からない。"""
    res = settle(_bet(("1=2=4", 3000), ("1=2=5", 2000)), None, None,
                 valid_cars={1, 2, 3, 5, 6, 7})
    assert res.pred_combo == "1=2=4 1=2=5"
