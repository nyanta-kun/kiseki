...
from src.indices.speed import LOOKBACK_RACES, WEIGHT_DECAY, SpeedIndexCalculator
...
class TestSingleRaceSpeedScore:
    ...
    def test_no_std_time_returns_none(self) -> None:
        ...
        result = calc._single_race_speed_score(
            _make_result(93.0), _make_race(), _make_entry()
        )
        assert result is None
...
class TestCalculateBatch:
    ...
    def _build_calc_with_past_data(
        ...
        def mock_batch(horse_ids_arg, before_date, exclude_race_id):
            result = {}
