...
from src.indices.meet_bias import MeetBias
...
def _make_calculator(
    ...
    mock_entries_result.scalars.return_value.all.return_value = entries or []
...
class TestCalculateBatch:
    ...
    def _build_calc(
        ...
        async def mock_batch(horse_ids_arg: list[int], before_date: str, exclude_race_id: int) -> dict:
        ...
        calc._get_avg_last3f = AsyncMock(return_value=35.0)

        # _get_course_features と _meet_bias.get_bias をモック（DB追加アクセスを防ぐ）
    ...
    async def test_no_entries_returns_empty(self) -> None:
        ...
        db = AsyncMock()
        mock_race_result = MagicMock()
        mock_race_result.scalar_one_or_none.return_value = _make_race_obj()
        mock_entries_result = MagicMock()
    ...
    async def test_unknown_runner_type_returns_50(self) -> None:
    ...
    async def test_escape_slow_pace_high_score(self) -> None:
    ...
    async def test_closer_fast_pace_high_score(self) -> None:
        """追込馬+ハイペース → 80。"""
        # 逃げ馬2頭 → fast ペース
        horse_configs = [
            {"horse_id": 1, "passing_4": 1, "head_count": 10},  # escape
    ...
    async def test_escape_fast_pace_low_score(self) -> None:
        """逃げ馬+ハイペース → 45。"""
        # 逃げ馬3頭 → fast ペース
        horse_configs = [
            {"horse_id": 1, "passing_4": 1, "head_count": 10},  # escape
    ...
    async def test_closer_slow_pace_low_score(self) -> None:
        """追込馬+スロー展開 → 45。"""
        # 逃げ馬なし → slow ペース
        horse_configs = [
    ...
    async def test_last3f_bonus_applied_when_faster(self) -> None:
    ...
    async def test_last3f_bonus_not_applied_when_slower(self) -> None:
    ...
    async def test_score_clipped_at_0(self) -> None:
        """スコア下限0にクリップされる。"""
        horse_configs = [
