...
sys.path.insert(
    ...
    else str(__import__("pathlib").Path(__file__).resolve().parents[1]),
)
...
with Session(engine) as db:
    ...
    print(
        f"{'コース':>4} {'距離':>5} {'芝ダ':>4} {'馬場':>4} {'件数':>5} {'平均(秒)':>9} {'標準偏差':>8}"
    )
    print("-" * 50)
    for r in rows:
        print(
            f"{r.course:>4} {r.distance:>5} {r.surface:>4} {str(r.condition or '-'):>4} {r.cnt:>5} {r.avg_time:>9} {r.std_time:>8}"
        )
    ...
    target_race = (
        db.query(Race)
        .filter(Race.date == "20260322", Race.course == "06", Race.race_number == 4)
        .first()
    )

    if target_race:
        print(
            f"対象レース: {target_race.course} R{target_race.race_number} {target_race.race_name} {target_race.distance}m {target_race.surface}"
        )
        ...
        for entry in sorted(entries, key=lambda e: e.horse_number):
            ...
            print(
                f"{entry.horse_number:>4} {(horse.name if horse else '?'):>20} {idx:>10.1f}{marker}"
            )

    # 3. 過去実績馬の詳細確認
    print("\n=== 過去実績馬の詳細（タガノバルコス・コスモゴレアドール） ===")
    results_detail = db.execute(
        text("""
    ...
    for r in results_detail:
        print(
            f"  {r.date} {r.course} {r.distance}m {r.surface}{r.condition} {r.finish_position}着 {r.finish_time}秒 {r.weight_carried}kg"
        )
