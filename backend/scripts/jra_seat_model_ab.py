"""中央競馬（JRA） Phase 1 — レース側モデル vs 学習不要ベースライン。
...
腕（事前登録 §5）:
  base_a   期待空席数（学習なし）      ← 実質のベースライン。Phase 0 で lift 1.502
  base_b0  頭数のみ（学習なし）
...
使い方:
    cd backend
    .venv/bin/python scripts/jra_seat_model_ab.py \
        --cache /tmp/jra_seat_cache.json --pop-rank-min 8 \
        --train-end 20260630 --out ../docs/model_verification/jra_seat_phase1.json
"""
...
from scripts.jra_upset_seat_label import (  # noqa: E402
    CLOSED_RACE_TOP3_SHARE,
    MIN_HEAD_COUNT,
    annotate,
    head_bin,
)
import scripts.jra_upset_seat_label as seat  # noqa: E402
...
# JRA は芝・ダート・障害が混在するので、定数になってよい列は無い。
# 定数が出たら取得漏れ（2026-09-02 に surface / race_type_code /
...
def build_features(races: list[dict]) -> None:
    ...
    for r in races:
        # JRA は芝・ダート・障害が混在する（地方と違い is_turf は定数にならない）
        r["is_turf"] = 1 if str(r.get("surface") or "").startswith("芝") else 0
...
def main() -> None:
    ...
    ap.add_argument("--train-end", default="20260630",
                    help="学習窓の終端（含む）。JRA は 936R / 648R と薄い（事前登録 §13）")
    ap.add_argument("--out")
    args = ap.parse_args()
    ...
    print(f"=== JRA Phase 1  レース側モデル vs ベースライン（pop_rank>={args.pop_rank_min}）===")
    if const:
