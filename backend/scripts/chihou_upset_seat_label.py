"""地方競馬 Phase 0 — 席数ラベルの実態集計と、学習不要ベースラインの測定。
...
🔴 **base_c を必ず並べること。** 現行の `closed_race` ゲートは既に粗い
「席が空くか」モデルで、前向き実測で 推奨あり 70.4% vs 棄権 46.1%（lift 1.53）を
出している（計画 §11.2）。これを無視した「勝った」は意味がない。

使い方:
    cd backend
    .venv/bin/python scripts/chihou_upset_seat_label.py \
        --start 20260407 --end 20260630 --out ../docs/model_verification/chihou_seat_phase0.json
"""
...
# 現行 chihou_is_place_pick のゲート定数（src/indices/buy_signal.py と揃える）
...
JOIN chihou.races r ON r.id = e.race_id
...
def stratified_lift(races: list[dict], defn: str, score_key: str,
    ...
    🔴 Phase 0 stop rule #2 の直接検定。地方は E>=1 の base rate が頭数で
    9.7%〜75.1% と激しく動くため（VAL 5,286R 実測）、頭数をまたいだ lift は
    「頭数の言い換え」でも簡単に 1.4 程度が出てしまう。**頭数を固定しても
...
def base_c_gate(races: list[dict], defn: str) -> dict[str, Any]:
    """現行 chihou_is_place_pick のレース側ゲート（closed_race / small_field）。
...
def main() -> None:
    ...
    ap.add_argument("--lead-minutes", type=int, default=6,
                    help="発走何分前のオッズを使うか（既定6＝前向き記録と同じ）")
    ...
    if args.start < "20260407":
        ap.error("地方の odds_history は 2026-04-07 開始。それ以前は発走前オッズが無い")
    ...
    print(f"=== 地方 席数ラベル Phase 0  {args.start}〜{args.end}"
