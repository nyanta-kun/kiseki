...
_EXPECTED_JOBS: tuple[tuple[str, frozenset[int] | None, str], ...] = (
    ...
    # 🔴 POG の結果通知。10 分ごとに走るので「今日 1 回も動いていない」は明確な異常。
    #    移設元は 2026-05-02 から 2025年度グループへ通知できなくなっていたのに
    #    誰も気づかなかった（77 件が届いていない）。同じ穴を開けないため監視に載せる。
    ("notify_pog_results", None, "POG 結果通知（10分ごと 10-23時）"),
    # 穴ぐさとパドックは土日月のみ。
