...
INDEX_COLS = [
    "composite_index", "speed_index", "last_3f_index", "course_aptitude",
    "position_advantage", "jockey_index", "pace_index", "rotation_index",
    "pedigree_index", "training_index",
]
INDEX_LABELS = {
    "composite_index": "総合", "speed_index": "スピード", "last_3f_index": "後3F",
    "course_aptitude": "コース適性", "position_advantage": "枠順", "jockey_index": "騎手",
    "pace_index": "展開", "rotation_index": "ローテ", "pedigree_index": "血統",
    "training_index": "調教",
}
...
def filter_valid(df: pd.DataFrame, min_runners: int = 4) -> pd.DataFrame:
    ...
    bad = df[(df["abnormality_code"] > 0) | df["finish_position"].isna() |
             df["composite_index"].isna()]["race_id"].unique()
...
def section_ev(df: pd.DataFrame) -> None:
    ...
    s = roi_stats(top1)
    print(f"\n▼ 全体（指数top1 単勝）")
    print(f"  賭け数: {s['bets']:,}, 的中: {s['wins']}, 勝率: {s['win_rate']}%, ROI: {s['roi_pct']}%")
    ...
    for threshold in [0.5, 0.7, 0.9, 1.0, 1.1, 1.2, 1.3, 1.5, 2.0]:
        ...
        sub_top1 = sub.loc[sub.groupby("race_id")["composite_index"].idxmax()] \
            if len(sub) > 0 else pd.DataFrame()
        if len(sub_top1) == 0:
            continue
        s2 = roi_stats(sub_top1)
        print(f"  EV≥{threshold:<6.1f}  {s2['bets']:>7,}  {s2['wins']:>6}  {s2['win_rate']:>6.1f}%  {s2['roi_pct']:>7.1f}%")
    ...
    for alpha in [0.05, 0.10, 0.15, 0.20, 0.30]:
        ...
        for race_id, grp in valid_with_odds.groupby("race_id"):
            ...
            for i, (_, row) in enumerate(grp.iterrows()):
                ...
                rows.append({"race_id": race_id, "ev": ev_val,
                             "score": row["composite_index"],
                             "finish_position": row["finish_position"],
                             "win_odds": row["win_odds"]})
        tdf = pd.DataFrame(rows)
        sub = tdf[tdf["ev"] >= 1.0]
        sub_top1 = sub.loc[sub.groupby("race_id")["score"].idxmax()] \
            if len(sub) > 0 else pd.DataFrame()
        if len(sub_top1) == 0:
            continue
        s3 = roi_stats(sub_top1.rename(columns={"finish_position": "finish_position",
                                                  "win_odds": "win_odds"}))
...
def section_index_power(df: pd.DataFrame) -> None:
    ...
    for col in INDEX_COLS:
        ...
        print(f"  {lbl:<15} {rho_all:>+8.4f} {rho_turf:>+8.4f} {rho_dirt:>+8.4f} {rho_2024:>+8.4f} {rho_2025:>+8.4f}")
...
def section_bad_conditions(df: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    def gap_bucket(g):
        if g < 3:  return "0〜3未満（拮抗）"
        if g < 6:  return "3〜6未満"
        if g < 10: return "6〜10未満"
        if g < 15: return "10〜15未満"
        return "15以上（支配的）"
...
def section_filter_effect(df: pd.DataFrame, bad_results: dict) -> None:
    ...
    for race_id, grp in df.groupby("race_id"):
        ...
        if len(sg) < 2:
            ...
            gap_map[race_id] = float(sg.iloc[0]["composite_index"]) - float(sg.iloc[1]["composite_index"])
    ...
    filters = [
        ("芝/ダート除外",         "surface_label", lambda d: ~d["surface"].isin(
                                                    [k for k,v in SURFACE_LABEL.items() if v in bad_surfaces])),
    ...
    for fname, col, mask_fn in filters:
        # 個別フィルター
        if col in ("surface", "distance_cat", "grade_cat", "condition", "head_count_cat", "course_name"):
            bad_list = {"surface": bad_surfaces, "distance_cat": bad_distances,
                        "grade_cat": bad_grades, "condition": bad_conditions,
                        "head_count_cat": bad_head_counts, "course_name": bad_courses}.get(col, [])
            if not bad_list:
                continue
        ...
        print(f"  {fname:<25} {bad_str:<35} {filtered['race_id'].nunique():>8,} "
    ...
    if n_cum > 0:
        ...
        print(f"  （除外レース数: {n_base - n_cum:,} / {n_base:,} = {(n_base - n_cum)/n_base*100:.0f}% を非賭け対象に）")
    ...
    for threshold in [0.8, 1.0, 1.1, 1.2, 1.5]:
        ...
        s = roi_stats(sub_t1)
        improve = s["roi_pct"] - base["roi_pct"]
        print(f"  EV≥{threshold:<6.1f}  {s['bets']:>8,}  {s['roi_pct']:>7.1f}%  "
              f"{'↑' if improve > 0 else '↓'}{abs(improve):>6.1f}%")
...
def section_monthly_pnl(df: pd.DataFrame, df_filtered: pd.DataFrame | None = None) -> None:
    ...
    for label, target in [("全条件", df), ("フィルター後", df_filtered)]:
        ...
        monthly = (valid.groupby("ym")
                   .agg(bets=("profit","count"), profit=("profit","sum"))
                   .reset_index())
        monthly["cumulative"] = monthly["profit"].cumsum()
        monthly["roi"] = (valid.groupby("ym")
                          .apply(lambda g: (g["win_odds"][g["finish_position"]==1].sum() /
                                           len(g) * 100))
                          .values)
        print(f"\n  ▼ {label}")
        ...
        for _, row in monthly.iterrows():
            ...
            print(f"  {row['ym']:<8} {row['bets']:>6} {sign}{abs(row['profit']):>7,}円 "
