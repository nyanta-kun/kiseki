...
## 並列開発プロトコル（複数 Codex セッションを同時に走らせる場合）

**同じフォルダで複数の Codex を動かしてはいけない。** 作業ツリー（未コミット変更・
...
2026-08-23 に**同じ根の誤りを1日で6回**繰り返し、そのうち2回は**ユーザーへ誤った
結論を報告**し、1回は**存在しない改善（ガミ率 46%→6%）を報告**した。
根本原因はすべて「**本番が実際に何をしているかを読まずに、仮定で母集団・
ベースライン・買い方を作って測った**」こと。

### 実験を設計する前に、本番コードから次を**書き出してから**始める

| # | 確認すること | 破ったときに起きたこと |
|---|---|---|
| 1 | **その商品が実際に何点買うか / 候補は何車か** | 7C を「5候補中4〜5点」と仮定（実際は選抜→落差カット→△削りの3段）。再現が不完全なまま §29 を報告 |
| 2 | **賭け金の配分はどうなっているか** | 7S を「均等 2,000円×5」と仮定。**実際は `tilt_stakes: True` でダッチング済み**。「ガミ率 46%→6% に改善」と報告したが、**現行のガミ率は既に 3.4%** で改善はほぼ存在しなかった |
| 3 | **母集団は何か（商品が売らないレースを含めていないか）** | 「◎○一致時の軸2差し替えは効かない（+0.34pt・有意でない）」と報告。**堅いレース（商品が売っていない）を含めていた**だけで、除くと **+2.30pt で有意**。結論が逆転した |
| 4 | **そのゲートに例外や別枠が無いか** | 「7S が overlap=2 を入稿している＝ゲート違反」と報告。実際は `rank_7a_market_agree_pool`（2026-08-11 追加）が**意図的に overlap==2 を取る**設計だった |
| 5 | **学習窓と評価窓が分かれているか** | 統一比較台で**探索窓でモデルを学習し同じ窓で評価**。モデル系の腕だけ ROI 106〜109% と出て、確認窓で崩れた |
| 6 | **ユーザーの提案を近似せずそのまま実装したか** | 「軸2に◎○を選ばない」を「○のみ除外・overlap=2 限定」と近似して測った。母集団も条件も違い、測り直しが必要になった |

### 🔴 実験スクリプトの定数から baseline を取ってはいけない

`RANK_7S_STAKE = unit_stake(5)` は**旧実験スクリプトでしか使われていない**のに、
そこから「本番は均等配分」と判断した。本番の入稿は
`RANK_CONFIGS["7S"]["tilt_stakes"]` → `_build_tilted_legs` → `tilted_stakes` →
`landing_weights`（予測オッズがあれば `1/オッズ` 単独）。
**定数の存在は使われている証拠にならない。呼び出し元を辿ること。**

### 🔴 baseline は**実データを1件表示して目視確認する**

実験の冒頭で本番の入稿を1件出す。今回それをしていれば、防府12R の
`4,500 / 2,300 / 1,400 / 1,100 / 700円`（＝どこが当たっても約17,000円）を見た瞬間に
「均等ではない」と気づけた。**表で1行出すだけの手間で、半日の誤った作業が消える。**

### 🔴 仕組みの存在から挙動を推論して報告しない

「スクレイパーが `INSERT OR REPLACE` だから WT の印は上書きされて変わっている」と
推論して報告したところ、ユーザーが「ロジックの根幹が揺らぐ」と判断しかけた。
**実測すると 1,355レースで変化 0件**だった。
**「変わりうる」と「変わっている」は別。測ってから言う。**

### 関連する既存の作法（同じ根）

- **「効果があった」と「その商品に効く」は別**（`docs/keirin_strategy` §29・
  [[keirin_market_agree_transfer_2026_08_22]]）。効果は測った買い方に紐づく
- **確認窓は年をまたぐこと**（同一年内の前後分割は確認にならない）
- **厳選を測るときは日次上限を外す**（上限があると選択効果が入る）
...
## 表示メニューのユーザーごとの ON/OFF（2026-09-10）

管理者が**ユーザーごと**に POG / 中央 / 地方 / 競輪 の表示を切り替える。
保存先は `keiba.users.menu_{pog,jra,chihou,keirin}`。

🔴 **判定の正本は `backend/src/services/menu_access.py`**（DB にも FastAPI にも
依存しない純関数）。フロントの写しが `frontend/src/lib/menuAccess.ts` にあり、
`backend/tests/test_menu_access.py` が両者の対応を固定している。**片方だけ直さない。**

| 段 | 関数 | 何を出すか |
|---|---|---|
| 保存する値 | `normalize_menu_flags` | 管理画面のチェックボックスの状態 |
| 見せる値 | `resolve_menu_access` | 実際の可視性（ナビとルートガードが使う） |

分けてあるのは、admin が自分のフラグを切っても全部見えるため
**保存値と可視性が一致しない**から。1 つにまとめると管理画面が
「OFF なのに見えている」を表現できない。

### 規則

1. 🔴 **POG が ON なら中央と地方も必ず ON。** POG の順位表・記録室は
   `/races/{id}` `/chihou/races/{id}` へ直接リンクしており、外すとその
   リンクが全部ガードに弾かれる。保存時にも読み出し時にも掛ける
2. **admin はフラグを無視して全部見える**（自分で自分を締め出さないため）
3. **競輪は当面 admin 限定。** フラグは保存するが可視性には効かせない
   （`KEIRIN_REQUIRES_ADMIN = True`）。`/keirin` 配下は netkeirin への
   入稿トリガーと入稿設定の編集を含むため。開放するときはこの定数を
   両側（Python / TS）で False にする

### 🔴 実効ガードは `proxy.ts` ではなくセクションの `layout.tsx` にある

フラグは DB にあり、Auth.js v5 の `jwt` コールバックは**サーバコンポーネントから
Cookie を書き戻せない**。JWT に載せるとサインイン時の値で固まり、
**管理者が切り替えても再ログインまで反映されない**。

→ 各セクションの `layout.tsx` が `requireMenu(key)`（`frontend/src/lib/menu.ts`）を
呼ぶ。毎リクエスト `GET /api/users/{id}/menu` を引き、React の `cache()` で
1 リクエスト内に畳む（ルートレイアウトのナビ描画と共用するので HTTP は 1 回）。

| パス | ガード |
|---|---|
| `/races` `/results` `/yoso` | `requireMenu("jra")` |
| `/chihou`（`/chihou/results` を含む） | `requireMenu("chihou")` |
| `/pog` | `requireMenu("pog")` |
| `/keirin` | `proxy.ts` の admin ロール判定（従来どおり） |

`backend/tests/test_menu_guard_wiring.py` がこの表を機械的に固定している。
**ナビからリンクを消すだけでは URL 直打ちを止められない。**

⚠️ 送り先は `landingPath()` が返す「その人に見える場所」。`/races` 決め打ちに
すると、中央が OFF の人でガードとリダイレクトが循環する。

⚠️ `/` `/login` から `/races` への転送（`proxy.ts`）は、中央が OFF の人だと
セクションガードで一度跳ね返される。middleware から DB を引くと全遷移に往復が
増えるので、余分な 1 ホップは許容している。

### 取得に失敗したときは「この機能が入る前の見え方」へ落とす

中央 ON / 地方 ON / POG OFF / 競輪 OFF（`DEFAULT_MENU_FLAGS`）。
全部隠す実装にすると、**バックエンドが数秒詰まっただけで全員がマイページへ
弾かれる**。既定値へ落としても追加で見えるのは中央と地方だけで、絞り込みの
主目的である POG と競輪は隠れたままになる。
🔴 デプロイ順の保険でもある（フロントが先に出て `/users/{id}/menu` がまだ
404 の間、全員が従来どおり使える）。

### ナビの項目は `buildNavItems` に一本化してある

`AppNav` / `HamburgerMenu` / `BottomNav` の 3 か所が別々に並べていた頃は、
**ハンバーガーにだけ POG が無かった**（2026-09-10 実測）。

### 移行時の既定値

`202609110930_shared` が既存行へ流し込む。**移行しても今日と見え方が変わらない**
ように選んである:

- `pog_group_members` に居る人（実測 9 人）→ `menu_pog=true`（＋中央・地方）
- `role='admin'`（実測 3 人）→ 4 つとも true
- それ以外 → 中央・地方だけ true

⚠️ 1 つめを入れ忘れると、移行した瞬間に POG 参加者のナビから POG が消える。

## 🔴 ページのルート要素に `mx-auto` を付けてはいけない（2026-09-10・本番で発生）

ルートレイアウトの `<div className="flex-1 min-h-0 flex flex-col">` は **flex コンテナ**。
その直下の子に `margin-left/right: auto` が付くと **`align-items: stretch` が無効になり、
幅が「内容の最大幅」で決まる**（Flexbox の仕様）。

POG の各ページが移設時に `<main className="mx-auto max-w-3xl p-4">` を直下へ置いていたため、
一覧に長い行（「最高 ヴェトロテンペスタ」等）が 1 つ入るだけで `<main>` がビューポートより
広くなり、**カードが画面からはみ出した**。最小再現で実測:

    幅 494px のコンテナ → <main> が 507px（mx-auto を直下に置いた場合）
    幅 494px のコンテナ → 494px（全幅 div で包み、内側で mx-auto した場合）

⚠️ **`max-w-*` 以上の画面幅では起きない。** `max-w-3xl`(768px) で頭打ちになって収まるので、
PC で見ている限り永久に気づけない。報告は必ずスマホ・狭い窓から来る。

| ルート要素 | 判定 |
|---|---|
| `mx-auto max-w-3xl p-4` | 🔴 幅が内容依存になる |
| `w-full sm:max-w-3xl sm:mx-auto` | ✅ `w-full` ＝明示幅があるので縮まない |
| `min-h-screen`（`mx-auto` は内側） | ✅ 直下の子に auto マージンが無い |

検査: `frontend/src/lib/responsive-layout.test.ts`（デフォルトエクスポートのルート要素だけを見る。
ファイル内の `return (` を全部見るとスケルトン等を拾って誤検知する——実際 4 件出した）。

### 併せて踏んだもの

- **負マージンは器の水平パディングと同じ値にする。** 横スクロール帯を端まで伸ばす
  `-mx-4` を `px-3` の器の中で使うと、左右 4px ずつはみ出してページ全体が横に流れる
- **一覧の数値列を固定幅 (`w-`) にしない。** 桁が増えると折り返して行の高さが倍になる
  （320px 幅で 41px → 85px を実測）。`min-w-` + `whitespace-nowrap` にして、
  はみ出す前に名前側が `truncate` で縮むようにする

### POG のスマホは「1 画面に収める」ことを設計目標にしている

実測の予算は **677px**（iPhone 390×844・Safari 上部バー展開時の可視 web view）。
サイトヘッダ 60px とボトムナビ 56px を引いた残りに収める。2026-09-10 実測:

    順位表  7人 599px / 8人 640px / 9人 680px
    スコア  9人 668px

そのため表彰台・統計タイル 4 枚・今週パネル 2 枚はスマホでは出さず、
1 行の要約帯と折りたたみ（`app/pog/ui.tsx` の `Collapsible`）に置き換えてある。
**縦を足す変更をするときは、この数字を更新できるか確かめること。**
...
（AGENTS.md は複数インスタンスの同時実行を可としているが、累積マスタ571ファイルは別物）。
...
> （Codex Routine・リポジトリ外）/ `GET ""`。まとめて止めないこと。
>
