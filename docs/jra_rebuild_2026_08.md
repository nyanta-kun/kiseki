...
### 15.5 週次で自動化した（2026-08-15）

一度きりの実行では**発走後の行がまた溜まる**（週 約1.5GB ペース）ので週次化した。

| | |
|---|---|
| ラッパ | `scripts/prune_odds_history_weekly.sh`（直近30日ぶんを `--execute`） |
| LaunchAgent | `com.kiseki.jra-odds-prune`（**月曜 05:00**） |
| ログ | `logs/prune_odds_history.log` |

**月曜 05:00 にした理由**: 中央は土日開催なので直前の週末ぶんを「発走後」として刈れる。
かつ**日次 DB バックアップ（03:30〜04:25）の直後**なので、
削除の直前に必ずバックアップがある状態になる。JRA realtime も動いていない。

**VPS ではなく Mac から回す理由**: VPS の backend は Docker で動いており
**コンテナに psycopg2 が入っていない**（アプリは asyncpg を使う）。
削除条件を SQL へ書き写すと正本が 2 つになるので、
既存の Python スクリプトをそのまま使える Mac 側に置いた。
Mac は毎日 03:15 に自動起床する（`pmset repeat`）。撃ち漏らしても launchd が
次の起床時に実行する。週次の掃除なので遅延は無害。

安全側の作り:

- **直近のバックアップが無い / 36時間以上古いなら実行しない**（削除は取り消せない）
- 多重起動しない（前回が長引いていたら見送る。次週で拾える）
- ⚠️ `~/GitHub/kiseki` が **main 以外のブランチに切り替わっているとスクリプトが存在しない**。
  その場合は明示的なエラーで落ちる（Python のトレースバックにしない）。
  **このクローンは他セッションがブランチを切り替えることがある**点に注意
  （`dm_auto_fetch` も同じ足元に乗っている）

定常状態の実測（刈り込み済みの状態で実行）: 対象30日＝8開催日を約1分で走査し `post=0 / exotic=0`。
