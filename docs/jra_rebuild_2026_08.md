...
| **P1** | **DM 取得経路の修復** | 🔜 未着手 | 4.4 節。**今週末のレースが既に DM 無しで走る状態** |
| P2 | TRAIN/VAL/TEST プロトコルの制定 | 🔜 未着手（7章に案） | 中央には取り決めも TEST 開封台帳も無い |
| P3 | train/serve 整合の是正 | 🔜 未着手 | 4.5〜4.7 節。配信条件での学習・`weight_change` の読み先 |
...
1. `progress[jravan_race_id] = "ok"` を **`updated > 0` のときだけ**にする。
   0 件のときは記録せず（＝次回再試行）、`--force` で `progress` を無視できるようにする
2. 既に `"ok"` で焼き付いた分を回復する。`data/dm_import_progress.json` から
   **DB 側で DM が 0 件のレースの項目だけ**を削って再実行する（全消しはしない。
   12,519 レースの再 POST は不要）
3. **今週末（8/15・8/16）に間に合わせる。** 22:00 の翌日算出より前に反映すること
4. 死活監視を足す: 「翌日の開催レースの DM 充足率が 90% 未満なら警告」。
   現在の `dm_auto_fetch.sh` は `saved=0` を正常終了として扱っており、
   **8/9 の全損を一度も報告していない**
...
| # | 課題 | 影響 | フェーズ |
|---|---|---|---|
| **1** | **DM 取り込みが `updated=0` を "ok" として焼き付ける**（4.4）。今週末のレースが DM 無しで配信される | 🔴 致命・**進行中** | P1 |
| **2** | DM 欠損時のフォールバックが無い。中立値 50 は「DM 無しで学習したモデル」より 3.65pt 悪い（4.5） | 🔴 高 | P3 |
| **3** | **学習データが v26 固定で 2026-08-02 に凍結**（4.7）。無言で古くなる | 🔴 高 | P3 |
| **4** | `weight_change` の読み先が `race_results`（4.6）。発走前は必ず NaN | 🟡 中（+0.39pt） | P3 |
...
`backend/src/chihou_protocol.py` に倣い `backend/src/jra_protocol.py` を新設する。

| 定数 | 値（案） | 根拠 |
|---|---|---|
| `TRAIN_END` | `20250630` | 既存スクリプトの事実上の標準。動かさない |
| `VAL_START` | `20250701` | |
| `TEST_START` | **当月1日（月次ローリング）** | 地方と同じ。固定すると学習終端も固定され、モデルが古くなる |
| `VAL_END` | `TEST_START` の前日 | |
| 環境変数 | `JRA_TEST_START=YYYYMMDD` | 過去分析の再現用 |

- `record_test_usage()` と `scripts/JRA_TEST_USAGE_LEDGER.md`（開封台帳）も同時に作る
- `BURNED_DECISIONS` に上記の既使用分を最初から記載する
- ⚠️ **地方と同じ値にする必然性は無い**（中央は開催が週2日で 1か月 ≒ 8〜10 開催日・
  約 300R しかなく、月次 TEST では n が小さい）。**四半期ローリングにするか**は P2 の論点。

> **確定は P2 で行う。** 本書では案として置くにとどめ、まだ `jra_protocol.py` は作らない。
> ここで安易に固めると「半端に決めたプロトコル」が既成事実になるため。
...
| 優先 | やること | なぜ今か |
|---|---|---|
| **1** | **DM 取り込みの `progress` バグを直し、8/15・8/16 を回復する**（4.4） | **今週末のレースが DM 無しで配信される。22:00 の翌日算出より前に間に合わせる** |
| **2** | DM 充足率の死活監視を足す（4.4-4） | 8/9 の全損を誰も検知していない。同じ日が繰り返される |
| 3 | 学習ソースを v26 固定から外す（4.7） | 直さない限り、以後の全ての検証が 2026-08-02 までのデータで行われる |
| 4 | TRAIN/VAL/TEST プロトコルを確定（7章） | 以後の測定の基準線。**P3 以降の作業を始める前に決める** |
| 5 | 配信条件での学習（4.2 ⑤）**または** `weight_change` の読み先修正（4.6） | +0.4〜0.5pt。**両者は重なるのでどちらか一方でよい**。モデル構造は変えない |
| 6 | 前向き記録の実装（8章） | 貯まるまで数か月かかる。**着手が遅れるほど結論が遠のく** |
