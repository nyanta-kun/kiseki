...
| P2 | TRAIN/VAL/TEST プロトコルの制定 | 🔜 **未着手・要判断**（7章に案） | 中央には取り決めも TEST 開封台帳も無い |
...
| **4** | **TRAIN/VAL/TEST プロトコルを確定**（7章） | 以後の測定の基準線。**ここから先の採否判断を始める前に決める。ローリング幅の判断が要る（7章末尾）** |
| 6 | モデルの再学習（`train_jra_reg_rank` / `train_jra_out_rate`） | 12.1 で学習データが 2026-08 まで伸びた。ただし**プロトコル確定後**に回すこと |
| 7 | 前向き記録の実装（8章） | 貯まるまで数か月かかる。**着手が遅れるほど結論が遠のく** |
