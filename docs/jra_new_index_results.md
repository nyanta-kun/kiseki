...
| 4-B | 調教データ(坂路)特徴量 | 拡張窓(2025-07〜2026-07, train1924R/valid431R/test1200R)で複数seed(5)平均＋test集合bootstrap 95%CI＋drop1 | 🟡 **full8はぎりぎり有意(95%CI [+0.30,+2.33]pt)だが単一窓限定。simple5は非有意。本番実装せず** | 保留（次の独立test窓での再現確認）。⚠️ **3年backfillは 2026-08-17 に不可能と確定**（差分の保持限界が 2025-05-27・下記追記）ので、窓は自然増を待つ |
...
### 2026-08-17 追記: 差分(option=1)も保持限界に達していた ＝ **backfill は不可能**

`windows-agent/probe_chokyo_retention.py` を新設し、JVOpen の戻り値だけを見る
（**JVRead を回さない**のでDLもDB反映も起きない）形で実測した。

| dataspec | from_time | 読込ファイル数 |
|---|---|---|
| SLOP | 20260701 | 92 |
| SLOP | 20260101 | 443 |
| SLOP | **20250527** | **726** |
| SLOP | 20250101 / 20240101 / 20230506 | **726（増えない）** |
| WOOD | **20250527** | **667** |
| WOOD | 20250101 / 20240101 / 20230506 | **667（増えない）** |

🔴 **`keiba.slope_training` が 2025-05-27 始まりなのは取得漏れではなく、
JRA-VAN の差分保持限界そのもの。** どれだけ古い `from_time` を渡しても件数が増えない。
仕様上 SLOP/WOOD は「読み出し終了ポイント時刻を指定できないデータ種別」に含まれず
`from_time` は確かに効くが、**サーバーがそれ以前を持っていない**。

option=4 も改めて 2 回（600秒 / 900秒）試し、いずれも JVOpen が返らないことを確認した。
2 回目は本番と同じ `BlockingCallGuard`（モーダル自動応答つき）に載せたが
**ダイアログは検出されず**ハートビートだけが進んだ ＝ ダイアログ待ちではない。

→ **3年backfillは現行経路では不可能。** 「backfill実現後に再評価」という保留条件は
成立しないので、**評価窓は自然増を待つ**方針へ切り替える。
UmaConn は SLOP を配信していない（地方側が probe 済み・rc=-1）。

⚠️ その「貯める」経路が止まっていた。調教取得は自動実行されておらず
（`kiseki-Chokyo-Setup` は最終実行 2026-06-07・Next 未設定、実行履歴は手動のみ）、
8/15〜8/16 開催週の調教が欠けていた。2026-08-17 に
`--mode chokyo --from-date 20260805` で回収済み（坂路 +6,882 / ウッド +2,166・所要60秒・冪等）。
**日次または週次のタスク登録が必要**（未実施）。
