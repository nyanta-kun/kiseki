...
- [x] 482レースの再分類（→ 実回収対象 425）
- [x] 根本原因の特定（蓄積系が定期実行されていない）
- [x] importer の破壊的 UPSERT を修正 + テスト
- [x] netkeiba 停止原因の特定（sekito のスケジュール欠落）
- [ ] **importer 修正を VPS へデプロイ**（回収の前提）
- [ ] A1 7日分の回収を 1 日で試験 → 全体へ展開
- [ ] `race_payouts` 2024-01〜 のバックフィル
- [ ] C の 2026-04-20 以前について欠損理由を特定
- [ ] `kiseki-UmaConn-Daily` タスク新設（再発防止）
- [ ] sekito に地方 netkeiba の日次スケジュール追加
- [ ] kichiuma 2024-07（1か月欠測）の遡り取得可否を確認
- [ ] `chihou_data_health_check.py` に「取得**日数**」ベースの検査を追加
      （比率ベースでは今回の段階的崩壊を検知できなかった）
