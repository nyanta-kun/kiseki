...
### 10.3 🔴 しかしその 51.5% は本番では出ていない可能性が高い

**検証に使われた walk-forward 指数は `ALL_FEATURES`（市場特徴5本を含む）で学習・予測している**
（`chihou_darkhorse_wf_build.py`）。そして市場特徴の入力は
`add_market_features` が `race_results.win_odds` / `win_popularity` から作る＝**確定オッズ**。

一方 **本番のライブ指数には市場特徴が入っていない**（5.4 節。`odds_map` がどこからも
渡されないため `odds_rank_n`=0.5 固定・`is_heavy_fav`=0・`is_dark_horse`=0）。

つまり **検証と本番で指数が別物**。同一スクリプト・同一 stage(discovery)・同一 lead で
両方を作って比較した（`--no-market` フラグを新設）:

**指数1位馬の素の成績（walk-forward・9四半期）**

| 四半期 | 市場あり 勝率 | 市場なし 勝率 | 差 |
|---|---|---|---|
| 2024Q3 | 41.5% | 34.0% | −7.5pt |
| 2024Q4 | 45.5% | 38.3% | −7.2pt |
| 2025Q1 | 43.7% | 37.3% | −6.4pt |
| 2025Q2 | 42.7% | 35.3% | −7.4pt |
| 2025Q3 | 43.1% | 36.5% | −6.6pt |
| 2025Q4 | 46.0% | 40.1% | −5.9pt |
| 2026Q1 | 47.0% | 39.9% | −7.1pt |
| 2026Q2 | 45.8% | 39.9% | −5.9pt |
| 2026-07 | 42.1% | 36.7% | −5.4pt |

**全四半期で 5.4〜7.5pt の劣化。例外なし。**

**本命の条件（発走前6番人気以下・discovery・母集団 複勝圏率 11.8%）**

| 条件 | 市場あり指数 | 市場なし指数（＝本番相当） |
|---|---|---|
| 全レース × 指数3位内 | **43.2%** (n=192) | **26.1%** (n=720) |
| **シェア<0.63 × 指数3位内** | **50.7%** (×4.28, n=71) | **28.3%** (×2.39, n=212) |

**条件の的中率がほぼ半減し、発生頻度は約3倍に膨らむ。**
市場を見ない指数は「不人気馬を上位に置きやすい」ため条件を満たす馬が増え、
その分だけ精度が落ちる。**目標である「1〜2頭に絞る」の精度が本番では出ていない。**

### 10.4 ただし 50.7% も上限であって到達点ではない

市場あり側は **確定オッズ**を入力にしている。馬券を買う時点で確定オッズは分からない。
実際に到達しうるのは **発走前オッズ**を入力にした場合で、その値は未測定:

    本番いま      28.3%   （市場特徴なし）
    実現可能      ?       （発走前オッズ）← 未測定
    検証値        50.7%   （確定オッズ・look-ahead 込みの上限）

### 10.5 次の一手

1. **発走前オッズで市場特徴を作った walk-forward 指数を作る**（`odds_history` の
   発走 N 分前スナップ）。これで「実現可能な上限」が確定する。
   使えるのは **2026-04-07 以降**のみ。
2. 効果が確認できたら **serve 時に `odds_map` を渡す**（P3 の本丸）。
   指数は 22:00 / 昼 / 21:30 の3回しか算出されないので、
   **発走直前に再算出する経路**が要る（毎分動いている
   `chihou_odds_decision_trigger.sh` に相乗りするのが自然）。
3. その上で「1レース1〜2頭・入着可能な時のみ」の閾値を設計する。

⚠️ **HOLDOUT（2026-07）は先行検証で一度開封済み**。新しい条件を選ぶ探索には使えない。
新しい一度きりの評価窓を切ること。
