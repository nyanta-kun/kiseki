...
## 15. 2026-08-14 時点の状態（スナップショット）

> ⚠️ この節は 2026-08-14 時点の記録。**現在の状態は `docs/PROJECT_STATUS.md` §2.3・§4.3 が正本。**
> 15.3 の各項目には 2026-09-23 時点の状態を注記した。
...
2. ~~**P5 の残り4カテゴリ**~~ → **決着（2026-09-01）**。5カテゴリ推奨は画面・エンドポイントとも
   実稼働を停止し、コードは参考として保持（#413・#414・`CLAUDE.md`「地方競馬 推奨カテゴリ」）。
   `place_bet` は前向き記録の上で注目馬と比較し直した（同節の表）。
3. ~~**P4 特徴量**~~ → 🔴 **この項目の「未活用」は誤り（2026-09-02 訂正）**。
   充足率は「値が入っている行の割合」で「使える値の割合」ではなかった。
   `running_style` は全行 `'0'` で取得不能、`passing_1` の実充足は 64.5%、
   展開の特徴は過去走の通過順から既に本番に入っている
   （`backend/docs/chihou_column_availability.md`・#434）。
   `chihou.pedigrees` 0件は UmaConn がデータを配信していないためで、
   血統は `keiba.pedigrees` 経由で 91.3% 取れる（17.5）。
4. **P2 の残り**
   - `calculated_indices` v1〜v12 削除（約3M行・760MB。付け替えは済んでいる）
     → **未着手（2026-09-23）**。`prune_calculated_indices.py` が地方に未対応
   - ~~`odds_history` 9.9GB の保持方針~~ → **済み**（#531・`prune_odds_history.py --schema chihou` を週次で実行）
