...
`chihou.horses.name + birthday` ↔ `keiba.horses.name + birthday` で突合できる:

| | |
|---|---|
| `chihou.horses` が中央マスタと一致 | 21,233 / 26,185（81.1%） |
| **出走馬ベースのカバー率（2024-01以降 375,167頭）** | **91.3%** |
...
⚠️ **netkeiba から血統を取る案は保留。** 取れていない 8.7% を埋めても、
