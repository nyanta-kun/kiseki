...
| 2 | 詰まっているのは recall。来た人気薄を指数5位内に置けた率 **27.8%**、レース単位捕捉率 **31.9%** | 同上 |
...
| **R2** | レース単位捕捉率 = 人気薄が複勝圏に来たレースのうち、1頭以上を捕捉できた割合 | **31.9%** |
...
- 特徴量: **オッズ以外**を主とする。
  - レース属性: `head_count` / `distance` / `is_turf` / クラス（`grade_code`）/ 場 / `race_num` / `is_handicap` / 新馬・障害フラグ / 転入馬の頭数と割合（地方）
  - `place_slots`
...
対策:
- レース側の特徴を**オッズ以外**（頭数・クラス・場・新馬/障害・転入馬）で主構成する
...
| # | 項目 | 確認方法 |
|---|---|---|
| 1 | `keiba.odds_history` / `chihou.odds_history` の実際の最小 `fetched_at`（台帳の値は 2026-08 時点のスナップショット） | `SELECT min(fetched_at) FROM ... WHERE bet_type IN ('win','place')` |
| 2 | 地方の `entropy_norm` が本番採用されているか（研究スクリプトのみ存在） | `chihou_recommender.py` / `chihou_calculator.py` の呼び出し元を辿る |
| 3 | `jra_pick_log_report.py` の実行実績（hit_tier の集計が一度でも回っているか） | `keiba.hit_tier_races` の件数と `settled_at` の充足率 |
| 4 | `chihou.place_picks` の現在の採点済レース数（572R は 2026-09-01 時点の値） | `chihou_pick_log_report.py --start 20260815 --end <直近>` |
| 5 | 地方の `odds_history` の現在容量（刈り込みスクリプトが無く年 +30GB） | `pg_total_relation_size('chihou.odds_history')` |
| 6 | 地方 races の JRA 除外条件の正確な書き方（`course_name` か `course` コードか） | `chihou_entropy_norm_analysis.py` の母集団定義（`course != '83'`）を確認 |
| 7 | JRA 側で「転入馬」に相当する使える列があるか（地方固有の可能性） | `backend/src/db/models.py` の horses / race_entries |

---

*本文書は計画であり、実測結果を1つも含まない。数値はすべて既存文書・既存コード・既存アーティファクトからの引用で、出典を併記している。🟡 と「要確認」を付けた項目は未検証。*
