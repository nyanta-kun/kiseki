...
> 🔴 **この取得は 2026-05 に停止している（2026-09-02 時点）。**
> 下記は動いていた当時の仕様で、現在の稼働状況ではない。
>
> 影響: `paddock_index` は上流が `is_paddock` 0 件になったため実質定数になっている。
> 特徴量の死活監視（`scripts/feature_health_weekly.sh`）でも既知の問題として
> 検出され続ける。除去しても指数は改善しないことを検定済みなので、列は残してある
> （`CLAUDE.md`「指数（特徴量）の死活監視」/ `docs/jra_rebuild_2026_08.md` 15.1）。
>
> 再開するならこの仕様書がそのまま使えるはずだが、**再開したかどうかは
> `check_feature_health.py` の出力で確認すること**（この文書は状態を持たない）。
