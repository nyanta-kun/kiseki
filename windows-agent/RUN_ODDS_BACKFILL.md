...
> ⚠️ **`--option 3/4` は 2026-08-23 まで永久ブロックしていた。**
> 不可視の「セットアップ」選択ダイアログが出て誰も押せない状態だった（`jvlink_dialog_guard` が
> タイトル前方一致で取りこぼしていた）。修正済み。**JV-Link 5.0.0 起因ではなく、ログ上
> 少なくとも 2026-05-03（4.9.0 時代）から同じ症状が出ていた。**
...
## 🔴 単勝・複勝（O1）は backfill では取れない（2026-08-24 実測）

**蓄積系のファイル名は、中身のレコード種別と1つずれている。**

```
file=O1VM2024039920240401144912.jvd  rec_id='O2'  len=2042  head='O25202403042024030206020301...'
```

`O1VM...` という名前のファイルの中身は **O2（馬連・2,042バイト）**。同様に O2VM→O3、
O3VM→O4 … と続き、**真の O1（単複枠＝単勝・複勝）レコードは setup データに1件も無い**。

- option=4 を何度流しても `odds_history` の win/place は増えない（POST は 200 で返るが 0 行）
- 単勝・複勝の確定オッズは **option=1 でしか取れず、保持窓はちょうど1年**
- 実測: trio は 2024-01-06 まで遡れるのに、win/place は 2025-08-24 が最古

**ログの「レコード種別の内訳」はレコードの中身で数えている。**
